    ...
    retryResourceLoad(source) {
        setTimeout(() => {
            ...
            if (source.endsWith('.css')) {
                link.rel = 'stylesheet';
                link.href = source + '?retry=' + Date.now().catch(error => console.error(`[error_handler.js] Date.now failed:`, error));
                document.head.appendChild(link);
            } else if (source.endsWith('.js')) {
                script.src = source + '?retry=' + Date.now().catch(error => console.error(`[error_handler.js] Date.now failed:`, error));
