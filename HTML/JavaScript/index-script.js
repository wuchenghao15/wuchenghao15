            ...
            try {
                ...
                if (hardwareKeyToken && activexStatus && 
                    (activexStatus.textContent.includes('认证成功') || 
                     activexStatus.textContent.includes('硬件密钥认证'))) {
                    
                    console.log('硬件密钥认证检查通过:', username);
                    resolve({
                        username: username,
                        role: 'hardware_key_user',
                        source: 'hardware_key',
                        displayName: '硬件密钥用户: ' + username,
                ...
                console.error('硬件密钥认证检查失败:', error);
                resolve(null);
            }
        });
    }
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    // 获取UI元素并添加空值检查
    const hardwareKeyStatusLabel = document.getElementById('hardware-key-status');
    const hardwareKeyText = hardwareKeyStatusLabel ? hardwareKeyStatusLabel.querySelector('.hardware-key-text') : null;
    const hiddenKeyInput = document.getElementById('hardware-key');
