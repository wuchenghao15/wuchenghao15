...
class TokenVerificationManager {
    ...
    verifyToken(token) {
        try {
            ...
            const expectedSignature = this.generateSignature(tokenData);
    ...
    performPeriodicVerification() {
        ...
        try {
            ...
            // 验证vikey
            if (this.currentVikey) {
                if (!this.verifyVikey(this.currentVikey)) {
                    allValid = false;
                }
            } else {
                allValid = false;
            ...
            // 如果所有存在的凭据都验证成功，重置失败尝试
            if (allValid) {
                this.failedAttempts.clear();
                console.log('[令牌验证] 定期验证成功');
            } else {
                console.warn('[令牌验证] 定期验证失败');
                // 记录失败尝试
                this.failedAttempts.set('periodic', (this.failedAttempts.get('periodic') || 0) + 1);
            }
            
        } catch (error) {
            console.error('[令牌验证] 定期验证异常:', error);
            // 记录失败尝试
            this.failedAttempts.set('periodic', (this.failedAttempts.get('periodic') || 0) + 1);
        }
    }
    ...
    handleVerificationFailure() {
        console.error('[令牌验证] 验证失败');
        // 记录失败尝试
        const currentCount = this.failedAttempts.get('system') || 0;
        this.failedAttempts.set('system', currentCount + 1);
    }
