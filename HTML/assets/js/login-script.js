...
async function simulateLogin(username, password, captcha, hardwareKey) {
    // 参数验证和标准化
    username = username !== undefined && username !== null ? String(username).trim() : '';
    password = password !== undefined && password !== null ? String(password).trim() : '';
    captcha = captcha !== undefined && captcha !== null ? String(captcha).trim() : '';
    ...
    try {
        ...
        if (window.authManager) {
            try {
                ...
                try {
                    ...
                    console.error('备用存储方案也失败:', storageError);
                }
            }
        } else {
        ...
        try {
            if (typeof setAntiHotlinkCookie === 'function') {
                ...
                // 使用统一认证管理器的防盗链功能
                window.authManager.setAntiHotlinkCookie();
            } else {
                // 如果两者都不存在，提供一个简单的实现
...
function initLoginPage() {
    try {
        ...
        if (window.authManager && typeof window.authManager.startActivityMonitoring === 'function') {
            try {
                window.authManager.startActivityMonitoring();
                console.log('活动监听已启动');
...
async function handleLogin() {
    ...
    try {
        ...
        const credentials = {
            ...
            authCode: document.getElementById('hardwareKeyCode')?.value || ''
        };
