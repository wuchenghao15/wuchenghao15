...
(function() {
    ...
    // 检查是否有窗口操作权限
    function checkWindowPermissions() {
        // 尝试执行一个简单的窗口操作来检查权限
        try {
            window.resizeTo(window.innerWidth, window.innerHeight);
            return true;
        } catch (error) {
            console.warn('没有窗口操作权限:', error);
            return false;
        }
    }

    // 强制最大化窗口
    window.forceMaximizeWindow = function() {
        if (!checkWindowPermissions()) {
            console.warn('无法执行窗口操作，可能是由于浏览器安全限制');
            return false;
        }

        try {
            // 首先尝试使用maximize()方法（适用于弹出窗口）
            if (window.outerWidth && window.outerHeight) {
                window.moveTo(0, 0);
                window.resizeTo(screen.availWidth, screen.availHeight);
            } else {
                // 如果是主窗口，使用fullscreen API（需要用户交互）
                document.documentElement.requestFullscreen().catch(err => {
                    console.warn('无法进入全屏模式:', err);
                });
            }
            console.log('窗口已强制最大化');
            return true;
        } catch (error) {
            console.error('强制最大化窗口失败:', error);
            return false;
        }
    };

    // 恢复原始窗口大小
    window.restoreOriginalWindowSize = function() {
        if (!checkWindowPermissions()) {
            console.warn('无法执行窗口操作，可能是由于浏览器安全限制');
            return false;
        }
    ...
    window.addEventListener('resize', function() {
        ...
        if (!document.fullscreenElement) {
            // 检查窗口是否小于屏幕可用尺寸的90%
            if (window.outerWidth < screen.availWidth * 0.9 || window.outerHeight < screen.availHeight * 0.9) {
                // 强制恢复最大化状态
                window.forceMaximizeWindow();
            }
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // 延迟执行，确保页面完全加载
        setTimeout(window.forceMaximizeWindow, 500);
    });

    // 页面卸载时恢复原始窗口大小
    window.addEventListener('beforeunload', function() {
        // 只在退出系统时恢复
        if (window.location.pathname.includes('logout') || window.location.pathname.includes('login')) {
