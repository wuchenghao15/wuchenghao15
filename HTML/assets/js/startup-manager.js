...
class MTSCOS_StartManager {
    ...
    createPidFile() {
        try {
            fs.writeFileSync(this.startupManagerPid, process.pid.toString());
    ...
    startHttpServer() {
        ...
        try {
            ...
            // 立即保存PID
            fs.writeFileSync(this.httpServerPid, serverPid.toString());
    ...
    stopHttpServer() {
        if (fs.existsSync(this.httpServerPid)) {
            ...
            try {
                ...
                if (output) {
                    const pids = output.trim().split('/n');
    ...
    startErrorDetector() {
        ...
        try {
            ...
            // 保存PID
            fs.writeFileSync(errorDetectorPid, detectorPid.toString());
    ...
    handleCommand(command, args) {
        ...
        switch (command) {
            ...
            case "stop":
                ...
                // 停止HTTP服务器
                const httpExitCode = this.stopHttpServer();
                
                // 停止错误检测器
                const detectorExitCode = this.stopErrorDetector();
