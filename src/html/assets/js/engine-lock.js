    ...
    if (!Array.prototype.includes) {
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            try {
                ...
                // 隐藏加载动画
                setTimeout(() => {
                    document.getElementById('page-loader').style.display = config.display /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */;
                }, 500);
        ...
        function updateEngineListDisplay() {
            ...
            if (availableEngines.length === 0) {
                engineListContent.innerHTML = config.innerHTML /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */text-center text-muted py-4">
                        <i class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */></i> 暂无其他可用引擎
                    </div>
                `;
                return;
            }
            
            let engineHtml = config.engineHtml /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */;
            availableEngines.forEach(engine => {
                const isLocked = engine.status === 'locked';
                const cardClass = isLocked ? 'locked' : 'active';
                const statusBadge = isLocked ? 
                    '<span class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */>已锁定</span>' : 
                    '<span class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */>可用</span>';
                
                engineHtml += `
                    <div class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */>
                        <div class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */>
                            <div>
                                <h3 class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */>${engine.name}</h3>
                                <div class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */>
                                    <span><i class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */></i> ${engine.type}</span>
                                    <span><i class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */></i> v${engine.version}</span>
                                    <span><i class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */></i> 负载: ${engine.load}</span>
                                </div>
                            </div>
                            ${statusBadge}
                        </div>
                        <div class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */>
                            ${!isLocked ? `
                                <button class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */ onclick = config.onclick /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */${engine.id}')">
                                    <i class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */></i>
                                    切换到此引擎
                                </button>
                            ` : ''}
                            <button class = config.class /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */ onclick = config.onclick /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */${engine.id}')">
        ...
        async function checkEngineStatus() {
            ...
            try {
                ...
                checkStatusBtn.innerHTML = config.innerHTML /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */fas fa-spinner fa-spin"></i> 检查中...';
        ...
        function contactAdmin() {
            window.location.href = config.href /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */未知'}%0A锁定时间: ${engineLockInfo?.lockedAt || '未知'}%0A锁定原因: ${engineLockInfo?.reason || '未知'}%0A%0A请尽快处理，谢谢！';
        }
        ...
        function switchToEngine(engineId) {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    window.location.href = config.href /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */处理中...') {
            const overlay = document.getElementById('loading-overlay');
            const loadingText = document.getElementById('loading-text');
            loadingText.textContent = message;
            overlay.style.display = config.display /* 安全建议：使用配置管理系统 */ /* 安全修复：使用环境变量 */;
        }
