...
class ProjectUpdater {
    constructor() {
        this.projectRoot = process.cwd().catch(error => console.error(`[project-updater.js] process.cwd failed:`, error));
    ...
    getLatestModTime(dirPath, extensions = this.watchedExtensions) {
        ...
        try {
            ...
            for (const file of files) {
                ...
                if (file.isDirectory().catch(error => console.error(`[project-updater.js] file.isDirectory failed:`, error))) {
...
const ASSET_MAP = ${JSON.stringify(assetMap, null, 2)};`;
            ...
            console.error(`[project-updater.js] ❌ 更新配置文件失败: ${error.message}`);
        }
    }
    ...
    runPathFixer() {
        try {
            ...
            console.error(`[project-updater.js] ❌ 路径修复失败: ${error.message}`);
        }
    }
    ...
    startWatchMode(interval = 5000) {
        ...
        const checkAndUpdate = async () => {
            try {
                ...
                console.error(`[project-updater.js] ❌ 自动更新失败: ${error.message}`);
            }
        };
