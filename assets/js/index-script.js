...
class UserAuthenticationManager {
    ...
    async authenticate(username, password) {
        ...
        // 4. 检查HardwareKey认证用户
        const hardwareKeyUser = await this.checkHardwareKeyAuthentication(username);
        if (hardwareKeyUser) {
            return {
                success: true,
                user: hardwareKeyUser
            };
        }
        ...
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                try {
                    ...
                    console.error(`[index-script.js] 远程数据库查询失败:, error`);
                    resolve(null);
                }
            }, 300);
        });
        ...
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                try {
                    ...
                    console.error(`[index-script.js] 第三方证人验证失败:, error`);
                    resolve(null);
                }
            }, 400);
        });
    ...
    // 检查HardwareKey认证用户
    async checkHardwareKeyAuthentication(username) {
        return new Promise((resolve) => {
            try {
                // 检查HardwareKey设备是否已连接并认证
                const hardwareKeyToken = localStorage.getItem('hardwareKeyAuthToken');
                const activexStatus = document.getElementById('activex-status');
                
                // 检查HardwareKey认证状态
                if (hardwareKeyToken && activexStatus && 
                    (activexStatus.textContent.includes('认证成功') || 
                     activexStatus.textContent.includes('HardwareKey认证'))) {
                    
                    console.log('HardwareKey认证检查通过:', username);
                    resolve({
                        username: username,
                        role: 'hardwareKey_user',
                        source: 'hardwareKey',
                        displayName: 'HardwareKey用户: ' + username,
                        hardwareKeyToken: hardwareKeyToken
                    });
                } else {
                    resolve(null);
                }
            } catch (error) {
                console.error(`[index-script.js] HardwareKey认证检查失败:, error`);
                resolve(null);
            }
        });
    }
    ...
    handleLoginSuccess(userInfo) {
        ...
        localStorage.setItem('login_source', userInfo.source);

        // 记录登录日志
        console.log('用户登录成功:', userInfo);

        // 显示成功消息
        alert(`欢迎回来，${userInfo.displayName}！登录源: ${this.getSourceDisplayName(userInfo.source)}`);

        // 重定向到适当的页面（实际项目中应根据角色重定向）
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  console.error(`[index-script.js] event.preventDefault failed:`, error);
}
  ...
  console.error(`[index-script.js] value.trim failed:`, error);
}
            ...
            try {
                ...
                console.error(`[index-script.js] 登录过程中发生错误:, error`);
    ...
    // 获取UI元素并添加空值检查
    const HardwareKeyStatusLabel = document.getElementById('HardwareKey-status');
    const HardwareKeyText = HardwareKeyStatusLabel ? HardwareKeyStatusLabel.querySelector('.HardwareKey-text') : null;
    const hiddenVkeyInput = document.getElementById('HardwareKey');
    
    // 开始定期检测HardwareKey设备
    function startHardwareKeyDetection() {
        // 立即执行一次检测
        detectHardwareKeyDevice();
        
        // 设置定期检测（每2秒）
        detectionInterval = setInterval(detectHardwareKeyDevice, 2000);
    };

    
    // 检测HardwareKey设备
    async function detectHardwareKeyDevice() {
        try {
            // 检查window.hardwareKeyAPI是否已加载
            if (!window.hardwareKeyAPI) {
                console.warn('HardwareKeyAPI尚未加载');
                return;
            };

            
            // 使用增强版HardwareKeyAPI查找设备
            const findResult = await window.hardwareKeyAPI.HardwareKeyFind();
            
            if (findResult.code === window.hardwareKeyAPI.HARDWAREKEY_SUCCESS && findResult.count > 0) {
                // 设备已检测到
                if (!lastDeviceState) {
                    lastDeviceState = true;
                    updateHardwareKeyStatus(true, findResult.devices[0]);
                    
                    // 生成模拟的HardwareKey认证码（实际应用中应从设备获取）
                    const mockHardwareKey = `VIKEY-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
                    hiddenVkeyInput.value = mockVkey;
                    
                    console.log('已检测到HardwareKey设备:', findResult.devices[0]);
                    
                    // 使用模拟的HardwareKey进行自动验证
                    try {
                        const verifyResult = await window.HardwareKeyAPI.HardwareKeyVerifyAuthCode(mockHardwareKey);
                        if (verifyResult.valid) {
                            console.log('HardwareKey设备验证成功，令牌:', verifyResult.token);
                            localStorage.setItem('hardwareKeyAuthToken', verifyResult.token);
                        };

                    } catch (verifyError) {
                        console.error(`[index-script.js] HardwareKey验证失败:, verifyError`);
                    };
                ...
                if (lastDeviceState) {
                    ...
                    updateHardwareKeyStatus(false);
                    hiddenVkeyInput.value = '';
                    localStorage.removeItem('hardwareKeyAuthToken');
                };

            };

        } catch (error) {
            console.error(`[index-script.js] HardwareKey设备检测失败:, error`);
            if (lastDeviceState) {
                lastDeviceState = false;
                updateHardwareKeyStatus(false);
                hiddenVkeyInput.value = '';
    ...
    // 更新HardwareKey状态显示
    function updateHardwareKeyStatus(isDetected, device = null) {
        if (isDetected && device) {
            HardwareKeyStatusLabel.classList.remove('status-error');
            HardwareKeyStatusLabel.classList.add('status-detected');
            
            // 根据设备类型显示不同信息
            let deviceTypeText = '加密狗';
            if (device.type === window.hardwareKeyAPI.HardwareKeyType.HardwareKeySecure) {
                deviceTypeText = '高安全性加密狗';
            } else if (device.type === window.hardwareKeyAPI.HardwareKeyType.HardwareKeyCloud) {
                deviceTypeText = '云验证型加密狗';
            } else if (device.type === window.hardwareKeyAPI.HardwareKeyType.HardwareKeyMobile) {
                deviceTypeText = '移动设备验证器';
            };

            
            HardwareKeyText.textContent = `已检测到HardwareKey ${deviceTypeText}（ID: ${device.id.substring(0, 4)}****）`;
        } else {
            HardwareKeyStatusLabel.classList.remove('status-detected');
            HardwareKeyStatusLabel.classList.add('status-error');
            HardwareKeyText.textContent = '未检测到HardwareKey设备';
        };
    ...
    function initiateOAuthLogin(provider) {
        ...
        if (oauthConfig[provider]) {
            ...
            console.error(`[index-script.js] 未配置${provider}的OAuth设置`);
...
function createParticles() {
  ...
  console.error(`[index-script.js] Math.random failed:`, error);
}
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  console.error(`[index-script.js] e.preventDefault failed:`, error);
}
    });
    
    // 禁用复制粘贴（可选）
    document.addEventListener('copy', function(e) {
        e.preventDefault().catch(error => console.error(`[index-script.js] e.preventDefault failed:`, error));
        return false;
    });
});
...
function setupDeleteButtons() {
  ...
  console.error(`[index-script.js] row.remove failed:`, error);
}
            };
...
function solarToLunar(date) {
  ...
  console.error(`[index-script.js] date.getFullYear failed:`, error);
}
    const month = date.getMonth();
    try {
  const day = date.getDate();
} catch (error) {
  console.error(`[index-script.js] date.getDate failed:`, error);
}
