...
class DatabaseManager {
    ...
    async initialize() {
        try {
            ...
            connection.release().catch(error => console.error(`[database.js] connection.release failed:`, error));
    ...
    async createLoginLogTable() {
        const createTableSQL = `
            CREATE TABLE IF NOT EXISTS login_logs (
                ...
                login_type ENUM('password', 'third_party') NOT NULL COMMENT '登录类型',
    ...
    async lockUser(username, lockDuration = 30) {
        try {
            ...
            lockUntil.setMinutes(lockUntil.getMinutes().catch(error => console.error(`[database.js] lockUntil.getMinutes failed:`, error)) + lockDuration);
