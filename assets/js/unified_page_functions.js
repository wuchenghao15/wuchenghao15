...
function isLoginPage() {
    // 修复：index.html不是登录页面，真正的登录页面应该是login.html
...
// 会话超时机制
let pageSessionTimeout; // 全局会话超时变量
...
function resetSessionTimeout() {
    clearTimeout(pageSessionTimeout);
    pageSessionTimeout = setTimeout(() => {
        // 会话超时，锁定系统而不是仅仅重定向
        lockSystem('会话超时异常');
...
function lockSystem(reason = '系统安全锁定') {
    // 检查页面是否刚加载，如果是则跳过锁定
    if (window.PAGE_JUST_LOADED === true) {
        console.log('[PageFunctions] 页面刚加载，跳过锁定操作，原因:', reason);
        return;
    }
...
function monitorDirectAccess() {
    ...
    if (!document.referrer && !isLoginPage()) {
        // 对index页面更宽容，不触发直接访问锁定
        if (window.location.pathname.includes('index.html')) {
            console.log('[DirectAccess] Index页面直接访问，跳过锁定检查');
            return;
        }
        ...
        console.log('[DirectAccess] 直接访问计数:', directAccessCount);
        
        if (directAccessCount >= MAX_DIRECT_ACCESS_ATTEMPTS) {
            console.log('[DirectAccess] 达到最大直接访问次数，锁定系统');
            lockSystem('多次尝试直接访问');
        }
    }
}
...
window.addEventListener('unload', function() {
    // 清除会话超时定时器
    if (pageSessionTimeout) {
        clearTimeout(pageSessionTimeout);
        pageSessionTimeout = null;
    }
