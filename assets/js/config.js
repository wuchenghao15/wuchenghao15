module.exports = {
    ...
    permissions: {
        ...
        rolePermissions: {
            user: ['view_rules'],
            password_admin: ['view_rules', 'change_password', 'approve_changes'],
            administrator: ['view_rules', 'edit_rules', 'manage_system', 'repair_system'],
            vikey_admin: ['view_rules', 'edit_rules', 'manage_users', 'change_password', 'manage_system', 'repair_system', 'approve_changes']
        },
        // 密码修改所需的管理员数量
        requiredAdminApprovals: 3,
        // 超级管理员角色
        superAdminRole: 'vikey_admin'
    },
    
    // 会话配置
    session: {
        timeout: 8 * 60 * 60 * 1000, // 8小时，单位毫秒
        warningThreshold: 5 * 60 * 1000, // 5分钟警告阈值
        cookieName: 'mtscos_session',
        localStorageKey: 'auth_session'
    },
    
    // 规则监测配置
    rulesMonitor: {
        checkInterval: 60000, // 1分钟检查一次规则状态
        maxRetries: 3,       // 自动修复最大重试次数
        repairTimeout: 30000, // 30秒修复超时
        alertThresholds: {
            critical: 1,     // 1个严重错误触发告警
            warning: 3,      // 3个警告触发告警
            info: 10         // 10个信息提示触发告警
        }
    },
    ...
    grayTest: {
        ...
        defaultGroup: 'internal_users',
        approvalRequired: true,
        approvalDeadline: 48 * 60 * 60 * 1000 // 48小时内必须完成审批
    },
    ...
    backup: {
        ...
        retentionPeriod: 30, // 保留30天的备份
        autoBackupTime: '00:00', // 每天凌晨0点自动备份
        backupPath: '/backups',
    ...
    ui: {
        theme: 'default',
        darkMode: false,
        notificationTimeout: 5000, // 通知显示5秒
        pageSize: 25,             // 分页大小
    ...
    autoRepair: {
        ...
        repairDelay: 5000, // 修复前等待5秒，允许用户取消
    ...
    rollback: {
        ...
        rollbackTimeout: 60000, // 60秒回滚超时
        minimumStabilityPeriod: 1800000 // 30分钟稳定期，期间不允许回滚
    }
};