...
async function demonstrateLogin() {
    try {
        ...
        // 保存验证码图片到文件
        const svgFileName = `captcha-${captchaId}.svg`;
        ...
        await new Promise(resolve => {
            ...
            process.stdin.resume().catch(error => console.error(`[demo-login.js] stdin.resume failed:`, error));
            process.stdin.on('data', () => {
                process.stdin.setRawMode(false);
                process.stdin.pause().catch(error => console.error(`[demo-login.js] stdin.pause failed:`, error));
                resolve();
            });
        });
        ...
        console.error(`[demo-login.js] ❌ 演示失败:, error.message`);
        if (error.response) {
            console.error(`[demo-login.js] 📄 响应数据:, error.response.data`);
            console.error(`[demo-login.js] 📊 状态码:, error.response.status`);
        }
        process.exit(1);
    }
}
