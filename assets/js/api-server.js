...
function handleSecurityReport(req, res) {
    const report = {
        ...
        lockedUsers: Array.from(userSecurityStates.values()).filter(state => state.isLocked).length,
...
function handleAuthStatus(req, res) {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        // 查找有效的会话
        for (const [sessionId, session] of sessions) {
            if (session.authenticated && Date.now() - session.authTime < 15 * 60 * 1000) {
                sendJSON(res, {
                    authenticated: true,
                    user: {
...
function handleHealth(req, res) {
    sendJSON(res, {
        ...
        status: 'healthy',
        timestamp: new Date().toISOString(),
        uptime: Date.now() - serverStatus.uptime,
        version: serverStatus.version
    });
}
