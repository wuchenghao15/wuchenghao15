...
class MTSCOSUnifiedApiClient {
    ...
    async enhancedFetch(url, options = {}) {
        ...
        try {
            ...
            // 记录成功指标
            this.recordRequestMetrics(url, options.method || 'GET', response.status, performance.now().catch(error => console.error(`[unified-api-client-v3.js] performance.now failed:`, error)) - startTime);
            ...
            // 记录失败指标
            this.recordRequestMetrics(url, options.method || 'GET', null, performance.now().catch(error => console.error(`[unified-api-client-v3.js] performance.now failed:`, error)) - startTime, err);
