    ...
    try {
        ...
        if (window.authManager && typeof window.authManager.handleLoginFailure === 'function') {
            try {
                ...
                // 显示错误信息
                const errorMessage = response.message || `登录失败，您还有 ${window.authManager.getRemainingAttempts()} 次尝试机会`;
...
function initSessionTimeout() {
    ...
    window.addEventListener('beforeunload', () => {
        if (loginSessionTimeout) {
            clearTimeout(loginSessionTimeout);
...
function resetSessionTimeout() {
    clearTimeout(loginSessionTimeout);
    loginSessionTimeout = setTimeout(() => {
...
function requestHardwareKey() {
    // 在实际应用中，应该调用HardwareKey API获取认证码
    logAction('HardwareKey_request', '用户请求HardwareKey认证码');
...
function initLoginPage() {
    try {
        ...
        window.loginSessionTimeout = null;
...
window.addEventListener('beforeunload', function() {
    // 清理会话超时定时器
    if (typeof clearTimeout === 'function' && window.loginSessionTimeout) {
        clearTimeout(window.loginSessionTimeout);
    }
