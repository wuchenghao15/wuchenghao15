...
REDUNDANT_PATTERNS = {
    ...
    'system': {
        'patterns': ['.DS_Store', '.Thumbs.db', '*.pid'],
        'delete': True
    'package_cache': {
        'description': '包缓存目录',
        'delete': True
...
def match_pattern(path, pattern):
    ...
    if '*' in pattern:
        ...
        if pattern.startswith('*'):
            ...
            return path.endswith(pattern[:-1]) or path.endswith(pattern[:-1] + '/')
...
def format_size(size_bytes):
    ...
    for item in redundant_items:
        ...
        if item['suggested_action'] != 'delete' and not force:
            ...
            logger.info(f"删除 {item['path']} ({item['redundant_type']})")
...
def generate_report(redundant_items, deletion_stats=None):
    ...
    for item in redundant_items:
        ...
        report['by_type'][item['redundant_type']]['count'] += 1
    ...
    try:
            ...
            if deletion_stats:
                ...
                f.write(f"  跳过: {deletion_stats['skipped']}\n")
            ...
            f.write("3. 可以使用本脚本的删除功能安全地清理这些项目\n")
...
def main():
    ...
    # 显示摘要
    print("\n扫描完成！")
    print(f"  - 文件: {report['redundant_summary']['files']}")
