...
SERVICES = [
    ...
    },
    {
        ...
        'command': ['python3', 'service_monitor.py'],
        'working_dir': os.path.join('Deployment', 'deploy_site'),
        'description': '服务监控服务'
    },
        'name': 'auto_backup_js',
        'command': ['python3', 'auto_backup_js_files.py', '--skip-existing'],
...
def start_service(service):
    ...
    try:
        ...
        # 在指定工作目录启动服务
        log_action(f"启动服务", f"{service['name']} 在 {cwd}")

        # 直接使用完整的命令，不做修改
        cmd = service['command']

        log_file = os.path.join(PROJECT_ROOT, 'Logs', f"{service['name']}_service.log")
...
def beautify_file_tree():
    ...
    for parent, child in directory_structure:
        if child:
...
def update_version():
    ...
    if os.path.exists(readme_path):
            ...
            for i, line in enumerate(lines):
                ...
                lines.insert(0, f"版本: {new_version}")
