...
def organize_files():
    ...
    for filename in os.listdir(PROJECT_ROOT):
        ...
        target_path = os.path.join(organize_dir, target_dir)
    ...
    try:
        ...
        logger.info(f"组织报告已生成: {report_file}")
    except Exception as e:
    ...
    readable_report = os.path.join(PROJECT_ROOT, f"organization_report_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt")
        with open(readable_report, 'w', encoding='utf-8') as f:
            ...
            f.write("=" * 60 + "\n\n")
            f.write("组织统计:\n")
            f.write(f"  成功移动: {report['organize_summary']['moved']}\n")
            f.write(f"  忽略: {report['organize_summary']['ignored']}\n")
...
def main():
    主函数
    logger.info("开始智能归类项目文件夹...")
    logger.info(f"项目根目录: {PROJECT_ROOT}")

    print("=" * 80)
    print("          项目文件智能归类工具          ")
    print("=" * 80)
    print("1. 本工具会生成文件组织的预览，不会直接修改原始文件")
    print("2. 所有操作都将在 ORGANIZED_TEMP 目录中进行")
    print("4. 操作完成后，请查看生成的组织报告")
    print("=" * 80)
