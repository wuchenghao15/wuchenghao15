...
class FolderOrganizer:
    ...
    def get_target_folder(self, file_path):
        ...
        # 特殊处理Scripts文件夹中的脚本文件 - 不要移动它们
        if self._is_in_scripts_folder(file_path):
    ...
    def _should_skip_file(self, file_path):
        ...
        if file_path.resolve() == Path(__file__).resolve():
            return True
        return True if self._is_in_scripts_folder(file_path) else False

    def _is_in_scripts_folder(self, file_path):
        """检查文件是否在Scripts文件夹中"""
        try:
            # 检查文件是否在Scripts文件夹内
            for parent in file_path.parents:
                if parent.name == 'Scripts':
                    # 对于Scripts文件夹中的脚本文件，不要移动
                    script_extensions = {'.sh', '.bat', '.ps1', '.bash', '.py'}
        except Exception:
            return False
    ...
    def generate_report(self):
        ...
        logger.info(f"- 未知文件类型: {self.stats['unknown_files']}")
    ...
    def run(self):
        ...
        # 创建日志目录
        log_dir = self.root_dir / 'Logs/文件夹整理'
        if not self.dry_run:
