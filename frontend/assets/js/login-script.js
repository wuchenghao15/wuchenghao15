...
function initCaptcha() {
    ...
    if (captchaInput) {
        captchaInput.addEventListener('input', function() {
            validateCaptchaInput(this.value);
        });
    }
}
...
function generateCaptcha() {
    ...
    const bgColor = getRandomColor(240, 255);
    ctx.fillStyle = bgColor;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    
    addNoise(ctx, canvas.width, canvas.height, 150);
    
    add干扰线(ctx, canvas.width, canvas.height, 8);
    ...
    for (let i = 0; i < CAPTCHA_CONFIG.length; i++) {
        ...
        ctx.save();
        ctx.translate(startX + i * 25 + offsetX, y + offsetY);
        ctx.rotate(angle);
        
        const fontFamily = CAPTCHA_CONFIG.fonts[Math.floor(Math.random() * CAPTCHA_CONFIG.fonts.length)];
    ...
    addFogEffect(ctx, canvas.width, canvas.height);
    
    sessionStorage.setItem('captcha', captcha);
    sessionStorage.setItem('captchaTime', Date.now().toString());
...
// 添加噪点
function addNoise(ctx, width, height, count) {
...
// 添加干扰线
function add干扰线(ctx, width, height, count) {
    for (let i = 0; i < count; i++) {
...
// 添加雾气效果
function addFogEffect(ctx, width, height) {
...
// 验证码输入验证
function validateCaptchaInput(value) {
    const storedCaptcha = sessionStorage.getItem('captcha');
    const captchaInput = document.getElementById('captcha');
    
    if (!captchaInput || !storedCaptcha) return;
    
    if (value.length === CAPTCHA_CONFIG.length) {
        if (value.toUpperCase() === storedCaptcha.toUpperCase()) {
            captchaInput.style.borderColor = '#52c41a';
            captchaInput.style.boxShadow = '0 0 0 2px rgba(82, 196, 26, 0.2)';
        } else {
            captchaInput.style.borderColor = '#ff4d4f';
            captchaInput.style.boxShadow = '0 0 0 2px rgba(255, 77, 79, 0.2)';
        }
    } else {
...
function startCaptchaCountdown() {
    if (CAPTCHA_CONFIG.countdownInterval) {
        clearInterval(CAPTCHA_CONFIG.countdownInterval);
    }
    
    CAPTCHA_CONFIG.expireTimestamp = Date.now() + CAPTCHA_CONFIG.expireTime;
    
    const countdownEl = document.getElementById('countdown');
    const timerEl = document.getElementById('captchaTimer');
    
    if (!countdownEl || !timerEl) return;
    
    timerEl.style.display = 'block';
    
    CAPTCHA_CONFIG.countdownInterval = setInterval(function() {
        const remaining = Math.max(0, Math.ceil((CAPTCHA_CONFIG.expireTimestamp - Date.now()) / 1000));
...
function isCaptchaExpired() {
    const captchaTime = sessionStorage.getItem('captchaTime');
    if (!captchaTime) return true;
    
    const elapsed = Date.now() - parseInt(captchaTime);
    return elapsed > CAPTCHA_CONFIG.expireTime;
}
...
function validateForm(username, password, captcha) {
    try {
        ...
        // 检查验证码长度
        if (captcha.length !== CAPTCHA_CONFIG.length) {
            showError(`验证码长度应为${CAPTCHA_CONFIG.length}位`);
            return false;
        }
