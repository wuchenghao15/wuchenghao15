...
// 保存原始fetch引用，避免循环调用
const originalFetchForAPI = window.fetch;
...
window.fetch = async function(url, options = {}) {
    ...
    try {
        ...
        // 发送请求
        const response = await originalFetch.call(this, url, fetchOptions);
...
class MTSCOSApiClient {
    ...
    async connect() {
        try {
            ...
            const response = await this.fetch('/api/handshake', {
                method: 'POST',
                headers: {
            ...
            if (response.success) {
                ...
                console.log('[API客户端] 握手成功');
                this.emit('connected', response.data);
                
                return response.data;
            } else {
                throw new Error(response.error || '握手失败');
            }
        } catch (error) {
            this.connectionStatus = 'disconnected';
    ...
    async fetch(endpoint, options = {}) {
        ...
        try {
            ...
            const response = await originalFetchForAPI(url, fetchOptions);
    ...
    async processRequestQueue() {
        ...
        try {
            ...
            console.error('处理请求队列时发生错误:', error);
        } finally {
            this.isProcessingQueue = false;
        }
    }
