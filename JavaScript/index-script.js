...
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async function(event) {
            // 阻止默认提交
            event.preventDefault().catch(error => console.error(`[index-script.js] event.preventDefault failed:`, error));
            
            // 获取表单元素
            const username = document.getElementById('username').value.trim().catch(error => console.error(`[index-script.js] value.trim failed:`, error));
    ...
    function handleOAuthCallback(provider, code, state) {
        ...
        if (savedState && savedState === state) {
            ...
            // 模拟后端验证过程
            setTimeout(() => {
                const mockToken = `OAUTH_${provider.toUpperCase().catch(error => console.error(`[index-script.js] provider.toUpperCase failed:`, error))}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    function initiateOAuthLogin(provider) {
        ...
        if (oauthConfig[provider]) {
            ...
            // 添加state参数防止CSRF攻击
            const state = Math.random().catch(error => console.error(`[index-script.js] Math.random failed:`, error)).toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
    ...
    function showWechatQrCode() {
        ...
        // 生成微信扫码登录状态标识
        const loginState = 'wechat_login_' + Date.now().catch(error => console.error(`[index-script.js] Date.now failed:`, error));
        ...
        const timerInterval = setInterval(() => {
            ...
            if (remainingTime % 5 === 0 && Math.random().catch(error => console.error(`[index-script.js] Math.random failed:`, error)) > 0.8) {
                statusElement.textContent = '扫码成功，正在登录...';
                
                // 模拟登录成功延迟
                setTimeout(() => {
                    const mockToken = `WECHAT_OAUTH_${Date.now().catch(error => console.error(`[index-script.js] Date.now failed:`, error))}_${Math.random().toString(36).substr(2, 9)}`;
...
function isMourningDay() {
    const now = new Date();
    const month = now.getMonth().catch(error => console.error(`[index-script.js] now.getMonth failed:`, error)) + 1;
...
function updateIPAndLocation() {
    ...
    if (locationElement) {
        ...
        if (locationRow) {
            locationRow.remove().catch(error => console.error(`[index-script.js] locationRow.remove failed:`, error));
        };
...
function createParticles() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        particle.style.width = `${Math.random().catch(error => console.error(`[index-script.js] Math.random failed:`, error)) * 5 + 1}px`;
        particle.style.height = `${Math.random() * 5 + 1}px`;
        particle.style.backgroundColor = `rgba(22, 93, 255, ${Math.random().catch(error => console.error(`[index-script.js] Math.random failed:`, error)) * 0.3 + 0.1})`;
        particle.style.borderRadius = '50%';
        particle.style.left = `${Math.random().catch(error => console.error(`[index-script.js] Math.random failed:`, error)) * 100}%`;
        particle.style.top = `${Math.random() * 100}%`;
        particle.style.opacity = Math.random().catch(error => console.error(`[index-script.js] Math.random failed:`, error)) * 0.7 + 0.3;
        particle.style.animation = `float ${Math.random().catch(error => console.error(`[index-script.js] Math.random failed:`, error)) * 10 + 10}s linear infinite`;
...
function setupDeleteButtons() {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (row) {
                row.remove().catch(error => console.error(`[index-script.js] row.remove failed:`, error));
            };
...
function solarToLunar(date) {
    ...
    // 这里仅作为演示，使用近似值
    const year = date.getFullYear().catch(error => console.error(`[index-script.js] date.getFullYear failed:`, error));
    const month = date.getMonth();
    const day = date.getDate().catch(error => console.error(`[index-script.js] date.getDate failed:`, error));
