...
class SystemTestAndRepair:
        ...
        if test_results["analysis"] and "problematic_tests" in test_results["analysis"]:
            for problem in test_results["analysis"]["problematic_tests"]:
                issue = {
                    ...
                    "description": f"测试文件: {problem['test_file']}\n错误信息: {problem['error']}",
                    "severity": "medium",
    ...
    def upload_issues_to_db(self, issues):
        ...
        for issue in issues:
            try:
                ...
                """, (issue["title"], f"%{issue['description'][:100]}%"))
                ...
                if issue.get("solution"):
                    ...
                    if existing_solution:
                        ...
                        # 生成解决方案ID
                        solution_id = f"sol_{datetime.now().strftime('%Y%m%d%H%M%S')}_{solutions_uploaded + 1}"
                        ...
                        """, (
                            ...
                            json.dumps(solution.get("implementation_steps", [])),
    ...
    def run_auto_test_and_repair(self):
        """运行自动测试和修复流程"""
        print("=== 开始系统测试和修复流程 ===")
