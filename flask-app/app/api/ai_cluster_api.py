...
@ai_cluster_api_bp.route('/tasks/batch', methods=['POST'])
...
@ai_cluster_api_bp.route('/tasks', methods=['GET'])
def get_all_tasks():
    """
    获取所有任务
    """
...
def get_healthy_nodes():
    ...
    try:
        ...
        logger.error(f"[AI集群API] 获取健康节点列表失败: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500
...
def get_unhealthy_nodes():
    ...
    try:
        ...
        logger.error(f"[AI集群API] 获取不健康节点列表失败: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500
