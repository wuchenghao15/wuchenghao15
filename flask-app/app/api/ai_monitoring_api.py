...
"""
系统监控API
提供系统状态监控、性能指标、AI员工状态、神经网络状态、集群状态等功能
"""
...
ai_monitoring_api = Blueprint('ai_monitoring_api', __name__)
    ...
    try:
        ...
        stats = {
            ...
            'maintenance': {
                ...
                'success_rate': round(successful_maintenance / max(successful_maintenance + failed_maintenance, 1) * 100, 2)
            }
        }
...
def get_employees_status():
    try:
        ...
        return jsonify({'success': True, 'data': employees, 'count': len(employees), 'timestamp': datetime.now().isoformat()})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_employee_detail(employee_id):
    try:
        with sqlite3.connect(DATABASE_PATH) as conn:
            ...
            cursor.execute("SELECT * FROM brain_learning_records WHERE employee_id = ? ORDER BY created_at DESC LIMIT 5", (str(employee_id),))
            learning_history = cursor.fetchall()
            
            cursor.execute("SELECT * FROM ai_upgrade_records WHERE employee_id = ? ORDER BY created_at DESC LIMIT 5", (employee_id,))
...
def get_cluster_status():
    try:
        ...
        return jsonify({'success': True, 'data': cluster_list, 'count': len(cluster_list), 'timestamp': datetime.now().isoformat()})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_learning_trends():
    try:
        with sqlite3.connect(DATABASE_PATH) as conn:
            ...
            cursor.execute('''
                SELECT DATE(recorded_at) as date, AVG(metric_value) as avg_value
                FROM performance_metrics
                WHERE metric_name = 'learning_progress'
    ...
    try:
        ...
        summary = {
            ...
            'system': {
                ...
                'status': 'healthy' if cpu_percent < 70 and memory.percent < 70 else 'warning' if cpu_percent < 90 and memory.percent < 90 else 'critical'
            },
            'ai_employees': {
                'active': active_employees
            },
