...
class VerificationUtils:
    ...
    def generate_unique_code(user_id, username, length=8):
        ...
        try:
            ...
            logger.info(f"生成唯一码: {unique_code}，用户: {username}，有效期: {expires_at}")
            return unique_code
        except Exception as e:
    ...
    def verify_whitelist_token(token, user_id=None, username=None):
        ...
        try:
            ...
            db_user_id, db_username, is_active, expires_at = token_data
            
            # 检查是否激活
            if not is_active:
                logger.warning(f"白名单 token 已失效: {token[:10]}...")
                return False, "白名单 token 已失效"
            ...
            logger.info(f"白名单 token 验证成功: {token[:10]}...，用户: {db_username}")
            return True, "验证成功"
        except Exception as e:
    ...
    def _verify_code(code, code_type, user_id=None, username=None):
        ...
        try:
            ...
            code_id, db_user_id, db_username, is_used, expires_at = code_data
            
            # 检查是否已使用
            if is_used:
                logger.warning(f"验证码已使用: {code}，类型: {code_type}")
                return False, "验证码已使用"
            
            # 检查是否过期
            if datetime.now() > datetime.strptime(expires_at, '%Y-%m-%d %H:%M:%S'):
                logger.warning(f"验证码已过期: {code}，类型: {code_type}")
                return False, "验证码已过期"
            
            # 标记为已使用
            db_manager.execute(
                'UPDATE verification_codes SET is_used = 1, used_at = CURRENT_TIMESTAMP WHERE id = ?',
                (code_id,)
            )
            
            logger.info(f"验证码验证成功: {code}，类型: {code_type}，用户: {db_username}")
            return True, "验证成功"
        except Exception as e:
    ...
    @staticmethod
    def verify_db_id(user_id, username):
        """验证数据库ID - 验证用户ID"""
        try:
            # 查询用户
            ...
            if user_data:
                logger.info(f"数据库ID验证成功: {user_id}，用户: {username}")
                return True, "验证成功"
            else:
                logger.warning(f"数据库ID验证失败: {user_id}，用户: {username}")
                return False, "用户ID不匹配"
        except Exception as e:
            logger.error(f"验证数据库ID失败: {str(e)}")
            return False, f"验证失败: {str(e)}"
    ...
    def verify_password(user_id, username, password):
        ...
        try:
            ...
            # 验证密码
            if security_utils.verify_password(user_data[0], password):
    ...
    def verify_vikey_hardware(hardware_id, is_admin=False):
        ...
        try:
            ...
            user_id, username, is_active, db_is_admin = hardware_data
            ...
            logger.info(f"Vikey硬件验证成功: {hardware_id}，用户: {username}")
            return True, "验证成功"
        except Exception as e:
            logger.error(f"验证Vikey硬件失败: {str(e)}")
            return False, f"验证失败: {str(e)}"
    
    @staticmethod
    def register_vikey_hardware(hardware_id, user_id, username, is_admin=False):
        """注册Vikey硬件"""
        try:
            # 检查是否已注册
            existing = db_manager.fetch_one(
                'SELECT id FROM vikey_hardware WHERE hardware_id = ?',
                (hardware_id,)
            )
            ...
            if existing:
                # 更新现有硬件信息
                db_manager.execute(
                    'UPDATE vikey_hardware SET user_id = ?, username = ?, is_admin = ?, updated_at = CURRENT_TIMESTAMP WHERE hardware_id = ?',
                    (user_id, username, 1 if is_admin else 0, hardware_id)
                )
                logger.info(f"更新Vikey硬件: {hardware_id}，用户: {username}")
            else:
                # 注册新硬件
                db_manager.execute(
                    'INSERT INTO vikey_hardware (hardware_id, user_id, username, is_admin) VALUES (?, ?, ?, ?)',
                    (hardware_id, user_id, username, 1 if is_admin else 0)
                )
                logger.info(f"注册Vikey硬件: {hardware_id}，用户: {username}")
            
            return True
        except Exception as e:
            logger.error(f"注册Vikey硬件失败: {str(e)}")
            return False
    ...
    def log_verification(username, verification_type, verification_value, is_successful, error_message=None, client_ip=None):
        ...
        try:
            ...
            # 保存日志
            db_manager.execute(
                'INSERT INTO login_verification_log (username, verification_type, verification_value, is_successful, error_message, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?, ?)',
                (username, verification_type, verification_value, 1 if is_successful else 0, error_message, ip_address, user_agent)
            )
            
            logger.info(f"记录验证日志: {username}，类型: {verification_type}，结果: {'成功' if is_successful else '失败'}，IP: {ip_address}")
        except Exception as e:
    ...
    def verify_all_factors(username, password, roll_code=None, unique_code=None, anti_fake_code=None, db_id=None, vikey_hardware_id=None, whitelist_token=None):
        ...
        try:
            ...
            login_attempts = db_manager.fetch_one(
                'SELECT COUNT(*) FROM login_verification_log WHERE username = ? AND ip_address = ? AND is_successful = 0 AND created_at > datetime("now", "-5 minutes")',
                (username, client_ip)
            )
            ...
            # 特殊情况：如果提供了白名单 token，可以直接验证，跳过其他验证
            if whitelist_token:
                # 如果提供了用户名，验证 token 与用户的关联
                if username:
                    # 获取用户 ID
                    user_data = db_manager.fetch_one(
                    ...
                    if user_data:
                        user_id = user_data[0]
                        token_valid, token_msg = VerificationUtils.verify_whitelist_token(whitelist_token, user_id, username)
                    else:
                        token_valid, token_msg = False, "用户不存在"
                else:
                    # 只验证 token 是否有效，不验证用户关联
                    token_valid, token_msg = VerificationUtils.verify_whitelist_token(whitelist_token)
                
                VerificationUtils.log_verification(username or 'anonymous', 'whitelist_token', whitelist_token[:10] + '...', token_valid, None if token_valid else token_msg)
                
                if token_valid:
                    logger.info(f"白名单 token 验证通过: {whitelist_token[:10]}...，IP: {client_ip}")
                    return True, "白名单 token 验证通过"
                else:
            ...
            # 1. 验证用户名和密码
            user_data = db_manager.fetch_one(
                'SELECT id, is_active FROM user WHERE username = ?',
                (username,)
            )
            
            if not user_data:
                VerificationUtils.log_verification(username, 'password', '', False, "用户不存在", client_ip)
                return False, "用户不存在"
            ...
            # 检查用户是否被禁用
            if not is_active:
                VerificationUtils.log_verification(username, 'password', '', False, "用户已被禁用", client_ip)
                return False, "用户已被禁用"
            
            # 2. 验证密码
            password_valid, password_msg = VerificationUtils.verify_password(user_id, username, password)
            VerificationUtils.log_verification(username, 'password', '', password_valid, None if password_valid else password_msg, client_ip)
            ...
            # 3. 验证数据库ID（如果提供）
            if db_id:
                db_id_valid, db_id_msg = VerificationUtils.verify_db_id(user_id, username)
                VerificationUtils.log_verification(username, 'db_id', db_id, db_id_valid, None if db_id_valid else db_id_msg, client_ip)
                
                if not db_id_valid:
                    return False, db_id_msg
            
            # 4. 验证滚码（如果提供）
            if roll_code:
                roll_code_valid, roll_code_msg = VerificationUtils.verify_roll_code(roll_code, user_id, username)
                VerificationUtils.log_verification(username, 'roll_code', roll_code, roll_code_valid, None if roll_code_valid else roll_code_msg, client_ip)
                
                if not roll_code_valid:
                    return False, roll_code_msg
            
            # 5. 验证唯一码（如果提供）
            if unique_code:
                unique_code_valid, unique_code_msg = VerificationUtils.verify_unique_code(unique_code, user_id, username)
                VerificationUtils.log_verification(username, 'unique_code', unique_code, unique_code_valid, None if unique_code_valid else unique_code_msg, client_ip)
                
                if not unique_code_valid:
                    return False, unique_code_msg
            
            # 6. 验证防伪码（如果提供）
            if anti_fake_code:
                anti_fake_code_valid, anti_fake_code_msg = VerificationUtils.verify_anti_fake_code(anti_fake_code, user_id, username)
                VerificationUtils.log_verification(username, 'anti_fake_code', anti_fake_code, anti_fake_code_valid, None if anti_fake_code_valid else anti_fake_code_msg, client_ip)
                
                if not anti_fake_code_valid:
                    return False, anti_fake_code_msg
            
            # 7. 验证Vikey硬件（如果提供）
            if vikey_hardware_id:
                vikey_valid, vikey_msg = VerificationUtils.verify_vikey_hardware(vikey_hardware_id)
                VerificationUtils.log_verification(username, 'vikey', vikey_hardware_id, vikey_valid, None if vikey_valid else vikey_msg, client_ip)
                
                if not vikey_valid:
                    return False, vikey_msg
            
            # 所有验证通过
            logger.info(f"所有验证因素通过: {username}，IP: {client_ip}")
            return True, "所有验证通过"
        except Exception as e:
            logger.error(f"验证所有因素失败: {str(e)}")
            client_ip = request.remote_addr if request else 'unknown'
            VerificationUtils.log_verification(username, 'all', '', False, str(e), client_ip)
            return False, f"验证失败: {str(e)}"
