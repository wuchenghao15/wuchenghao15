        ...
        try:
            ...
            if existing:
                ...
                cursor.execute('''
                    ...
                    WHERE id = ?
                ''', (json.dumps(data), current_time, existing[0]))
            else:
                # 插入新记录
                cursor.execute('''
                    INSERT INTO cache_records (cache_key, cache_type, content, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?)
                ''', (cache_key, 'general', json.dumps(data), current_time, current_time))
            ...
            logger.error(f"缓存同步到数据库失败 {cache_key}: {str(e)}")
            return False
            ...
            for root, dirs, files in os.walk(self.cache_dir):
                for file in files:
                    if file.endswith('.cache'):
                        # 从文件名获取cache_key
                        # 注意：这里需要改进，因为文件名是哈希值，无法直接获取cache_key
