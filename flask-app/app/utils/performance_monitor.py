...
class PerformanceMonitor:
    ...
    def get_process_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"获取进程性能数据时出错: {str(e)}")
            return {}
        ...
        try:
            ...
            if performance_data.get('disk_usage', 0) > 80:
                bottlenecks.append({
                    ...
                    'severity': 'medium',
                    'suggestion': '清理磁盘空间，考虑使用更高效的存储方案'
                })
            ...
            # 检查进程内存使用
            if performance_data.get('process_memory', 0) > 50:
                bottlenecks.append({
                    'type': 'process_memory',
    ...
    def get_performance_trend(self, window: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_avg = sum(item['memory_usage'] for item in recent_history) / window
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        """生成性能报告"""
        try:
            current_perf = self.get_system_performance()
    ...
    def _generate_recommendations(self, bottlenecks: List[Dict[str, Any]], trend: Dict[str, Any]) -> List[str]:
        ...
        if trend:
            ...
            if trend.get('memory_trend', 0) > 10:
                recommendations.append('内存使用率呈上升趋势，建议检查是否有内存泄漏')
