...
class ExamService:
    ...
    def save_exam_auto_fill(self, user_id, exam_id, question_id, field_name, field_value, context=None):
        ...
        try:
            ...
            if existing:
                ...
                """
                self.cursor.execute(sql, (user_id, exam_id, question_id, field_name, field_value, json.dumps(context) if context else None))
        ...
        try:
            ...
            for row in self.cursor.fetchall():
                performance = {
                    ...
                    "strengths": json.loads(row[7]) if row[7] else None,
                    "weaknesses": json.loads(row[8]) if row[8] else None,
                    "created_at": row[9]
                }
            ...
            print(f"获取考试性能数据失败: {str(e)}")
            return []
        finally:
            self.close()
    
    def set_exam_setting(self, user_id, setting_key, setting_value, category=None):
        """设置考试设置"""
        if not self.connect():
            return False
    ...
    def get_exam_behavior(self, user_id, exam_id, question_id=None, limit=100, offset=0):
        ...
        try:
            ...
            where_clause = " AND ".join(conditions)
            sql = f"""
            SELECT id, question_id, action_type, action_data, timestamp, time_spent, attempt_count, difficulty_perceived, confidence_level
            FROM exam_behavior
            WHERE {where_clause}
            ORDER BY timestamp
            LIMIT ? OFFSET ?
    ...
    def analyze_exam_performance(self, user_id, exam_id):
        """分析考试性能"""
        if not self.connect():
            return None
        ...
        try:
            ...
            # 分析数据
            score, time_spent, correct_answers, total_questions, difficulty_level, strengths, weaknesses = row
            ...
            if analysis["accuracy"] < 0.6:
                analysis["recommendations"].append("建议加强基础知识的学习")
            
            if time_spent and total_questions and (time_spent / total_questions) < 30:
    ...
    def analyze_answer_behavior(self, user_id, exam_id):
        ...
        try:
            ...
            for behavior in behaviors:
                action_type = behavior["action_type"]
                behavior_analysis["action_distribution"][action_type] = behavior_analysis["action_distribution"].get(action_type, 0) + 1
                
                # 统计修改次数
                if action_type == "modify_answer":
                ...
                if behavior["difficulty_perceived"]:
                    behavior_analysis["difficulty_perceptions"].append(behavior["difficulty_perceived"])
    ...
    def create_exam_with_ai(self, name, questions, education_version, time_limit):
        """使用AI创建考试
        ...
        try:
            ...
            print(f"使用AI创建考试失败: {str(e)}")
            return None
    
    def score_exam_with_ai(self, exam_id, answers, correct_answers):
        """使用AI评分考试
    ...
    def detect_cheating_with_ai(self, user_id, exam_id, exam_behavior):
        """使用AI检测作弊行为
    ...
    def check_scoring_rules(self, score):
        """检查评分规则
    ...
    def get_all_roles(self):
        ...
        """
        try:
            roles = exam_permission_manager.get_roles()
            return roles
        except Exception as e:
...
if __name__ == "__main__":
    ...
    print("保存自动填充数据...")
    result = service.save_exam_auto_fill(
    ...
    print(f"检测结果: {json.dumps(cheating_detection, indent=2, ensure_ascii=False)}")
