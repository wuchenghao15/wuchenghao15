...
class UserBehaviorServer:
    ...
    def _process_task(self, task):
        ...
        try:
            ...
            elif task_type == 'predict_user_behavior':
    ...
    def _handle_analyze_behavior(self, task_data):
        """处理用户行为分析任务"""
    ...
    def _handle_detect_anomalies(self, task_data):
        ...
        if analysis['success']:
            anomalies = analysis['data'].get('anomalies', [])
            return {
                'success': True,
                'message': '行为异常检测完成',
                'data': {
    ...
    def _handle_predict_behavior(self, task_data):
        """处理预测用户行为任务"""
        user_id = task_data.get('user_id')
        time_range = task_data.get('time_range', '7d')
...
def get_user_behavior_server():
    ...
    if _user_behavior_server is None:
        try:
            ...
            logger.error(f"用户行为子服务器初始化失败: {str(e)}")
...
# 分配用户行为分析任务
def assign_behavior_analysis_task(user_id, time_range="24h"):
    """
    分配用户行为分析任务
...
def assign_detect_anomalies_task(user_id, time_range="24h"):
    ...
    """
    task_info = {
        'task_type': 'detect_behavior_anomalies',
        'task_data': {
            'user_id': user_id,
            'time_range': time_range
        }
