...
class GuestUserManager:
                ...
                for exam_record in guest_data['exam_records']:
                    try:
                        ...
                        if not existing:
                            ...
                            logger.info(f"同步游客考试记录成功: 注册用户ID={registered_user_id}, 考试ID={exam_record['exam_id']}")
                ...
                for test_result in guest_data['language_test_results']:
                    try:
                        ...
                        if not existing:
                            # 插入语言测试结果
                            db_manager.execute(
                                'INSERT INTO language_test_results (user_id, test_type, level, score, completed_at) VALUES (?, ?, ?, ?, ?)',
                                (registered_user_id, test_result['test_type'], test_result['level'], test_result['score'], test_result['completed_at'])
                            )
                            logger.info(f"同步游客语言测试结果成功: 注册用户ID={registered_user_id}, 测试类型={test_result['test_type']}")
                    except Exception as test_error:
                        logger.error(f"同步游客语言测试结果失败: {str(test_error)}")
    ...
    def cleanup_guest_user(self, user_id):
        """清理游客用户数据"""
        try:
            # 从内存中删除游客信息
            if user_id in self.guest_users:
    ...
    def get_guest_permissions(self):
        ...
        return {
            ...
            'can_access_admin': False  # 不能访问管理功能
        }
