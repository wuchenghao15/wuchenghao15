...
class AdaptiveUpgradeService:
    ...
    def start_auto_upgrade(self, interval=3600):
        ...
        if not hasattr(self, '_auto_upgrade_thread'):
            self._auto_upgrade_thread = threading.Thread(target=self._auto_upgrade_loop, args=(interval,), daemon=True)
    ...
    def upgrade_ai_employee(self, employee_id):
        ...
        try:
            ...
            if not ai_employee:
                logger.warning(f"⚠️  未找到AI员工: {employee_id}")
                return False
    ...
    def _sync_with_ai_brain(self, ai_employee, upgrade_info):
        ...
        try:
            ...
            knowledge_content = json.dumps(upgrade_info, ensure_ascii=False, indent=2)
...
ai_brain_service.add_knowledge(
            ...
            logger.error(f"❌ 与AI脑库同步升级信息失败: {str(e)}")
