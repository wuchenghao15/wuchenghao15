...
class UserAnswerAnalysisService:
    ...
    def analyze_answer_patterns(self, user_id, days=30):
        ...
        try:
            ...
            if declining_types:
                analysis['recommendations'].append(f'您在 {declining_types[0]["question_type"]} 题型上有下降趋势，建议加强练习')
    ...
    def generate_personalized_study_plan(self, user_id):
        ...
        try:
            ...
            # 添加薄弱环节
            for weak_type in weaknesses_analysis['weak_question_types']:
                if weak_type['accuracy'] < 0.6:
                    study_plan['weak_areas'].append({
                        'area': weak_type['question_type'],
                        'accuracy': weak_type['accuracy'],
                        'recommendation': f'每天练习 {weak_type["question_type"]} 题型 10-15 题'
                    })
            
            # 添加推荐练习
            if weaknesses_analysis['overall_accuracy'] < 0.7:
                study_plan['recommended_practice'].append('增加基础题目的练习量，确保基础知识掌握')
            
            if patterns_analysis and patterns_analysis['best_time_slot']:
...
def get_user_answer_analysis_service():
    """获取用户答题分析服务实例"""
...
if __name__ == "__main__":
    ...
    if weaknesses:
        print(f"薄弱环节分析: {json.dumps(weaknesses, indent=2, ensure_ascii=False)}")
    
    # 测试分析答题模式
    print("\n分析答题模式...")
    patterns = service.analyze_answer_patterns(user_id)
    if patterns:
        print(f"答题模式分析: {json.dumps(patterns, indent=2, ensure_ascii=False)}")
    
    # 测试生成个性化学习计划
    print("\n生成个性化学习计划...")
    study_plan = service.generate_personalized_study_plan(user_id)
    if study_plan:
        print(f"学习计划: {json.dumps(study_plan, indent=2, ensure_ascii=False)}")
