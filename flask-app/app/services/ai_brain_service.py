...
class AIBrainService:
    ...
    def update_knowledge(self, knowledge_id, title=None, content=None, knowledge_type=None, 
        ...
        """更新AI脑库知识"""
        try:
            knowledge = AIBrainKnowledge.get_by_id(knowledge_id)
            if not knowledge:
            ...
            self._log_activity(
                ...
                description=f"更新知识: {knowledge_id}",
                source="system",
                metadata={
    ...
    def get_all_knowledge(self, knowledge_type=None, source=None, tags=None):
        """获取所有知识"""
        try:
            return AIBrainKnowledge.get_all(knowledge_type=knowledge_type, source=source, tags=tags)
        except Exception as e:
    ...
    def search_knowledge(self, keyword, knowledge_type=None):
        """搜索知识"""
        try:
            return AIBrainKnowledge.search(keyword, knowledge_type=knowledge_type)
        except Exception as e:
    ...
    def add_solution(self, title, content, source, source_id=None, tags=None, related_problem_id=None):
        """添加解决方案到AI脑库"""
        if related_problem_id:
            # 添加关联标签
            if not tags:
                tags = []
            tags.append(f"related-to:{related_problem_id}")
    ...
    def add_rule(self, title, content, source, source_id=None, tags=None):
        """添加规则到AI脑库"""
        return self.add_knowledge(
            title=title,
            content=content,
    ...
    def get_recent_activities(self, limit=50):
        """获取最近的活动"""
        try:
            return AIBrainActivity.get_recent(limit=limit)
        except Exception as e:
            logger.error(f"✗ 获取AI脑库活动失败: {str(e)}")
            return []
    
    def _log_activity(self, activity_type, description, source, source_id=None, metadata=None):
        """记录活动日志"""
        try:
    ...
    def batch_enhance_knowledge(self):
        """批量增强所有知识"""
        try:
            all_knowledge = self.get_all_knowledge()
            enhanced_count = 0
    ...
    def enhance_knowledge_with_ai(self, knowledge_id, engine_type="qianwen"):
        """使用外部AI引擎增强知识"""
        try:
    ...
    def sync_ai_instance_knowledge(self, ai_instance):
        ...
        try:
            # 从AI实例获取知识
            if isinstance(ai_instance, dict):
                # 从字典格式的AI实例获取知识
                knowledge_items = []
                ...
                if 'functions' in ai_instance:
                    for func in ai_instance['functions']:
                        knowledge_items.append({
                            'title': f"功能: {func}",
    ...
    def sync_all_instances_knowledge(self, ai_instances):
        ...
        try:
            synced_count = 0
            for ai_instance in ai_instances:
    ...
    def _calculate_confidence_score(self, knowledge):
        ...
        try:
            ...
            # 基于标签数量调整评分
            if hasattr(knowledge, 'tags') and knowledge.tags:
    ...
    def _detect_conflicts(self, knowledge):
        ...
        try:
            ...
            for other_knowledge in all_knowledge:
                ...
                other_content = other_knowledge.content.lower()
                other_title = other_knowledge.title.lower()
                
                # 检查直接矛盾
                if any(term in content for term in ["1+1=2", "1 + 1 = 2"]) and any(term in other_content for term in ["1+1=8", "1 + 1 = 8"]):
    ...
    def auto_acquire_knowledge(self, topics, sources=None, limit=5):
        ...
        try:
            ...
            logger.error(f"✗ 自动获取知识失败: {str(e)}")
            return 0
    
    def self_learn_from_interactions(self, interactions):
        """从AI交互中自我学习"""
        try:
            learned_count = 0
            ...
            logger.info(f"✓ 从交互中自我学习完成，共学习 {learned_count} 条知识")
