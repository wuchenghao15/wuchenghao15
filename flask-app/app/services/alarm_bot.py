...
class AlarmBot:
    ...
    def _check_security_status(self):
        ...
        try:
            # 检查攻击尝试
            attack_attempts = len(security_defense.attack_attempts)
                self._trigger_alarm(
                    '攻击尝试次数过多',
                    'high'

            # 检查蜜罐触发
            if honeypot_triggers > self.config['alarm_rules']['security']['honeypot_triggers_threshold']:
                self._trigger_alarm(
                    '蜜罐触发次数过多',
                    'high'
                )
            cheese_status = security_defense.check_cheese_model()
            vulnerable_layers = len([layer for layer in cheese_status['layers'] if not layer['active']])
    ...
    def _check_redis_status(self):
        ...
        with self.lock:
            ...
            # 保存到Redis
            alarm_key = f"alarm:{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            redis_manager.set(alarm_key, alarm, expire=86400)
