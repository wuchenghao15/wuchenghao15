...
class SystemVersionService:
    ...
    def _save_versions_to_file(self):
        ...
        try:
            with open(self._version_file, 'w', encoding='utf-8') as f:
                json.dump(self._current_versions, f, ensure_ascii=False, indent=2)
    ...
    def check_compatibility(self, required_version):
        ...
        # 确保两个版本号的部分数量一致
        max_length = max(len(current_parts), len(required_parts))
    ...
    def save_version_snapshot(self, reason=""):
        """保存当前版本快照"""
        snapshot = {
            'timestamp': time.time(),
            'datetime': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
