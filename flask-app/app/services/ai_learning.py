...
class AILearningSystem:
    ...
    def _load_upgrade_history(self):
        ...
        for file_info in upgrade_files:
            if file_info["type"] == "file" and file_info["filename"].endswith(".json"):
    ...
    def learn_from_experience(self, experience_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            ai_enhanced_result = self.enhance_with_ai(
                task_type="learning",
                content=json.dumps(experience_data, ensure_ascii=False, indent=2),
                temperature=0.7,
                max_tokens=2048
            )
            ...
            # 更新学习状态
            self._learning_state["last_learning_time"] = time.time()
            self._learning_state["knowledge_count"] = len(self._knowledge_graph["entities"]) + len(self._knowledge_graph["relations"]) + len(self._knowledge_graph["rules"]) + len(self._knowledge_graph["tasks"])
    ...
    def self_upgrade(self) -> bool:
        ...
        try:
            ...
            if upgrade_result:
                # 记录升级历史
                self._record_upgrade_history(upgrade_plan, upgrade_result)
                ...
                self._ai_brain_service._log_activity(
                    ...
                    description=f"AI自我升级成功: {upgrade_plan['version']} -> {upgrade_plan['target_version']}",
                    source="ai_learning_system",
                    source_id=upgrade_plan["id"],
                    metadata={
                        "upgrade_plan": upgrade_plan,
                ...
                self._ai_brain_service._log_activity(
                    ...
                    description=f"AI自我升级失败: {upgrade_plan['version']} -> {upgrade_plan['target_version']}",
                    source="ai_learning_system",
                    source_id=upgrade_plan["id"],
                    metadata={
                        "upgrade_plan": upgrade_plan,
            ...
            # 更新升级检查时间
            self._learning_state["last_upgrade_check"] = time.time()
    ...
    def _check_upgrade_conditions(self) -> bool:
        ...
        if self._learning_state["upgrade_count"] > 0:
            # 计算平均升级成功率
            success_count = sum(1 for upgrade in self._upgrade_history if upgrade["result"])
            success_rate = success_count / len(self._upgrade_history) if self._upgrade_history else 0
            if success_rate < 0.5:
    ...
    def monitor_trae_entry(self, entry_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 记录监控活动
            self._ai_brain_service._log_activity(
                activity_type="trae_entry_monitored",
            ...
            # 触发潜在的升级需求分析
            self._check_upgrade_conditions()
    ...
    def _analyze_upgrade_needs(self) -> List[Dict[str, Any]]:
        ...
        # 5. trae词条相关升级需求（新增）
        trae_entries = self._ai_brain_service.search_knowledge(
            keyword="trae_entry",
            knowledge_type="trae_entry"
        )
        ...
        ai_enhanced_result = self.enhance_with_ai(
            ...
            content=json.dumps({
                "knowledge_graph": {
                    ...
                    "tasks_count": len(self._knowledge_graph["tasks"])
                },
                "learning_state": self._learning_state,
                "capability_expansions": list(self._capability_expansions.keys()),
                "feature_correlation_matrix": len(self._feature_correlation_matrix),
        ...
        if ai_enhanced_result and ai_enhanced_result["success"]:
            ai_generated_needs = {
                ...
                "ai_enhanced_content": ai_enhanced_result["enhanced_content"],
        ...
        if need["type"] == "knowledge_graph_optimization":
            ...
            actions.extend([
                ...
                },
                {
                    ...
                    "priority": need["priority"],
                    "description": "规则优先级排序",
                    "method": "usage_based_prioritization",
                    "params": {
    ...
    def _execute_upgrade(self, upgrade_plan: Dict[str, Any]) -> bool:
        ...
        Returns:
            bool: 是否执行成功
        """
        logger.info(f"执行升级方案: {upgrade_plan['id']}")
        ...
        for action in upgrade_plan["actions"]:
            ...
            if action["type"] == "entity_clustering":
                ...
                logger.warning(f"未知的升级动作类型: {action['type']}")
        ...
        # 计算升级成功率
        success_rate = success_count / len(upgrade_plan["actions"]) if upgrade_plan["actions"] else 0
        ...
        return success_rate > 0.5  # 成功率超过50%视为升级成功
    ...
    def _cluster_entities(self, action: Dict[str, Any]) -> bool:
        ...
        for entity_id, entity in self._knowledge_graph["entities"].items():
            # 提高实体置信度
            entity["confidence"] = min(0.99, entity["confidence"] + self._learning_params["learning_rate"] * 0.5)
        return True
    ...
    def _mine_relations(self, action: Dict[str, Any]) -> bool:
        ...
        for relation_id, relation in self._knowledge_graph["relations"].items():
            # 提高关系置信度
            relation["confidence"] = min(0.99, relation["confidence"] + self._learning_params["learning_rate"] * 0.5)
        return True
    ...
    def _update_feature_correlations(self, action: Dict[str, Any]) -> bool:
        ...
        if hasattr(self, '_feature_names') and self._feature_names:
            ...
            descriptions = [self._feature_descriptions[name] for name in feature_names]
    ...
    def _prioritize_rules(self, action: Dict[str, Any]) -> bool:
        ...
        for rule_id, rule in self._knowledge_graph["rules"].items():
            ...
            # 计算优先级得分
            priority_score = (confidence * 0.7) + (min(usage / min_usage, 1.0) * 0.3)
    ...
    def _enhance_knowledge_from_trae(self, action: Dict[str, Any]) -> bool:
        ...
        for keyword in keywords:
            ...
            self._knowledge_graph["tasks"][task_type][task_id] = {
                "id": task_id,
                "task": f"{task_type}:trae_enhanced",
                "result": "pending",
                "feedback": 0,
                "context": {
                ...
                "learned_at": time.time(),
                "confidence": 0.6,
                "description": f"基于trae关键词 {keyword} 生成的增强任务"
            }
        ...
        try:
            ...
            if feature_associations:
                ...
                adaptation_plan["actions"].append({
                    ...
                    "description": "更新功能关联",
                    "associations_count": len(feature_associations),
                    "status": "success"
                })
            ...
            if knowledge_extension:
                ...
                adaptation_plan["actions"].append({
                    ...
                    "rules_added": len(knowledge_extension.get("rules", {})),
                    "status": "success"
                })
            ...
            if ai_enhanced_result and ai_enhanced_result["success"]:
                adaptation_plan["ai_enhanced_content"] = ai_enhanced_result["enhanced_content"]
            ...
            # 6. 保存适配方案
            adaptation_file = os.path.join(self._project_adaptation_dir, f"adaptation_{adaptation_plan['id']}.json")
    ...
    def _generate_project_knowledge_extension(self, project_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if project_type == "web_application":
            ...
            knowledge_extension["relations"]["uses_framework"] = {
            ...
            knowledge_extension["rules"]["web_performance"] = {
                "condition": "project_type == 'web_application' and goal == 'performance'",
    ...
    def generate_feature_associations(self, current_features: List[str]) -> List[Dict[str, Any]]:
        ...
        if not hasattr(self, '_feature_names') or not self._feature_names:
            ...
            try:
                ...
                all_descriptions = [self._feature_descriptions[name] for name in all_features]
                    ...
                    if feature_name not in current_features:
                        ...
                        if similarity > self._learning_params["feature_similarity_threshold"]:
                            associations.append({
                                ...
                                "reason": f"与当前功能相似度高: {similarity:.2f}",
