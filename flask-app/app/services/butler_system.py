...
class ButlerSystem:
    ...
    def __init__(self):
        ...
        self._task_queue = []  # 优先队列，格式: (priority, task_id)
        self._task_threads = []
        ...
        self._roles = {
            ...
            "developer": {
                "name": "开发者",
                "permissions": ["task_management", "project_management", "ai_engine_access"]
            },
            "analyst": {
                "name": "分析师",
    ...
    def _execute_task(self, task: Dict[str, Any]):
        ...
        try:
            ...
            with self._lock:
                ...
                self._status["task_stats"]["completed"] += 1
            
            logger.info(f"任务执行完成: {task_id} - {task_type} (执行时间: {task['duration']:.2f}秒)")
        except Exception as e:
            ...
            with self._lock:
                ...
                if task.get("start_time"):
                    task["duration"] = task["end_time"] - task["start_time"]
                task["error"] = str(e)
                self._status["task_stats"]["failed"] += 1
    ...
    def get_subsystem_status(self, subsystem_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        with self._lock:
            return self._status["subsystems"].get(subsystem_name)
        ...
        try:
            ...
            risks = [
                ...
                },
                {
                    ...
                    "probability": 0.4,
                    "impact": "高",
                    "mitigation": "提前采购资源，寻找替代方案"
                },
                {
                    "id": f"risk_{int(time.time())}_3",
                    "type": "技术风险",
    ...
    def optimize_resource_allocation(self, project_id: str, resources: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"开始资源优化分配，项目: {project_id}")
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._lock:
                self._status["system_health"] = health_data
    ...
    def get_smart_recommendations(self, project_id: str = None, task_id: str = None) -> Dict[str, Any]:
        """
        获取智能建议
        ...
        try:
            ...
            logger.info(f"智能建议生成完成，共 {len(recommendations)} 条建议")
            self._notify_event("smart_recommendations_generated", result)
        ...
        return [
            {
                ...
                "category": "进度管理",
                "title": "优化项目进度计划",
                "description": "建议重新评估项目进度，调整关键路径",
                "priority": "medium",
                "action": "调整项目计划",
                "estimated_impact": "高"
            },
            {
                "id": f"proj_rec_{int(time.time())}_2",
                "type": "project",
                "category": "资源管理",
                "title": "优化资源分配",
            ...
            {
                "id": f"proj_rec_{int(time.time())}_3",
                "type": "project",
                "category": "风险管理",
                "title": "加强风险监控",
    ...
    def _generate_task_recommendations(self, task_id: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "id": f"task_rec_{int(time.time())}_2",
                "type": "task",
                "category": "时间管理",
                "title": "设置合理的时间估计",
    ...
    def _generate_system_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if health.get("status") == "warning":
            system_recommendations.append({
                "id": f"sys_rec_{int(time.time())}_1",
                "type": "system",
                "category": "系统健康",
                "title": "系统负载过高",
        ...
        system_recommendations.extend([
            ...
            {
                "id": f"sys_rec_{int(time.time())}_3",
                "type": "system",
                "category": "安全管理",
                "title": "加强安全监控",
    ...
    def update_user_roles(self, user_id: str, roles: List[str]) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                logger.info(f"用户角色更新成功: {user_id}")
                return {
                    "status": "success",
                    "message": "用户角色更新成功",
                    "user_id": user_id,
    ...
    def _log_audit_event(self, event_type: str, event_data: Dict[str, Any]):
        ...
        with self._lock:
            ...
            if len(self._status["security"]["audit_logs"]) > 1000:
                self._status["security"]["audit_logs"] = self._status["security"]["audit_logs"][-1000:]
        
        logger.info(f"审计事件记录: {event_type}")
        ...
        def monitor_memory():
            while not self._task_stop_event.is_set():
                try:
                    # 模拟内存使用监控
                    import random
                    current_memory = random.uniform(20, 90)
                    
                    with self._lock:
                        self._status["memory_usage"]["current"] = current_memory
                        if current_memory > self._status["memory_usage"]["peak"]:
                            self._status["memory_usage"]["peak"] = current_memory
                        self._status["memory_usage"]["last_checked"] = time.time()
    ...
    def _cleanup_memory(self):
        ...
        with self._lock:
            ...
            if self._cache_size > 0:
                cache_size_before = self._cache_size
                self._cache.clear()
                self._cache_size = 0
                logger.info(f"清理缓存，释放 {cache_size_before} 个缓存项")
            
            # 清理已完成的任务
            completed_tasks = []
            for task_id, task in self._status["tasks"].items():
                if task.get("status") in ["completed", "failed"] and time.time() - task.get("end_time", 0) > 86400:  # 超过24小时
            ...
            if len(self._status["security"]["audit_logs"]) > 500:
                ...
                self._status["security"]["audit_logs"] = self._status["security"]["audit_logs"][-500:]
    ...
    def get_memory_status(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                memory_usage = self._status.get("memory_usage", {})
                cache_size = self._cache_size
                tasks_count = len(self._status.get("tasks", {}))
                events_count = len(self._status.get("events", []))
                audit_logs_count = len(self._status.get("security", {}).get("audit_logs", []))
    ...
    def clear_cache(self):
        ...
        with self._lock:
            self._cache.clear()
            self._cache_size = 0
            logger.info(self.translate("memory_cleanup_completed"))
