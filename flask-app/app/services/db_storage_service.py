...
class DatabaseStorageService:
    ...
    def _create_tables(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS complex_data (
                ...
                object_type TEXT NOT NULL,  # dict or list
                element_key TEXT,  # 字典的键或列表的索引
                element_type TEXT NOT NULL,  # string, int, float, bool, nested
                string_value TEXT,
                int_value INTEGER,
                float_value REAL,
                bool_value INTEGER,
                parent_id INTEGER,  # 嵌套对象的父ID
                index_pos INTEGER,  # 列表中的位置
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')
    ...
    def set(self, key, value):
        ...
        try:
            ...
            if isinstance(value, str):
                cursor.execute('''
                    INSERT INTO serialized_data (data_key, data_type, string_value)
                    VALUES (?, ?, ?)
                ''', (key, 'string', value))
            
            elif isinstance(value, int):
                cursor.execute('''
                    INSERT INTO serialized_data (data_key, data_type, int_value)
                    VALUES (?, ?, ?)
                ''', (key, 'int', value))
            
            elif isinstance(value, float):
                cursor.execute('''
                    INSERT INTO serialized_data (data_key, data_type, float_value)
                    VALUES (?, ?, ?)
                ''', (key, 'float', value))
            
            elif isinstance(value, bool):
                cursor.execute('''
                    INSERT INTO serialized_data (data_key, data_type, bool_value)
                    VALUES (?, ?, ?)
            ...
            logger.error(f"存储数据失败 {key}: {str(e)}")
            conn.rollback()
            return False
        finally:
            conn.close()
    ...
    def get(self, key, default=None):
        ...
        try:
            ...
            # 检查复杂数据
            cursor.execute('SELECT object_type FROM complex_data WHERE data_key = ? LIMIT 1', (key,))
            row = cursor.fetchone()
            
            if row:
                return self._load_complex_data(key, row[0])

            return default

        except Exception as e:
            logger.error(f"获取数据失败 {key}: {str(e)}")
            return default
        finally:
            conn.close()
    ...
    def exists(self, key):
        ...
        try:
            ...
            logger.error(f"检查键存在失败 {key}: {str(e)}")
            return False
        finally:
            conn.close()
    ...
    def _store_element(self, cursor, key, object_type, element_key, value, parent_id, index_pos=None):
        ...
        if isinstance(value, str):
            cursor.execute('''
                INSERT INTO complex_data (data_key, object_type, element_key, element_type, string_value, parent_id, index_pos)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (key, object_type, element_key, 'string', value, parent_id, index_pos))
        
        elif isinstance(value, int):
            cursor.execute('''
                INSERT INTO complex_data (data_key, object_type, element_key, element_type, int_value, parent_id, index_pos)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (key, object_type, element_key, 'int', value, parent_id, index_pos))
        
        elif isinstance(value, float):
            cursor.execute('''
                INSERT INTO complex_data (data_key, object_type, element_key, element_type, float_value, parent_id, index_pos)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (key, object_type, element_key, 'float', value, parent_id, index_pos))
        
        elif isinstance(value, bool):
            cursor.execute('''
                INSERT INTO complex_data (data_key, object_type, element_key, element_type, bool_value, parent_id, index_pos)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            # 存储嵌套字典
            cursor.execute('''
                INSERT INTO complex_data (data_key, object_type, element_key, element_type, parent_id, index_pos)
                VALUES (?, ?, ?, ?, ?, ?)
    ...
    def _load_complex_data(self, key, object_type):
        ...
        try:
            ...
            if object_type == 'dict':
                ...
                result = []
                # 获取最大索引
                cursor.execute('SELECT MAX(index_pos) FROM complex_data WHERE data_key = ? AND parent_id IS NULL', (key,))
                ...
                if max_index is not None:
                    ...
                    for row in rows:
                        ...
                        if index_pos is not None:
                            value = self._load_element(cursor, row, key)
                            result[index_pos] = value
                
                # 过滤None值（如果列表有缺失索引）
                return [item for item in result if item is not None]

        except Exception as e:
            logger.error(f"加载复杂数据失败 {key}: {str(e)}")
            return {} if object_type == 'dict' else []
        finally:
            conn.close()
