...
class RuleManagementService:
    ...
    def init_rule_manager_ai(self):
        ...
        try:
            ...
            logger.error(f"初始化规则管理AI失败: {str(e)}")
            self.rule_manager_ai = None
    
    def update_rule_manager_ai(self):
        """更新规则管理AI配置"""
        try:
            # 跳过AI实例更新，避免循环依赖
    ...
    def _collect_business_rules(self):
        ...
        for source in self.rule_sources["business_rules"]:
            if os.path.exists(source) and source.endswith(".json"):
                try:
                    with open(source, "r", encoding="utf-8") as f:
                        rules = json.load(f)
        ...
        self.rules["business_rules"] = business_rules
    ...
    def _collect_test_rules(self):
        ...
        test_rules = {
            "test_generator": {
                ...
                "test_generation_limit": "不能生成高于用户等级的试卷"
            }
        }
        ...
        ai_management_rules = {
            ...
            "ai_constraint_rules": {
                "instance_type_constraints": {
                    ...
                    "rule_management": {"max_instances": 5, "allowed_functions": ["rule_management", "rule_execution", "rule_optimization"]},
                ...
                "function_constraints": {
                    ...
                    "system_monitoring": {"requires_role": "admin"},
    ...
    def add_rule_source(self, rule_type, source_path):
        ...
        if rule_type in self.rule_sources:
            if source_path not in self.rule_sources[rule_type]:
    ...
    def delete_rule(self, rule_type, rule_name):
        ...
        with self.rule_lock:
            if rule_type in self.rules and rule_name in self.rules[rule_type]:
                ...
                logger.info(f"已删除规则: {rule_name} 从 {rule_type}")
                # 更新规则管理AI
                self.update_rule_manager_ai()
                return True
        return False
    ...
    def set_update_interval(self, interval):
        ...
        if self.rule_manager_ai:
            self.update_rule_manager_ai()
        logger.info(f"规则更新间隔已设置为: {interval}秒")
