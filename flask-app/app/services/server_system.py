...
class ServerSystem:
    ...
    def _perform_health_check(self):
        ...
        with self._lock:
            ...
            self._notify_event("health_check_completed", {
                ...
                "unhealthy_servers": len([s for s in self._servers.values() if s["status"] == "unhealthy"]),
    ...
    def _register_service_for_server(self, service_name: str, server_id: str):
        ...
        if len(self._services[service_name]["servers"]) >= self._config["max_servers_per_service"]:
            logger.warning(f"服务 {service_name} 已达到最大服务器数量: {self._config['max_servers_per_service']}")
            return
        ...
        with self._lock:
            ...
            for service_name in server_info["services"]:
                if service_name in self._services:
                    ...
                    # 如果服务没有服务器了，移除该服务
                    if not self._services[service_name]["servers"]:
    ...
    def get_service(self, service_name: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            if service_info:
                ...
                service_info_with_servers["servers"] = [self._servers[server_id] for server_id in service_info["servers"] if server_id in self._servers]
        ...
        with self._lock:
            ...
            if selected_strategy == "random":
                ...
                if service_name not in self._round_robin_counter:
                    self._round_robin_counter[service_name] = 0
    ...
    def update_server(self, server_id: str, updates: Dict[str, Any], auth_key: Optional[str] = None) -> bool:
        ...
        with self._lock:
            ...
            server_info["last_updated"] = time.time()
    ...
    def decrease_connections(self, server_id: str, auth_key: Optional[str] = None) -> bool:
        """
        减少服务器的连接数
    ...
    def shutdown(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                self._status["service_count"] = 0
                
                logger.info("服务器系统关闭成功")
                return True
            except Exception as e:
                logger.error(f"服务器系统关闭失败: {str(e)}")
                return False
