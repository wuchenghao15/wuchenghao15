...
class LoginContainer:
    ...
    def _update_login_attempts(self, username: str, ip_address: str, success: bool):
        ...
        if not success:
            self.ip_login_attempts[ip_address]["attempts"] += 1
            self.ip_login_attempts[ip_address]["usernames"].add(username)
            self.ip_login_attempts[ip_address]["timestamp"] = current_time
    ...
    def process_login(self, username: str, password: str, ip_address: str = "127.0.0.1", request=None) -> Dict[str, Any]:
        """处理用户登录请求"""
        try:
            ...
            # 使用登录AI处理登录请求
            login_result = self.login_ai.login_user(username, password, ip_address, request)
            ...
            if login_result["success"]:
                ...
                login_result["container_id"] = self.container_id
