...
class ContainerManager:
        ...
        try:
            ...
            for container_name, container in self.containers.items():
                try:
                    ...
                    logger.info(f"✅ 容器 {container_name} 状态: {status['status']}")
    ...
    def _perform_health_check(self):
        ...
        try:
            ...
            logger.info(f"✅ 健康检查完成: 活跃容器: {self.stats['active_containers']}/{len(self.containers)}")
        except Exception as e:
    ...
    def restart_container(self, container_name: str) -> Dict[str, Any]:
        ...
        if container:
            try:
                ...
                # 这里可以添加容器重启逻辑
                # 对于当前实现，我们只重置容器状态
                result = container.reset_container()
    ...
    def update_container_config(self, container_name: str, config_updates: Dict[str, Any]) -> Dict[str, Any]:
        """更新容器配置"""
        container = self.containers.get(container_name)
        if container:
            try:
    ...
    def _cleanup(self):
        """清理资源"""
        try:
            logger.info(f"🧹 清理容器管理器资源...")
            
            # 停止健康检查线程
            if self.health_check_thread:
                self.health_check_thread = None
            
            logger.info(f"✅ 容器管理器资源清理完成")
        except Exception as e:
