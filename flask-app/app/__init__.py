...
def create_app(config_type=None):
    ...
    # 获取中文日志记录器
    logger = logging_manager.get_logger('客户端交互日志')
    
    # 添加客户端请求日志中间件
    @app.before_request
    def log_request_info():
        """记录请求信息"""
        logger.info(f"[客户端请求] {request.remote_addr} - {request.method} {request.path}")
        logger.info(f"[请求头] {dict(request.headers)}")
        if request.method in ['POST', 'PUT', 'PATCH']:
            logger.info(f"[请求体] {request.get_data(as_text=True)}")
    
    # 添加响应日志中间件
    @app.after_request
    def log_response_info(response):
        """记录响应信息"""
        logger.info(f"[客户端响应] {request.remote_addr} - {request.method} {request.path} - 状态码: {response.status_code}")
    ...
    # 初始化并注册所有路由
    try:
        from app.routes import route_manager, init_routes
        init_routes()
...
# 初始化碎片化临时缓存系统
try:
    from app.utils.fragmented_cache import get_cache_manager
...
# 初始化网管AI
try:
    from app.ai.network_admin_ai import init_network_admin_ai
