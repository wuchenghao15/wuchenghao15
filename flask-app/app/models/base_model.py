...
class BaseModel:
        ...
        if table_columns:
            for col_name in table_columns.keys():
                if col_name in kwargs:
                    self._data[col_name] = kwargs[col_name]
                else:
                    # 设置默认值
            ...
            if isinstance(col_def, dict):
                ...
                if col_def.get('primary_key'):
                    if col_def.get('auto_increment'):
                        col_type = f"{col_type} PRIMARY KEY AUTOINCREMENT"
                    else:
        ...
        if table_columns:
            for col_name in table_columns.keys():
                if col_name != self.primary_key:  # 主键由数据库自动生成
                    insert_data[col_name] = self._data.get(col_name)
        
        # 执行插入
        primary_key_value = db_manager.insert(self.table_name, insert_data)
    ...
    def _update(self):
        ...
        for col_name in self._dirty:
            update_data[col_name] = self._data.get(col_name)
        
        if not update_data:
            logger.info(f"没有修改任何字段，跳过更新 {self.table_name} 记录")
            return True
        ...
        if db_manager.update(self.table_name, update_data, where_clause, where_params):
            ...
            logger.error(f"更新 {self.table_name} 记录失败，主键: {self._data[self.primary_key]}")
            return False
    
    def delete(self):
        """删除记录"""
        if not self.primary_key or not self._data.get(self.primary_key):
            logger.error(f"无法删除 {self.table_name} 记录，主键未设置")
            return False
    ...
    @classmethod
    def get_by_id(cls, primary_key_value):
        """通过主键获取记录"""
    ...
    def add_column(cls, column_name, column_type):
        ...
        if not cls.table_name:
            logger.error(f"模型 {cls.__name__} 没有定义 table_name 属性")
            return False
