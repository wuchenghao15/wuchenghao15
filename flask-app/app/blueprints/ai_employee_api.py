...
def get_ai_employee(employee_id):
    ...
    try:
        ...
        if not employee:
            return jsonify({"success": False, "message": f"未找到AI员工: {employee_id}"}), 404
...
def upgrade_all_ai_employees():
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "data": result,
            "message": f"成功升级 {result['success_count']}/{result['total_count']} 个AI员工"
        })
    except Exception as e:
...
def instantiate_ai_employee():
    ...
    try:
        ...
        # 实例化新的AI员工
        employee = ai_employee_manager.instantiate_employee(role)
        if not employee:
