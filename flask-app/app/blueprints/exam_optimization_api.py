...
def provide_feedback():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'message': f'老师AI提供反馈失败: {str(e)}'
        }), 500


@exam_optimization_api.route('/teacher-ai/practice-questions', methods=['POST'])
...
@exam_optimization_api.route('/teacher-ai/progress/<int:user_id>', methods=['GET'])
...
def analyze_learning_interest(user_id):
    """
    分析学习兴趣
    """
    try:
...
@exam_optimization_api.route('/learning-analysis/direction/<int:user_id>', methods=['GET'])
...
@exam_optimization_api.route('/learning-activities', methods=['POST'])
