...
def get_ai_rule(rule_id):
    ...
    try:
        ...
        # 获取规则历史版本
        rule_history = AIRule.get_rule_history(rule_id)
        ...
        logger.error(f"获取AI规则详情失败: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'获取AI规则详情失败: {str(e)}'
        }), 500
...
def create_ai_rule():
    ...
    try:
        ...
        logger.error(f"创建AI规则失败: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'创建AI规则失败: {str(e)}'
        }), 500

@smart_ai_rule_management_bp.route('/api/smart-ai-rule-management/rules/<int:rule_id>', methods=['PUT'])
...
def toggle_ai_rule_status(rule_id):
    ...
    try:
        ...
        updated_rule = AIRule.update_rule(rule_id, status=new_status)
        
        return jsonify({
            'success': True,
            'rule': {
                'id': updated_rule.rule_id,
...
@smart_ai_rule_management_bp.route('/api/smart-ai-rule-management/rules/<int:rule_id>/optimize', methods=['POST'])
def optimize_ai_rule(rule_id):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'优化AI规则失败: {str(e)}'
        }), 500

@smart_ai_rule_management_bp.route('/api/smart-ai-rule-management/generate-rule', methods=['POST'])
...
@smart_ai_rule_management_bp.route('/api/smart-ai-rule-management/rule-suggestions')
...
def generate_new_rule_suggestions():
    ...
    return [
        ...
        },
        {
            ...
            'priority': 'medium',
            'confidence': 0.8,
            'suggested_content': '当CPU使用率持续超过90%达5分钟以上时，触发系统警报'
        },
        {
            'type': 'new_rule',
