...
@smart_system_config_bp.route('/api/smart-system-config/optimization-history')
...
def get_config_impact():
    ...
    try:
        ...
        # 分析配置变更影响
        impact_analysis = analyze_config_impact(config_key, new_value)
        ...
        logger.error(f"获取配置变更影响分析失败: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'获取配置变更影响分析失败: {str(e)}'
        }), 500

@smart_system_config_bp.route('/api/smart-system-config/save-config', methods=['POST'])
def save_config():
    """保存系统配置"""
    try:
        data = request.json
...
def perform_auto_optimization(optimization_type):
    ...
    if optimization_type in ['all', 'performance']:
        optimized_configs.append({
            ...
            'new_value': Config.MAX_WORKERS + 2,
            'reason': '基于当前系统负载，建议增加工作进程数以提高性能'
        })
        optimized_configs.append({
            'config_key': 'timeout',
            'old_value': Config.TIMEOUT,
    ...
    if config_key == 'debug':
        if new_value == 'true' or new_value is True:
            ...
            impact_analysis.update({
                ...
                'resource_impact': 'low',
                'recommendation': '关闭调试模式将提高系统安全性和性能'
            })
