...
@ai_bp.route('/delete_instance/<instance_id>')
@check_user_status
def delete_instance(instance_id):
    """删除AI实例"""
    try:
        # 从内存中删除
        ai_instance_manager.delete_ai_instance(instance_id)
...
def bind_instance(instance_id, user_id):
    ...
    try:
        ...
        flash(f'绑定AI实例时发生错误: {str(e)}', 'danger')
        return redirect(url_for('ai.instances'))

# AI监控和学习功能已整合到主仪表盘，保留API路由供仪表盘调用
...
def update_learning_config():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': '学习配置更新成功'})
    except Exception as e:
    ...
    try:
        ...
        for instance in db_instances:
            try:
                ...
                # 示例升级逻辑：添加升级标记和版本信息
                updated_config = instance.config.copy()
        ...
        logger.error(f"升级AI实例时发生错误: {str(e)}")
...
def dispatch_task():
    ...
    try:
        ...
        if not task_type:
            return jsonify({'success': False, 'message': '任务类型不能为空'}), 400
