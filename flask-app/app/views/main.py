...
def check_username(func):
    ...
    return check_user_status(func)

@main_bp.route('/')
def index():
    """固定系统主页为index.html，并将参数上传到数据库，验证用户信息"""
    try:
            ...
            try:
                ...
                if session_id:
                    ...
                    if not session_valid:
                        logger.warning(f"会话无效: {session_message}")
                        # 清除会话，重新登录
                        session.clear()
                        # 重定向到首页，触发自动游客登录
                        return redirect(url_for('main.index'))
                ...
                # 验证用户是否存在于数据库
                if user_id:
                    user_from_db = User.get_by_id(user_id)
                    if not user_from_db:
        ...
        try:
            ...
            logger.info("主页参数已成功上传到数据库")
        except Exception as db_error:
            logger.error(f"上传主页参数到数据库失败: {str(db_error)}")
        
        # 固定显示index.html
        logger.info("固定显示index.html")
        return render_template('index.html', 
                           user=user,
                           versions=versions)
    except Exception as e:
        logger.error(f"主页路由处理失败: {str(e)}")
        import traceback
        traceback.print_exc()
...
def smart_redirect():
    ...
    try:
        ...
        return redirect(url_for(best_route))
    except Exception as e:
...
def dashboard():
    ...
    try:
        ...
        return render_template('dashboard.html', 
                           ...
                           ai_instance_count=dashboard_data['ai_instance_count'],
...
def admin_center():
    ...
    try:
        ...
        if 'user_management' in user_permissions:
            admin_pages.append({
                'name': '用户管理',
                'url': url_for('main.permissions'),
                'icon': 'fas fa-users',
                'description': '管理用户账户和权限'
            })
        
        # 系统监控页面
        if 'log_view' in user_permissions:
            admin_pages.append({
                'name': '系统监控',
                'url': url_for('main.system_monitoring'),
        ...
        if 'permission_management' in user_permissions:
            admin_pages.append({
                'name': '权限管理',
                'url': url_for('smart_permission_management.smart_permission_management'),
        ...
        if 'log_view' in user_permissions:
            try:
                admin_pages.append({
                    'name': '智能仪表板',
                    'url': url_for('smart_dashboard.smart_dashboard'),
...
def permissions():
    ...
    try:
        ...
        return f"访问权限管理页面时发生错误: {str(e)}", 500
...
def system_monitoring():
    ...
    try:
        ...
        if 'log_view' not in user_permissions:
            flash('没有权限访问系统监控页面', 'error')
            return redirect(url_for('main.smart_redirect'))
...
def cleanup():
    ...
    try:
        ...
        return f"访问系统清理页面时发生错误: {str(e)}", 500
...
def hardware_keys():
    ...
    try:
        ...
        return f"访问硬件密钥管理页面时发生错误: {str(e)}", 500
...
# 测试生成API
@main_bp.route('/api/japanese-test/generate', methods=['POST'])
def generate_japanese_test():
    """生成日语测试试卷"""
    try:
        # 检查用户角色，管理员、超级管理员和硬件管理员不能生成测试试卷
        ...
        logger.error(f"生成日语测试试卷失败: {str(e)}")
        return jsonify({
            'success': False,
...
def submit_japanese_level_test():
    ...
    try:
        ...
        logger.error(f"提交日语等级测试失败: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'提交测试失败: {str(e)}'
        }), 500
...
def save_language_level():
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            ''', (level, session.get('user_id'), subject))
        else:
            # 创建新记录
            cursor.execute('''
        ...
        logger.info(f"用户 {session.get('user_id')} 的 {language} 语言等级已保存为: {level} (得分: {score})")
...
def sync_database():
    ...
    try:
        ...
        logger.info(f"收到数据库同步请求，数据大小: {len(json.dumps(local_data))} 字节")
        
        # 这里可以添加数据整合AI的逻辑
        # 目前只是记录日志，实际项目中可以实现数据合并
...
def add_system_config():
    ...
    try:
        from app.models.system_config import SystemConfig
        from app.models.config_approval import ConfigApproval
        ...
        # 检查配置键是否已存在
        existing_config = SystemConfig.get_by_key(data['config_key'])
        if existing_config:
            return jsonify({
                'success': False,
                'error': '配置项已存在'
        ...
        if user_role in ['super_admin', 'hardware_vikey_admin']:
            ...
            approval = ConfigApproval(
                ...
                data_type=data['config_type'],
                requested_by=username,
                requested_role=user_role
            )
            approval.save()
            
            logger.info(f"用户{username}提交配置添加请求: {data['config_key']}，等待审批")
            
            return jsonify({
                'success': True,
                'message': '配置添加请求已提交，等待审批'
            }), 201
    except Exception as e:
...
def update_system_config(config_key):
    ...
    try:
        ...
        if not config:
            ...
            config = next((c for c in all_configs if c.config_key == config_key), None)
            if not config:
                return jsonify({
                    'success': False,
                    'error': '配置项不存在'
        ...
        if user_role in ['super_admin', 'hardware_vikey_admin']:
            ...
            logger.info(f"超级管理员/硬件管理员{username}直接更新配置: {config_key}")
            ...
            approval = ConfigApproval(
                ...
                data_type=data.get('config_type', config.config_type),
                requested_by=username,
                requested_role=user_role
            )
            approval.save()
...
# 审批相关API
@main_bp.route('/api/config-approval', methods=['GET'])
def get_config_approvals():
    ...
    try:
        ...
        # 获取审批列表
        approvals = ConfigApproval.get_pending_approvals()
...
def start_placement_test():
    ...
    try:
        ...
        logger.error(f"开始初次等级评测失败: {str(e)}")
        return jsonify({
            'success': False,
...
def next_placement_test_questions():
    ...
    try:
        ...
        # 从session中获取测试状态
        test_state = session.get('placement_test', {})
        if not test_state:
            return jsonify({
                'success': False,
        ...
        if result['success']:
            ...
            logger.info(f"获取下一组题目，当前难度: {result['current_difficulty']}，已完成: {result['current_question']}/{result['total_questions']}")
...
def complete_placement_test():
    ...
    try:
        ...
        logger.info(f"完成初次等级评测，用户: {user_id}，语言: {test_state.get('language')}，等级: {final_level}")
