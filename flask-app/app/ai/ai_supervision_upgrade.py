...
class AISupervisionManager:
    ...
    def manually_perform_maintenance(self) -> Dict:
        ...
        try:
            ...
            self.logger.error(f"手动执行维护失败: {str(e)}")
            return {
                "success": False,
                "error": str(e)
            }
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            health = supervision_manager.get_system_health()
            print(f"\n系统状态更新:")
            print(f"  CPU: {health['system_metrics']['cpu_usage']:.1f}% | 内存: {health['system_metrics']['memory_usage']:.1f}% | 磁盘: {health['system_metrics']['disk_usage']:.1f}%")
            print(f"  AI成功率: {health['ai_metrics']['system_success_rate']:.2f} | 活跃AI员工: {health['ai_metrics']['active_ai_employees']}")
