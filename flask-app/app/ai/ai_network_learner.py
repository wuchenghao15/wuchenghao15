...
KNOWLEDGE_SOURCES = [
    ...
    },
    {
        ...
        'category': 'system',
        'domain': 'Architecture',
        'keywords': ['microservices', 'design pattern', 'distributed system', 'scalability', 'availability', 'resilience']
    },
    {
        'name': '软件工程',
...
class NetworkKnowledgeCollector:
    ...
    def _init_database(self):
        try:
            ...
            conn.commit()
            conn.close()
            print("[NetworkLearner] 数据库表初始化完成")
        except Exception as e:
            print(f"[NetworkLearner] 初始化数据库失败: {e}")
    ...
    def fetch_web_content(self, url, timeout=10):
        ...
        try:
            ...
            print(f"[NetworkLearner] 获取网页失败 {url}: {e}")
            return None, 0
    ...
    def fetch_and_extract(self, source_info):
        """获取并提取知识"""
        print(f"[NetworkLearner] 开始采集: {source_info['name']}")
        ...
        if knowledge_points:
            ...
            print(f"[NetworkLearner] 成功提取 {len(knowledge_points)} 个知识点")
        else:
    ...
    def _save_knowledge(self, knowledge_points):
        ...
        try:
            ...
            for point in knowledge_points:
                knowledge_id = f"NK-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
            ...
            print(f"[NetworkLearner] 保存知识失败: {e}")
    ...
    def _update_source_success(self, source_name):
        ...
        try:
            ...
            print(f"[NetworkLearner] 更新源统计失败: {e}")
    ...
    def _update_source_failed(self, source_name):
        ...
        try:
            ...
            print(f"[NetworkLearner] 更新源失败统计失败: {e}")
    ...
    def feed_to_brain(self):
        ...
        try:
            ...
            if not records:
                print("[NetworkLearner] 无可投喂的知识")
                conn.close()
                return {'success': False, 'message': '无可投喂的知识'}
            
            fed_count = 0
            knowledge_id = f"K-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
            ...
            print(f"[NetworkLearner] 成功投喂 {fed_count} 条知识到脑库")
            return {'success': True, 'fed_count': fed_count, 'message': f"成功投喂 {fed_count} 条知识到脑库"}
        
        except Exception as e:
            print(f"[NetworkLearner] 投喂脑库失败: {e}")
            return {'success': False, 'error': str(e)}
    
    def run_collection(self):
        """执行知识采集"""
        print("[NetworkLearner] 开始网络知识采集...")
        all_points = []
        ...
        for source_info in KNOWLEDGE_SOURCES:
            try:
                ...
                print(f"[NetworkLearner] 采集源 {source_info['name']} 失败: {e}")
        
        print(f"[NetworkLearner] 采集完成, 共获取 {len(all_points)} 个知识点")
        return all_points
    ...
    def get_learning_records(self, limit=100):
        ...
        try:
            ...
            print(f"[NetworkLearner] 获取学习记录失败: {e}")
            return []
