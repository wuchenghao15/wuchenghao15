...
class AIAutomationManager:
        ...
        try:
            ...
            for row in rows:
                plan = {
                    ...
                    'start_time': row[4],
                    'end_time': row[5],
                    'tasks': json.loads(row[6]) if row[6] else [],
                    'created_by': row[7],
                    'created_at': row[8],
                    'updated_at': row[9]
                }
    ...
    def create_ai_plan(self, name, description="", tasks=None):
        ...
        try:
            ...
            db_manager.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (plan_id, name, description, "draft", json.dumps(tasks or []), "system", current_time, current_time))
            ...
            ai_roles = [
                {
                    "name": "系统监控员",
                    "role": "monitor",
                    "responsibilities": ["监控系统运行状态", "分析系统性能", "生成告警信息"]
                },
                {
                    "name": "系统优化师",
                    "role": "optimizer",
    ...
    def execute_ai_plan(self, plan_id):
        ...
        try:
            ...
            from app.utils.db import db_manager
            db_manager.execute('''
                UPDATE ai_plans SET status=?, start_time=?, updated_at=?
                WHERE plan_id=?
            ''', (plan['status'], plan['start_time'], time.strftime("%Y-%m-%d %H:%M:%S"), plan_id))
            ...
            # 保存到数据库
            db_manager.execute('''
                UPDATE ai_plans SET status=?, end_time=?, tasks=?, updated_at=?
                WHERE plan_id=?
            ''', (plan['status'], plan['end_time'], json.dumps(plan['tasks']), 
