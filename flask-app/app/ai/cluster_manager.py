...
class ClusterManager:
    ...
    def __init__(self):
        ...
        logger.info(f"[集群管理] 集群配置: {self.config}")
        ...
        try:
            with self.leader_lock:
                if not self.leader_node:
                    logger.warning("[集群管理] 没有领导者节点，跳过数据同步")
                    return
                ...
                try:
                    ...
                    if config_response.status_code == 200:
                        ...
                        if config_data.get('success'):
                            ...
                            logger.error(f"[集群管理] 系统配置同步失败: {config_data.get('message')}")
                except Exception as e:
    ...
    def _elect_leader(self):
        ...
        选举领导者
        """
        logger.info("[集群管理] 开始领导者选举...")
        
        # 简单的领导者选举算法：选择第一个健康的节点
        with self.status_lock:
        ...
        if healthy_nodes:
            ...
            logger.warning("[集群管理] 没有健康的节点，无法选举领导者")
    ...
    def get_cluster_status(self):
        ...
        with self.leader_lock:
            return {
                "cluster_name": self.config.get('CLUSTER_NAME', 'mtscos-cluster'),
