...
class UserLoginAI:
    ...
    def check_user_blacklist(self, username):
        ...
        try:
            ...
            logger.error(f"检查用户黑名单失败: {str(e)}")
            return False
        ...
        try:
            ...
            if result:
                if isinstance(result, dict):
                    return result.get('group_name', 'default')
    ...
    def archive_login_info(self, user_id, username, ip_address, user_agent, user_group):
        """
        归档登录信息
        ...
        try:
            ...
            logger.info(f"登录信息归档成功: {username}, IP: {ip_address}")
        except Exception as e:
            logger.error(f"归档登录信息失败: {str(e)}")
    
    def record_login_attempt(self, username, ip_address, user_agent, success, reason):
        """
        记录登录尝试
    ...
    def add_to_blacklist(self, target_type, target_value, reason):
        ...
        try:
            if target_type == "ip":
                ...
                # 检查是否已存在
                existing = db_manager.fetch_one(
                    'SELECT id FROM user_blacklist WHERE username = ?',
                    (target_value,)
                )
...
def get_user_login_ai():
    ...
    if _user_login_ai is None:
        try:
            _user_login_ai = UserLoginAI()
        except Exception as e:
            logger.error(f"用户登录AI初始化失败: {str(e)}")
            # 创建一个简化版的UserLoginAI实例，确保应用能继续运行
