...
class AIEnsemble:
    ...
    def add_component(self, component_id: str, component, role: str):
        self.components[component_id] = {'instance': component, 'role': role, 'status': 'active', 'added_at': datetime.now().isoformat()}
    ...
    def add_relationship(self, from_component: str, to_component: str, relationship_type: str):
        self.relationships[(from_component, to_component)] = {'type': relationship_type, 'created_at': datetime.now().isoformat()}
    ...
    def get_status(self) -> Dict[str, Any]:
        return {
            ...
            'component_details': {id: {'role': comp['role'], 'status': comp['status']} for id, comp in self.components.items()},
...
def init_ai_ensemble():
    ...
    for comp_id, name, role in components:
        ...
        ai_ensemble.add_component(comp_id, SimpleComponent(name, role), role)
