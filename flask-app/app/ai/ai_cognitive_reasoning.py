...
class AICognitiveReasoning:
    ...
    def _generate_task_id(self):
        return 'task_' + hashlib.sha256((str(datetime.now().timestamp()) + str(random.random())).encode()).hexdigest()[:16]
    
    def _generate_knowledge_id(self):
        return 'kb_' + hashlib.sha256((str(datetime.now().timestamp()) + str(random.random())).encode()).hexdigest()[:16]
