...
class QuestionBankExpander:
        ...
        self._config = {
            ...
            "subjects": {
                ...
                "japanese": {
                    ...
                    "templates": {
                        "single_choice": [
                            "正しい単語を選択してください：{sentence}",
                            "{word}の同義語はどれですか？",
                            "正しい文法構造を選択してください：{sentence}"
                        ],
                        "multiple_choice": [
                            "{topic}の正しい使い方はどれですか？",
    ...
    def _generate_question(self, language_id: int, level_id: int, category_id: int, 
        ...
        try:
                ...
                if question:
                    if check_duplicate:
                        ...
                        if not is_duplicate:
                            ...
                            logger.info(f"生成的题目重复，尝试重新生成 (尝试 {attempt+1}/{max_attempts})")
                    else:
                        # 跳过重复检测，直接返回题目
                        return question
            ...
            # 多次尝试后仍然重复
            if check_duplicate:
                logger.warning(f"生成题目失败：多次尝试后仍然重复")
            return None
        except Exception as e:
    ...
    def _determine_subject(self, language_id: int, category_id: int) -> str:
        ...
        if language_id == 1:  # 日语
            ...
            if category_id in [1, 2, 3]:  # 数学相关分类
                ...
                return "english"
            elif category_id in [10, 11, 12]:  # 日语相关分类
                return "japanese"
            else:
                return "general"
    ...
    def _generate_single_choice_question(self, language_id: int, level_id: int, 
        ...
        # 根据语种生成不同的题目
        if language_id == 1:  # 日语
            word = self._generate_english_vocabulary(difficulty)
            ...
            answer = "A"
            explanation = "The meaning of this word is..."
    ...
    def _generate_true_false_question(self, language_id: int, level_id: int, 
        ...
        # 根据语种生成不同的题目
        if language_id == 1:  # 日语
            content = f"次の文は正しいですか？{self._generate_english_statement(difficulty)}"
            options = ["正しい", "間違っている"]
            answer = "A"
            explanation = "この文は..."
        else:  # 英语
    ...
    def _generate_english_statement(self, difficulty: str) -> str:
        ...
        statements = {
            ...
            "hard": [
                ...
                "Artificial intelligence will greatly change our lives.",
    ...
    def _generate_japanese_fill_blank(self, difficulty: str) -> str:
        ...
        """
        blanks = {
            "easy": ["私は___です。", "___はかわいいです。", "今日は___です。"],
            "medium": ["昨日は友達と___を見ました。", "毎朝___をしています。", "明日は___があります。"],
            "hard": ["この問題は___で解決が難しいです。", "彼は仕事で大きな___を上げました。", "将来の___を立てる必要があります。"]
        }
