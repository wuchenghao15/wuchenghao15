...
class UserBehaviorAI:
    ...
    def generate_behavior_report(self, user_id, time_range="7d"):
        ...
        Returns:
            dict: 行为报告
        """
        logger.info(f"生成用户 {user_id} 的行为报告，时间范围: {time_range}")
    ...
    def _save_behavior_report(self, report):
        ...
        try:
            ...
            db_manager.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                ''',
                (report["report_id"], report["user_id"], report["generated_at"], report["time_range"], json.dumps(report))
            )
    ...
    def get_behavior_report(self, report_id):
        ...
        try:
            report_data = db_manager.fetch_one(
                'SELECT report_data FROM user_behavior_reports WHERE report_id = ?',
                (report_id,)
            )
            
            if report_data:
                report = json.loads(report_data[0])
                return {
                    "success": True,
                    "message": "行为报告获取成功",
                    "data": report
    ...
    def get_user_reports(self, user_id, limit=10):
        ...
        try:
            ...
            return {
                "success": False,
                "message": f"获取用户行为报告失败: {str(e)}",
                "data": []
            }
    
    def analyze_all_users_behavior(self, time_range="7d"):
        """
        分析所有用户的行为
        ...
        try:
            ...
            overall_stats = {
                ...
                "most_common_operation": max(operation_types.items(), key=lambda x: x[1])[0] if operation_types else "无",
