...
"""
AI学习规则引擎
实现自我发现学习方向，自动写入学习规则到系统规则表，并严格执行学习政策
"""
...
class LearningRuleEngine:
    ...
    def _init_database(self):
        try:
            ...
            conn.commit()
            conn.close()
            print("[LearningRuleEngine] 数据库表初始化完成")
        except Exception as e:
            print(f"[LearningRuleEngine] 初始化数据库失败: {e}")
    ...
    def discover_learning_directions(self):
        """自我发现学习方向"""
        print("[LearningRuleEngine] 开始自我发现学习方向...")
        ...
        print(f"[LearningRuleEngine] 发现 {len(discovered)} 条学习规则")
        return discovered
    ...
    def _analyze_learning_history(self):
        ...
        try:
            ...
            cursor.execute('''
                SELECT domain, topic, COUNT(*) as count, AVG(proficiency_gain) as avg_gain
            ...
            print(f"[LearningRuleEngine] 分析学习历史失败: {e}")
    ...
    def _analyze_upgrade_records(self):
        ...
        try:
            ...
            print(f"[LearningRuleEngine] 分析升级记录失败: {e}")
    ...
    def _analyze_maintenance_logs(self):
        ...
        try:
            ...
            print(f"[LearningRuleEngine] 分析维护日志失败: {e}")
    ...
    def _analyze_error_patterns(self):
        ...
        try:
            ...
            print(f"[LearningRuleEngine] 分析错误模式失败: {e}")
    ...
    def _analyze_network_learning(self):
        ...
        try:
            ...
            print(f"[LearningRuleEngine] 分析网络学习失败: {e}")
    ...
    def _write_rule_to_system_rules(self, rule):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            print(f"[LearningRuleEngine] 写入规则: {rule['rule_code']}")
        except Exception as e:
            print(f"[LearningRuleEngine] 写入规则失败: {e}")
    ...
    def _save_learning_rule(self, rule):
        ...
        try:
            ...
            print(f"[LearningRuleEngine] 保存学习规则失败: {e}")
    
    def execute_learning_policy(self):
        """执行学习政策"""
        print("[LearningRuleEngine] 开始执行学习政策...")
        
        active_rules = self._get_active_learning_rules()
        if not active_rules:
            print("[LearningRuleEngine] 无活动学习规则")
        ...
        print(f"[LearningRuleEngine] 执行完成, 成功 {executed_count}/{len(active_rules)}")
    ...
    def _get_active_learning_rules(self):
        ...
        try:
            ...
            cursor.execute('''
                ...
                ORDER BY learning_priority DESC, confidence DESC
                LIMIT 20
            ''')
            rows = cursor.fetchall()
            conn.close()
            ...
            print(f"[LearningRuleEngine] 获取活动规则失败: {e}")
            return []
    ...
    def _execute_rule(self, rule):
        ...
        try:
            ...
            print(f"[LearningRuleEngine] 执行规则失败 {rule['rule_code']}: {e}")
            return {'success': False, 'error': str(e)}
    
    def _execute_python_learning(self, rule):
        return {'success': True, 'message': f"执行Python领域学习政策: {rule['rule_name']}"}
    
    def _execute_flask_learning(self, rule):
        return {'success': True, 'message': f"执行Flask领域学习政策: {rule['rule_name']}"}
    
    def _execute_database_learning(self, rule):
        return {'success': True, 'message': f"执行数据库领域学习政策: {rule['rule_name']}"}
    
    def _execute_security_learning(self, rule):
        return {'success': True, 'message': f"执行安全领域学习政策: {rule['rule_name']}"}
    
    def _execute_ai_learning(self, rule):
        return {'success': True, 'message': f"执行AI领域学习政策: {rule['rule_name']}"}
    
    def _execute_upgrade_learning(self, rule):
        return {'success': True, 'message': f"执行升级领域学习政策: {rule['rule_name']}"}
    
    def _execute_maintenance_learning(self, rule):
        return {'success': True, 'message': f"执行维护领域学习政策: {rule['rule_name']}"}
    
    def _execute_error_handling_learning(self, rule):
        return {'success': True, 'message': f"执行错误处理学习政策: {rule['rule_name']}"}
    
    def _execute_default_learning(self, rule):
        return {'success': True, 'message': f"执行默认学习政策: {rule['rule_name']}"}
    ...
    def _update_rule_execution(self, rule_code):
        ...
        try:
            ...
            cursor.execute('''
                ...
                SET execution_count = execution_count + 1, last_executed = ?, updated_at = ?
                WHERE rule_code = ?
            ''', (datetime.now().isoformat(), datetime.now().isoformat(), rule_code))
            conn.commit()
            conn.close()
        except Exception as e:
            print(f"[LearningRuleEngine] 更新规则执行计数失败: {e}")
    ...
    def _record_policy_execution(self, rule, result):
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO learning_policy_executions
                (policy_id, policy_name, execution_type, target_domain, execution_result, success, executed_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                rule['rule_code'],
                rule['rule_name'],
            ...
            print(f"[LearningRuleEngine] 记录政策执行失败: {e}")
    ...
    def get_discovered_rules(self):
        ...
        try:
            ...
            cursor.execute('''
                SELECT * FROM learning_rules
                ORDER BY confidence DESC, learning_priority DESC
            ''')
            rows = cursor.fetchall()
            conn.close()
            ...
            print(f"[LearningRuleEngine] 获取发现规则失败: {e}")
            return []
    ...
    def get_policy_executions(self, limit=100):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                SELECT * FROM learning_policy_executions
            ...
            print(f"[LearningRuleEngine] 获取政策执行记录失败: {e}")
            return []
