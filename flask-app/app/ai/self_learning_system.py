...
class AISelfLearningSystem:
    ...
    def add_user_behavior(self, behavior_data: Dict):
        """添加用户行为数据"""
    ...
    def analyze_feature_importance(self):
        """分析特征重要性（外部调用接口）"""
        data = self._batch_process_data()
        return self._analyze_feature_importance(data)
    ...
    def get_learning_status(self):
        ...
        return {
            ...
            'data_counts': {
                'performance_metrics': len(self.learning_data['performance_metrics']),
                'resource_usage': len(self.learning_data['resource_usage']),
                'user_behaviors': len(self.learning_data['user_behaviors']),
                'error_logs': len(self.learning_data['error_logs'])
            },
            'config': self.config
        }
