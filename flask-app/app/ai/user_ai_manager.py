...
class UserAIManager:
    ...
    def process_login_request(self, username, password, request=None):
        ...
        if not login_ai:
            ...
            if user:
                # 绑定或获取用户专用AI
                self.bind_user_to_ai(user.user_id)
                return {
                    "success": True,
    ...
    def transfer_task_between_ais(self, source_ai_id, target_ai_id, task_info):
        """
        在AI实例之间转移任务
        
        Args:
            source_ai_id: 源AI实例ID
        ...
        """
        # 获取源AI和目标AI实例
        source_ai = ai_instance_manager.get_ai_instance(source_ai_id)
        ...
        if source_ai['status'] != 'active' or target_ai['status'] != 'active':
            logger.error(f"AI实例状态异常，源AI状态: {source_ai['status']}, 目标AI状态: {target_ai['status']}")
            return False
        
        # 这里可以添加任务转移的具体逻辑
        # 例如：将任务从源AI的任务队列中移除，添加到目标AI的任务队列中
    ...
    def create_user_ai_team(self, user_id):
        ...
        sub_ais = [
            ...
            },
            {
                ...
                "functions": ["user_query", "help_desk", "feature_guidance"],
                "responsibilities": ["处理用户查询", "提供帮助", "指导功能使用"]
            }
        ]
...
def get_user_ai_manager():
    ...
    """
    global _user_ai_manager
    if _user_ai_manager is None:
        _user_ai_manager = UserAIManager()
