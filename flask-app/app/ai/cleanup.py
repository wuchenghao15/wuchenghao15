...
class CleanupAI:
    ...
    def cleanup_resources(self):
        ...
        try:
            ...
            # 清理临时文件
            if self.cleanup_config["temp_files"]["enabled"]:
                temp_result = self._cleanup_temp_files()
                cleanup_results["temp_files"] = temp_result
            
            # 清理数据库
            if self.cleanup_config["database"]["enabled"]:
    ...
    def optimize_storage(self):
        ...
        try:
            ...
            # 分析大文件
            large_files = self._find_large_files(Config.BASE_DIR, threshold=10 * 1024 * 1024)  # 10MB
                ...
                for root, dirs, files in os.walk(directory):
                    for file in files:
                        ...
                        try:
                            ...
                            self.logger.error(f"{self.instance_id} 删除缓存文件失败: {file_path} - {str(e)}")
        ...
        try:
            ...
            stats = {
                ...
                "database": {
                    ...
                    "record_age": self.cleanup_config["database"]["record_age"]
                },
                "cache": {
                    "directories": len(self.cleanup_config["cache"]["directories"]),
