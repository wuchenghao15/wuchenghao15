...
class MechanismPolicyRuleServer:
    ...
    def __init__(self):
        ...
        self.config = {
            "enabled": True,
            "auto_adapt": True,  # 自动适配功能
            "ai_enhancement": True,  # AI增强功能
            "auto_optimization": True,  # 自动优化功能
            "auto_closure": True,  # 自动闭合功能
    ...
    def _monitor_server_status(self):
        """监控服务器整体状态"""
        logger.info(f"机制策略规则服务器状态: {self.status}, 配置: {json.dumps(self.config, indent=2)}")
    ...
    def update_session_activity(self, session_id):
        """更新会话活动时间"""
        return self.mechanism_ai.update_session_activity(session_id)
    ...
    def get_stats(self):
        ...
        return {
            ...
            "rule_manager_stats": rule_stats,
            "mechanism_ai_stats": mechanism_stats,
            "config": self.config
        }
    ...
    def update_config(self, new_config):
        ...
        self.updated_at = time.time()
        logger.info(f"机制策略规则服务器配置已更新: {new_config}")
        return True
    ...
    def _cleanup_expired_data(self):
        ...
        try:
            # 清理过期规则
            logger.info("    清理过期规则...")
            expired_rules = self.rule_manager.cleanup_expired_rules()
            logger.info(f"    清理了 {expired_rules} 个过期规则")
            
            # 清理过期锁定
            logger.info("    清理过期锁定...")
            expired_locks = self.mechanism_ai.cleanup_expired_locks()
            logger.info(f"    清理了 {expired_locks} 个过期锁定")
    ...
    def _close_unused_sessions(self):
        ...
        try:
            ...
            logger.info(f"    当前活跃会话数: 普通会话 {active_sessions}, Vikey会话 {active_vikey_sessions}")
            
            # 自动关闭长时间未活动的会话
            closed_sessions = self.mechanism_ai.close_idle_sessions()
    ...
    def start(self):
        ...
        if not self.running:
            ...
            self.monitor_thread.start()
            self.status = "active"
            logger.info(f"机制策略规则服务器已启动，服务器ID: {self.server_id}")
        return True
    ...
    def stop(self):
        ...
        if self.running:
            self.running = False
            self.status = "stopped"
            logger.info(f"机制策略规则服务器已停止，服务器ID: {self.server_id}")
        return True

# 创建机制策略规则服务器实例
mp_rule_server = MechanismPolicyRuleServer()
