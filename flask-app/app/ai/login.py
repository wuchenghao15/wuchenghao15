...
class LoginAI:
    ...
    def login_user(self, username, password, ip_address="127.0.0.1", request=None):
        ...
        try:
            ...
            if attempts_left <= 0:
                ...
                self.logger.warning(f"{self.instance_id} 登录尝试次数过多: {username}, IP: {ip_address}")
                return {
                    "success": False,
            ...
            if not validation_result["success"]:
                ...
                # 更新详细的登录记录
                login_context["status"] = "failed"
                login_context["reason"] = "validation_failed"
            ...
            # 增加成功登录次数
            self.login_stats["successful_logins"] += 1
            
            # 更新每日成功统计
            self._update_daily_stats(success=True, username=username, ip_address=ip_address)
            
            # 更新详细的登录记录
            login_context["status"] = "success"
            login_context["user_id"] = user.user_id if hasattr(user, 'user_id') else None
            login_context["user_role"] = user.role
            login_context["duration"] = time.time() - login_start_time
            self._record_login_attempt(login_context)
            
            self.logger.info(f"{self.instance_id} 用户登录成功: {username}, IP: {ip_address}, 角色: {user.role}, 耗时: {login_context['duration']:.3f}s")
            ...
            login_context["error_message"] = str(e)
            self._record_login_attempt(login_context)
    ...
    def _validate_login_data(self, username, password):
        """验证登录数据"""
    ...
    def _cleanup_old_attempts(self):
        ...
        # 清理旧的登录记录（超过配置的保留时间）
        retention_time = self.login_config.get('login_record_retention', 86400)  # 默认1天
    ...
    def _lock_user(self, username):
        ...
        if username not in self.login_stats["locked_out_users"]:
            ...
            # 设置解锁定时器
            threading.Timer(self.login_config["lockout_duration"], self._unlock_user, args=[username]).start()
