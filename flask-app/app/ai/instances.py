...
class AIInstanceManager:
    ...
    def create_ai_instance(self, instance_id, ai_type="general", name="", description="", 
                          functions=None, responsibilities=None, config=None, collection_id=None, user_role="admin", 
        ...
        with self.instance_lock:
            ...
            # 2. 跳过规则检查，避免循环依赖
            # 注意：这里移除了对rule_management_service的依赖，解决循环依赖问题
    ...
    def unbind_ai_instance(self, instance_id):
        ...
        with self.instance_lock:
            ...
            logger.info(f"AI实例 {instance_id} 已解除绑定")
            return True
    ...
    def remove_instance_from_collection(self, instance_id):
        ...
        with self.instance_lock:
            ...
            db_instance = AIInstance.get_by_id(instance_id)
            if db_instance:
                db_instance.collection_id = None
                db_instance.save()
    ...
    def get_instance_stats(self):
        ...
        with self.instance_lock:
            stats = {
                ...
                'active_instances': len([inst for inst in self.ai_instances.values() if inst['status'] == 'active']),
                'instance_types': {},
            ...
            for instance in self.ai_instances.values():
                ...
                if 'collaboration' in instance:
                    stats['collaboration_stats']['active_collaborations'] += len(instance['collaboration'].get('active_collaborations', []))
                    stats['collaboration_stats']['shared_tasks'] += len(instance['collaboration'].get('shared_tasks', []))
            
            return stats
    
    def send_message(self, from_instance_id, to_instance_id, message_type, content, metadata=None):
        """发送消息给另一个AI实例
    ...
    def start_collaboration(self, instance_id1, instance_id2, collaboration_type, goal, metadata=None):
        ...
        with self.instance_lock:
            ...
            # 创建协作
            collaboration_id = f"collab_{int(time.time())}_{instance_id1}_{instance_id2}"
            collaboration = {
            ...
            for instance_id, instance in list(self.ai_instances.items()):
                try:
                    ...
                    if should_upgrade:
                        ...
                        config['version'] = current_version + 0.1
    ...
    def share_knowledge_in_session(self, session_id, from_instance_id, knowledge_item):
        ...
        with self.instance_lock:
            ...
            # 检查分享者是否是会话参与者
            if from_instance_id not in participants:
            ...
            learning_session['activities'].append({
                ...
                'from_instance': from_instance_id,
                'timestamp': time.time(),
                'details': f"共享知识: {knowledge_item.get('title', '未知')}"
            })
        ...
        with self.instance_lock:
            ...
            for instance_id in participants:
                ...
                instance['audit_log'].append({
                    ...
                    'user_role': 'system',
                    'details': f"完成学习会话 {session_id}，主题: {learning_session['topic']}",
                    'status': 'success'
                })
            ...
            try:
                brain_service = _get_ai_brain_service()
                for knowledge_item in learning_session['shared_knowledge']:
    ...
    def create_vikey_ai_instance(self):
            ...
            for instance_id, instance in list(self.ai_instances.items()):
                try:
                    ...
                    default_functions = {
                        ...
                        'technical': ['代码编写', '系统设计', '故障诊断', '性能优化', '安全审计', '硬件管理', '网络配置', '系统集成'],
                        'education': ['教学辅助', '学习分析', '个性化辅导', '考试评估', '知识图谱构建', '学习路径规划'],
                        'business': ['市场分析', '商业决策', '客户服务', '销售预测', '风险管理', '运营优化']
                    }
                    
                    default_responsibilities = {
                        'general': ['提供全面支持', '协助日常任务', '保持沟通顺畅', '自动发现问题', '执行自我修复', '知识共享', '跨领域协作'],
                        'research': ['深入调研分析', '提供数据支持', '生成专业报告', '发现数据异常', '提供优化建议', '数据可视化', '学术研究支持'],
                        'creative': ['激发创意灵感', '设计优质内容', '提升用户体验', '适应不同风格', '满足个性化需求', '艺术创作', '品牌建设'],
                        'technical': ['解决技术难题', '优化系统性能', '确保系统稳定', '修复代码缺陷', '防范安全风险', '管理硬件设备', '系统集成'],
                        'education': ['提供教学支持', '分析学习数据', '个性化辅导', '评估学习效果', '构建知识体系', '规划学习路径'],
                        'business': ['分析市场趋势', '辅助商业决策', '提升客户服务', '预测销售业绩', '管理商业风险', '优化运营流程']
                    }
                    ...
                    if needs_upgrade:
                        ...
                        if db_instance:
                            ...
                            db_instance.name = instance.get('name')
                            db_instance.description = instance.get('description')
                            db_instance.functions = instance.get('functions', [])
    ...
    def query_brain_knowledge(self, keyword, knowledge_type=None):
        """从AI脑库查询知识"""
        try:
            brain_service = _get_ai_brain_service()
            return brain_service.search_knowledge(keyword, knowledge_type=knowledge_type)
        except Exception as e:
    ...
    def create_enhanced_ai_employee(self, employee_id=None, name=None, ai_type=None, 
        ...
        """
        with self.instance_lock:
            logger.info(f"🤖 开始创建强化版AI员工: {name}")
            ...
            try:
                ...
                # 保存到数据库
                enhanced_employee.save()
                
                logger.info(f"🎉 成功创建强化版AI员工: {enhanced_employee.employee_id}, 名称: {enhanced_employee.name}")
                
                # 返回强化版AI员工信息
                return enhanced_employee.to_dict()
            except Exception as e:
    ...
    def get_enhanced_ai_employee(self, employee_id):
        ...
        try:
            ...
            logger.error(f"❌ 获取强化版AI员工失败: {str(e)}")
            return None
    ...
    def get_all_enhanced_ai_employees(self):
        ...
        """
        try:
            enhanced_employees = EnhancedAIEmployee.get_all()
            return [employee.to_dict() for employee in enhanced_employees]
        except Exception as e:
            logger.error(f"❌ 获取所有强化版AI员工失败: {str(e)}")
            return []
    ...
    def instantiate_targeted_ai_employee(self, ai_type, target_system, capabilities=None, name=None):
        ...
        with self.instance_lock:
            ...
            try:
                ...
                description = f"针对{target_system}系统的{ai_type}类型AI员工"
                
                # 基于AI类型和目标系统生成能力列表
                base_capabilities = capabilities or []
                
                # 根据目标系统添加特定能力
                system_specific_capabilities = {
                ...
                type_specific_capabilities = {
                    'optimizer': ['javascript_optimization', 'code_optimization'],
                    'manager': ['system_version_management', 'system_monitoring'],
                    'developer': ['code_analysis', 'bug_fixing', 'web_development'],
                    'analyst': ['data_analysis', 'query_optimization']
                }
                
                if ai_type in type_specific_capabilities:
                    base_capabilities.extend(type_specific_capabilities[ai_type])
    ...
    def initialize_system_with_ai_employees(self):
        ...
        try:
            ...
            js_optimizer = self.create_enhanced_ai_employee(
                name="JavaScript优化AI",
                ai_type="optimizer",
