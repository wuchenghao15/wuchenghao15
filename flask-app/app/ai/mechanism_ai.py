...
class MechanismAI:
    ...
    def lock(self, resource_id, lock_type="exclusive", user_id=None, metadata=None):
        ...
        self.logger.info(f"资源已锁定: {lock_id}, 资源ID: {resource_id}, 用户ID: {user_id}")
        return lock_id
    ...
    def create_vikey_session(self, session_id, user_id, vikey_info, metadata=None):
        ...
        self.logger.info(f"Vikey会话已创建: {session_id}, 用户ID: {user_id}, Vikey硬件ID: {vikey_info.get('hardwareId')}")
        return session_id
    
    def update_vikey_activity(self, session_id):
        """更新Vikey会话活动时间"""
        if session_id in self.vikey_sessions:
            self.vikey_sessions[session_id]["last_activity"] = time.time()
            self.logger.debug(f"Vikey会话活动已更新: {session_id}")
            return True
        return False
    ...
    def handle_vikey_removal(self, session_id, vikey_info):
        """处理Vikey硬件拔出，包括自动清除痕迹、上传日志和强制退出"""
        if session_id in self.vikey_sessions:
            vikey_data = self.vikey_sessions[session_id]
            user_id = vikey_data['user_id']
    ...
    def handle_non_vikey_user_insert(self, current_user_id, session_id, vikey_info):
        ...
        try:
            ...
            if vikey_user_info:
                ...
                log_data = {
                    ...
                    'vikey_user_id': vikey_user_info.get('user_id'),
    ...
    def _notify_ai_instances(self, event_type, event_data):
        """通知相关AI实例"""
        self.logger.debug(f"通知AI实例: 事件类型: {event_type}, 数据: {event_data}")
        # 这里可以添加具体的AI实例通知逻辑
        
    def _take_user_snapshot(self, user_id, session_id):
        """获取用户状态快照"""
        self.logger.info(f"获取用户快照: 用户ID: {user_id}, 会话ID: {session_id}")
    ...
    def _verify_vikey_user(self, vikey_info):
        """验证Vikey硬件用户信息"""
        self.logger.info(f"验证Vikey用户信息: 硬件ID: {vikey_info.get('hardwareId')}")
    ...
    def _start_auxiliary_thread(self, thread_type):
        """启动辅助AI线程"""
        self.logger.info(f"启动辅助AI线程: 类型: {thread_type}")
    ...
    def _auxiliary_thread_worker(self, thread_type):
        ...
        try:
            ...
            self.logger.error(f"辅助线程 {thread_type} 出错: {e}")
            # 标记线程为失败
            if thread_type in self.auxiliary_threads:
