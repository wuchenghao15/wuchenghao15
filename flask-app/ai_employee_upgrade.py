...
class AIEmployeeManager:
    ...
    def instantiate_option_generator(self):
        ...
        try:
            # 尝试导入IntelligentOptionGenerator
            try:
                ...
                logger.warning("IntelligentOptionGenerator不存在，尝试使用AI服务管理器")
    ...
    def upgrade_exam_generator(self):
        ...
        try:
            ...
            elif hasattr(self.employees['exam_generator'], 'optimize_ensemble'):
    ...
    def run_upgrade_cycle(self):
        ...
        if 'exam_generator' in self.employees:
            # 测试生成试卷功能
            try:
                test_exam = self.employees['exam_generator'].generate_wrong_question_practice(
                    username="test_user",
                    subject="english",
                    count=1
                )
