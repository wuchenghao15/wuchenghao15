...
class IPManager:
        ...
        try:
            ...
            if existing:
                ...
                cursor.execute('''
                    ...
                    WHERE ip_address = ?
                ''', (ip_type, reason, added_by, expires_at, json.dumps(metadata), ip_address))
            else:
                # 插入新记录
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (ip_address, ip_type, reason, added_by, expires_at, json.dumps(metadata)))
    ...
    def get_ip_status(self, ip_address):
        ...
        try:
            ...
            print(f"Error getting IP status for {ip_address}: {str(e)}")
            return 'normal'
        finally:
            conn.close()
...
if __name__ == '__main__':
    ...
    # 测试移除IP
    print(f"\n移除10.0.0.1: {ip_manager.remove_ip('10.0.0.1')}")
    print(f"10.0.0.1 黑名单状态: {ip_manager.is_ip_blacklisted('10.0.0.1')}")
