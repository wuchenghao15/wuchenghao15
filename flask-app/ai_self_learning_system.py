            ...
            for row in cursor.fetchall():
                ...
                knowledge_list.append({
                    ...
                    "content": {
                        ...
                        "solution": {
                            "strategy": json.loads(fix_strategy),
                            "implementation": json.loads(fix_implementation)
                        },
...
class CodeRepositoryExtractor(KnowledgeExtractor):
    ...
    def __init__(self, repo_path: str):
        super().__init__("code_repositories")
    ...
    def extract_knowledge(self) -> List[Dict]:
        ...
        try:
            ...
            if result.stdout:
                pylint_result = json.loads(result.stdout)
            ...
            for css_file in css_files:
                ...
                # 提取CSS变量
                css_vars = re.findall(r':root\s*{([^}]*)}', content, re.DOTALL)
                if css_vars:
            ...
            logger.info(f"从代码仓库提取了 {len(knowledge_list)} 条知识")
        except Exception as e:
...
class FrontendBeautificationExtractor(KnowledgeExtractor):
    ...
    def extract_knowledge(self) -> List[Dict]:
        ...
        try:
            # 检查是否存在前端美化历史文件
            beautification_history_file = os.path.join(self.repo_path, "ai_frontend_beautification_history.json")
                    ...
                    for optimization in optimizations:
                        knowledge_list.append({
                            ...
                            "content": {
                                ...
                                "average_improvement": optimization["average_improvement"],
                                "last_used": optimization["last_used"]
                            }
                        })
        except Exception as e:
            logger.error(f"从前端美化结果提取知识出错: {str(e)}")
    ...
    def _extract_from_beautification(self, record: Dict) -> List[Dict]:
        ...
        try:
            # 提取美化效果知识
            improvement = record["new_score"] - record["original_score"]
            knowledge_list.append({
                "source": "beautification:effectiveness",
                "type": "code_pattern",
                "content": {
            ...
            logger.error(f"从美化记录提取知识出错: {str(e)}")
...
class ExternalAPIExtractor(KnowledgeExtractor):
    ...
    def extract_knowledge(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"从外部API提取了 {len(knowledge_list)} 条知识")
        except Exception as e:
...
class AILearningManager:
    ...
    def initialize(self):
        ...
        if cursor.fetchone()[0] == 0:
            test_logs = [
                ...
                ("/about", "GET", "10.0.0.1", "Mozilla/5.0", 200, 0.109, datetime.now(datetime.UTC).isoformat())
            ]
            cursor.executemany('''
                INSERT INTO access_logs (path, method, ip, user_agent, status_code, response_time, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', test_logs)
            logger.info("添加了测试访问日志数据")
    ...
    def _calculate_importance(self, knowledge: Dict) -> float:
        ...
        # 基于内容评分
        if "difficulty" in content and content["difficulty"] in ["hard", "medium"]:
            importance += 0.1
        # 前端美化知识的特殊内容评分
        elif "improvement" in content and content["improvement"] > 10:
        ...
        elif "count" in content and content["count"] > 5:
    ...
    def _check_code_issues(self):
        ...
        try:
            ...
            if result.stdout:
                ...
                # 只处理严重程度较高的问题
                critical_issues = [issue for issue in pylint_issues if issue.get('score', 0) < 6.0]
        ...
        try:
            ...
            if cursor.fetchone()[0] == 0:
                ...
                ''', (
                    "config_problem",
                    f"配置问题: {config_file} {issue}",
                    json.dumps({"file": config_file, "issue": issue}),
                    json.dumps({"suggestion": "需要人工修复"}),
                    1,  # 严重程度: 1表示低
                    "system",
                    datetime.now(UTC).isoformat(),
                ...
                conn.commit()
                logger.info(f"记录配置问题: {config_file} {issue}")
    ...
    def _optimize_knowledge_graph(self):
        ...
        try:
            ...
            # 2. 优化知识图谱查询性能
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_knowledge_graph_source ON knowledge_graph(source_knowledge_id)')
    ...
    def _integrate_knowledge(self, knowledge_list: List[Dict]):
        ...
        for knowledge in knowledge_list:
            ...
            cursor.execute('''
                SELECT COUNT(*) FROM ai_brain_features
                WHERE issue_description = ?
            ''', (knowledge["content"].get("problem", knowledge_type),))
            ...
            if cursor.fetchone()[0] == 0:
                ...
                ''', (
                    ...
                    json.dumps(knowledge["content"].get("solution", {})),
                    1,  # 默认严重程度
                    "general",  # 默认影响范围
                    datetime.now(UTC).isoformat(),
                    datetime.now(UTC).isoformat()
    ...
    def _build_knowledge_graph(self, knowledge_ids: List[int]):
        """构建知识图谱，建立知识之间的关联"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        # 获取所有知识
        cursor.execute('''
            SELECT id, feature_type, issue_description, issue_characteristics
    ...
    def _upgrade_knowledge_base(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS knowledge_graph (
                ...
                FOREIGN KEY (source_knowledge_id) REFERENCES ai_brain_features(id),
    ...
    def _create_knowledge_version(self, description: str):
        """创建知识版本"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def manual_learn(self, knowledge_source: str = "all"):
        """手动触发学习"""
        logger.info(f"手动触发学习，知识来源: {knowledge_source}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "start":
            ...
            print(json.dumps(ai_system.status(), indent=2, ensure_ascii=False))
        elif command == "learn":
