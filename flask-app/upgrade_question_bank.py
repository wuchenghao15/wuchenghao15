...
try:
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_questions_language ON questions(language)')
    ...
    if existing_count == 0:
        ...
        for i, q in enumerate(sample_questions, 1):
            cursor.execute('''
                INSERT INTO questions (language, level, category, content, options, correct_answer, explanation, source, question_type)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                q['language'], q['level'], q['category'], q['content'], 
        ...
        print(f"   ℹ️  数据库中已有 {existing_count} 道题目，跳过示例数据插入")
    
    # 6. 初始化题目统计信息
    print("\n6. 初始化题目统计信息:")
    cursor.execute('''
        INSERT OR IGNORE INTO question_stats (level, category, question_type, total_count, used_count, correct_rate, avg_accuracy)
        SELECT 
            level, 
            category, 
            question_type, 
