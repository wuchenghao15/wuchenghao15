    ...
    for root, dirs, files in os.walk(templates_dir):
        for f in files:
            if f.endswith('.html'):
                ...
                num_colors = (10 + secrets.randbelow(int(30) - int(10) + 1))
                num_colors = (10 + secrets.randbelow(int(30) - int(10) + 1))
                num_spacings = (5 + secrets.randbelow(int(20) - int(5) + 1))
                ...
                for i in range(num_spacings):
                    elements.append({
                        'type': 'container',
                        'margin': f'{secrets.choice([4, 8, 12, 16, 20, 24, 32, 40, 48, 56, 64])}px',
                        'padding': f'{random.choice([4, 8, 12, 16, 20, 24, 32, 40, 48])}px'
                    })
                for i in range(num_font_sizes):
                    elements.append({
                        'type': 'text',
                        'font_size': f'{secrets.choice([10, 11, 12, 13, 14, 15, 16, 17, 18, 20, 22, 24, 28, 32, 36, 40, 48])}px',
                ...
                for i in range(num_colors):
                    elements.append({
                        'type': 'element',
                        'color': f'#{(0 + secrets.randbelow(int(0xFFFFFF) - int(0) + 1)):06x}',
