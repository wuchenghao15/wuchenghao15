...
def test_ai_instance_creation_with_permissions():
    ...
    ai_instance_guest = ai_instance_manager.create_ai_instance(
        ...
        ai_type="general",
        name="测试AI实例-访客",
        description="用于测试访客权限的AI实例",
        functions=["general_assistant"],
        user_role="guest"
    )
    
    if not ai_instance_guest:
        logger.info(f"✓ 成功拒绝访客创建AI实例（权限不足）")
    else:
        logger.error(f"✗ 错误：允许访客创建AI实例")
    ...
    if ai_instance_technical:
        logger.info(f"✓ 成功创建技术AI实例: {ai_instance_technical['instance_id']}")
        logger.info(f"  功能: {ai_instance_technical['functions']}")
    else:
        logger.error(f"✗ 创建技术AI实例失败")
    ...
    if result_admin:
        ...
        logger.error(f"✗ 管理员执行规则失败")
    
    # 使用guest角色执行规则（应该失败）
    result_guest = rule_manager_ai.execute_rule(
        "test_rules",
        "test_generator",
        user_role="guest"
    )
...
def test_ai_instance_audit_log():
    ...
    # 获取AI实例
    ai_instance = ai_instance_manager.get_ai_instance("test-ai-instance-001")
    ...
    if ai_instance:
        ...
        if audit_log:
            ...
            logger.warning(f"⚠ AI实例审计日志为空")
    else:
        logger.error(f"✗ 未找到AI实例")
