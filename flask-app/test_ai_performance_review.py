...
def test_performance_trend():
    ...
    try:
        ...
        if trend['success']:
            logger.info(f"  绩效趋势: {trend['trend']}")
            logger.info(f"  平均评分: {trend['average_score']:.2f}")
            logger.info(f"  最高评分: {trend['highest_score']}")
            logger.info(f"  最低评分: {trend['lowest_score']}")
            logger.info(f"  评分历史: {trend['scores']}")
        else:
...
def test_performance_report():
    ...
    try:
        ...
        if report['success']:
            ...
            logger.info(f"  总审评数: {report['total_reviews']}")
            logger.info(f"  最新审评评分: {report['latest_review']['total_score']}")
            logger.info(f"  最新审评等级: {report['latest_review']['performance_level']}")
            logger.info(f"  绩效趋势: {report['performance_trend']['trend']}")
            logger.info("  建议:")
...
def test_improvement_plan():
    ...
    try:
        ...
        # 获取改进计划
        improvement_plans = ai_performance_review_system.get_all_improvement_plans()
        ...
        if improvement_plans:
            ...
            for item in plan['action_items']:
                logger.info(f"    - {item['goal']} (状态: {item['status']})")
...
def test_career_development_plan():
    ...
    try:
        ...
        logger.error(f"❌ 职业发展计划测试失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
