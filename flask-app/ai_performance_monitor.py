...
class AIPerformanceMonitor:
    ...
    def _init_db(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_anomaly_timestamp ON performance_anomalies(timestamp)')
    ...
    def record_request(self, endpoint, method, response_time, is_error=False):
        ...
        self.app_performance['error_rate'] = (
            ...
            if self.app_performance['request_count'] > 0 else 0
        )
        ...
        self._detect_performance_anomaly('error_rate', self.app_performance['error_rate'], endpoint)
    ...
    def _detect_performance_anomaly(self, metric, value, context=None):
        ...
        try:
            ...
            print(f"Error recording anomaly: {str(e)}")
            conn.rollback()
        finally:
            conn.close()
    ...
    def _generate_anomaly_details(self, metric, value, thresholds, context=None):
        ...
        """
        # 找到第一个超过的阈值键
        threshold_key = min(thresholds, key=lambda k: thresholds[k] if thresholds[k] < value else float("inf"))
        ...
        metric_details = {
            ...
            'disk': {
                ...
                'suggestion': '清理磁盘空间，考虑增加磁盘容量或优化存储使用'
            },
            'response_time': {
        ...
        return metric_details.get(metric, (
            f'性能异常: {metric} 值 {value} 超过阈值',
            '请检查系统或应用配置'
        ))
    ...
    def get_performance_report(self, time_window=3600):
        ...
        try:
            ...
            return {
                ...
                'system_performance': {
                    'avg_cpu_usage': round(system_stats[0], 2),
                    'max_cpu_usage': round(system_stats[1], 2),
                    'avg_memory_usage': round(system_stats[2], 2),
                    'max_memory_usage': round(system_stats[3], 2),
                    'avg_disk_usage': round(system_stats[4], 2),
                    'max_disk_usage': round(system_stats[5], 2),
                    'avg_network_sent': round(system_stats[6], 2),
                    'avg_network_recv': round(system_stats[7], 2)
                },
                'app_performance': {
    ...
    def run_monitor(self, duration=3600):
        ...
        while time.time() < end_time:
            ...
            try:
                ...
                print(f"Error saving system stats: {str(e)}")
                conn.rollback()
            finally:
                conn.close()
            ...
            self._detect_performance_anomaly('disk', system_stats['disk_usage'])
    ...
    def get_current_status(self):
        ...
        # 确保app_performance中的所有datetime对象都被转换为ISO格式字符串
...
if __name__ == '__main__':
    ...
    print(json.dumps(system_status, indent=2, ensure_ascii=False, default=str))
    
    # 测试记录请求
    print("\n2. 记录测试请求:")
    perf_monitor.record_request('/api/test', 'GET', 0.123)
    perf_monitor.record_request('/auth/login', 'POST', 0.456, is_error=True)
    perf_monitor.record_request('/api/test', 'GET', 0.098)
