...
# 防堆积单例锁（本地盘，避免 OneDrive IO 挂起）
SINGLETON_LOCK_FILE = '/tmp/mtscos_governance_engine.singleton.lock'
...
def run_daemon(interval=1200):
    ...
    try:
        ...
        _lock_fd = os.open(SINGLETON_LOCK_FILE, os.O_CREAT | os.O_RDWR, 0o644)
        try:
            fcntl.flock(_lock_fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except (BlockingIOError, OSError):
            os.close(_lock_fd)
            _log('已有 governance_engine daemon 实例在运行，当前进程退出（防堆积）')
            return
    except Exception as e:
        _log(f'获取单例锁失败（继续运行）: {e}')
        _lock_fd = None
