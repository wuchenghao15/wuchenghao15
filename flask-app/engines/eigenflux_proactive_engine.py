...
try:
    # [unused] from mt_ir14_dev_flow import _get_conn, _LOCK, feed_brain, transition, get_session, ensure_tables
    HAS_DEV_FLOW = True
except Exception:
    HAS_DEV_FLOW = False
...
def _query_main_db(sql, params=(), max_rows=12):
    ...
    try:
        if not os.path.isfile(_MAIN_DB):
            return out
        c = sqlite3.connect(f'file:{_MAIN_DB}?mode=ro', uri=True, timeout=30)
        try:
