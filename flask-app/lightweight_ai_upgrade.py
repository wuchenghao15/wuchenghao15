...
class LightweightAIUpgrade:
    ...
    def create_exam_generator(self):
        ...
        try:
            ...
            class LightweightExamGenerator:
                ...
                def __init__(self):
                    self.name = "lightweight_exam_generator"
                    self.version = "1.0.0"
    ...
    def create_option_generator(self):
        ...
        try:
            ...
            class LightweightOptionGenerator:
                """轻量级选项生成器"""
                
                def __init__(self):
                    self.name = "lightweight_option_generator"
                    self.version = "1.0.0"
                    # 不同题型的选项数量规则
                    self.option_rules = {
                        'single_choice': 4,    # 单选题4个选项
                        'multiple_choice': 5,  # 多选题5-6个选项
                ...
                def get_option_count(self, question_type):
                    """根据题型返回选项数量"""
                    return self.option_rules.get(question_type, 4)
    ...
    def verify_upgrade(self):
        ...
        if 'exam_generator' in self.employees:
            ...
            try:
                ...
                logger.info(f"✓ 试卷生成器测试成功，生成了{len(questions)}道题目")
            except Exception as e:
        ...
        if 'option_generator' in self.employees:
            ...
            try:
                ...
                logger.info(f"✓ 选项生成器测试成功，生成了{len(options)}个选项")
                
                # 测试选项数量规则
                single_choice_count = option_gen.get_option_count("single_choice")
                multiple_choice_count = option_gen.get_option_count("multiple_choice")
