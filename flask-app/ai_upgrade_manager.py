...
class AIUpgradeManager:
    ...
    def upgrade_ai_brain(self):
        ...
        try:
            # 创建知识表的新字段（如果不存在）
            self._add_ai_brain_columns()
            
            # 验证版本管理功能
            if hasattr(AIBrainKnowledge, 'save') and callable(getattr(AIBrainKnowledge, 'save')):
    ...
    def upgrade_feature_library(self):
        ...
        try:
            # 创建特征库表
            self._create_feature_library_table()
    ...
    def _migrate_feature_library(self):
        ...
        for feature in feature_library.get("features", []):
            try:
                ...
                # 处理JSON字段
                tags = json.dumps(feature.get("tags", [])) if feature.get("tags") else None
                resolution = json.dumps(feature.get("resolution", {})) if feature.get("resolution") else None
