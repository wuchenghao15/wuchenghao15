...
class MTSCOSSystemTestEngine:
    ...
    def test_all_users(self):
        ...
        print('使用超级管理员测试: wuchenghao15 (super_admin)')
        print()
        ...
        if logged_in:
            ...
            print()
            print('使用测试用户测试: test_auto_system (admin)')
            print()
        ...
        try:
            ...
            if self.errors_found:
                for error in self.errors_found:
                    knowledge_id = f'TEST-ERROR-{error.get("type")[:3]}-{datetime.datetime.now().strftime("%Y%m%d")}-{hashlib.sha256(json.dumps(error, ensure_ascii=False).encode()).hexdigest()[:8].upper()}'
