#!/usr/bin/env python3
"""iron_rule_flow_runner_system_governance.py
=================================================================================
§14 IRON_RULE 强制开发12步骤/18节点/5张强制表 实落库执行脚本
—— 系统治理与AI主动参与引擎（前端排版审计+路由链路闭环+AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排 · 本地零token）
   daemon sys_system_governance (1200s轮巡)
=================================================================================
D1 唯一flow_id生成；D2 边集合严格校验；D3 4方出席(张=A组,AI偶)；D4 5张表齐
D5 9A不回环+9B四落库；D6 mandatory=True 评估 v22.9.0→v22.10.0
D7 git SSH 优先；D8 千轮矩阵(400+300+300) + 重复1-11断言 → FINAL_DONE

STEP_12 AST 提取 ai_system_governance_engine.py 内 8常量+9纯函数 做1:1真源矩阵:
  常量: _MIN_CONSENSUS / _MAX_ISSUES / _MAX_SKILLS / _MAX_FEEDS / _MAX_TEAM_ADJUSTS
        / _MSG_TOP_K / _SEVERITY_LEVELS / _ISSUE_CATEGORIES
  函数: classify_issue / governance_wellformed / risk_priority / issue_uid
        / route_has_guard / engagement_cap / governance_decision / offline_first / skill_value
"""
...
SRC_FILE = f'{ROOT}/flask-app/engines/ai_system_governance_engine.py'
assert os.path.isfile(SRC_FILE), f'引擎正本不存在: {SRC_FILE}'
SELF_REL = 'flask-app/scripts/iron_rule_flow_runner_system_governance.py'
ISOLATED_GIT = f'{ROOT}/_runtime/git_push_ws/mtscos_push'

NOW_F = lambda fmt='%Y-%m-%d %H:%M:%S': datetime.datetime.now().strftime(fmt)
FLOW_ID = f'flow_system_governance_{NOW_F("%Y%m%d_%H%M%S")}'
...
# STEP 1 PROPOSAL  D1
# ============================================================
TITLE = '系统治理与AI主动参与引擎：前端排版审计(占位符/权限缺口/重复导航/硬编码)+路由链路闭环+AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排建议（本地零token）'
SUMMARY = ('daemon sys_system_governance(1200s) 七步闭环: '
 '①AUDIT 前端排版审计: 4类问题 classify_issue 关键词命中(placeholder/占位符/TODO/FIXME→PLACEHOLDER; 无装饰器/缺权限/未挂载system_container→PERMISSION_GAP; 重复导航/重复nav/重复菜单→DUPLICATE_NAV; 硬编码颜色/#xxx/rgb(→HARDCODED_COLOR; None/空/未命中→GENERAL兜底); '
 '②WELLFORMED 治理项格式校验: governance_wellformed 3参数(domain,severity,title)全非空+severity∈_SEVERITY_LEVELS+len(title)≥4 才落库; '
 '③ROUTE 路由链路闭环: route_has_guard 行文本含system_container→True, 空/None/不含→False; '
 '④RISK 风险优先级+建议落池: risk_priority CRITICAL→P0/HIGH→P1/MEDIUM·LOW→P2/非法·None→UNKNOWN; issue_uid(类别,目标) SG-前缀散列uid幂等(≤18) + engagement_cap(used,limit) cap(0,5)=5/cap(3,5)=2/cap(6,5)=0/cap(None,5)=0 负数·bool→0; '
 '⑤DECISION 共识决策+AI主动参与: governance_decision 0.75/0.65/10→full_create, 0.60→advise_only, (0.8,0.65,0)→(advise_only,no-gaps), None→(skip,bad-consensus) fail-safe; '
 '⑥ENGAGE AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排: skill_value(learning_value,msg_type) 0.9+LEARNING_SEDIMENT→HIGH/0.5→MEDIUM/0.1→LOW/None·非法→LOW; '
 '⑦OFFLINE+PERSIST 本地零token: offline_first force=True→OFFLINE_ONLY, None兜底→OFFLINE_ONLY fail-safe; 4新表留痕(系统治理轮巡log/前端审计问题库/路由链路闭环库/团队重排建议库) + 脑库投喂(列名1:1) + 团队重排建议.'
 '实跑证据：两轮闭环（首轮幂等0）→ 前端治理4类审计+路由链路闭环+AI主动试探激发+广播技能提取+脑库投喂频次提升+团队重排+建议池SG-*落池+第二轮幂等全0 → ZXF决策NOT_USE_SUSPEND.')
CHANGED_FILES = [
    'flask-app/engines/ai_system_governance_engine.py',
    'flask-app/engines/ai_smart_mount_engine.py',
    SELF_REL,
]
proposal = {'title': TITLE, 'summary': SUMMARY,
    'scope': {'daemon':'sys_system_governance 1200s轮巡(smart_mount注册 inspect_cycle=1200s, timeout=1152s)',
              'audit':'前端排版审计: classify_issue 4类关键词命中(placeholder/占位符/TODO/FIXME→PLACEHOLDER; 无装饰器/缺权限/未挂载system_container→PERMISSION_GAP; 重复导航/重复nav/重复菜单→DUPLICATE_NAV; 硬编码颜色/#xxx/rgb(→HARDCODED_COLOR)',
              'wellformed':'治理项格式校验: governance_wellformed 3参数(domain,severity,title)全非空+severity∈_SEVERITY_LEVELS+len(title)≥4',
              'route':'路由链路闭环: route_has_guard 行文本含system_container→True, 空/None/不含→False',
              'risk_priority':'风险优先级: risk_priority CRITICAL→P0/HIGH→P1/MEDIUM·LOW→P2/非法→UNKNOWN',
              'issue_uid':'建议落池: issue_uid(类别,目标) SG-前缀散列uid幂等(≤18) + engagement_cap CAP上限',
              'decision':'9纯函数fail-safe: classify_issue/governance_wellformed/risk_priority/issue_uid/route_has_guard/engagement_cap/governance_decision/offline_first/skill_value',
              'engage':'AI主动参与3机制: AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排建议',
              'offline_guarantee':'offline_first force=True→OFFLINE_ONLY 本地零token；所有AI走本地推理不触发远程API',
              'persist':'4新表留痕(系统治理轮巡log/前端审计问题库/路由链路闭环库/团队重排建议库) + 脑库投喂(列名1:1 flow_id/feed_target/payload_preview/fed_at/fed_by)'},
    'goals':['两轮实跑闭环(首创新→次幂等全0)','1000轮矩阵全PASS(vuln=0)','MT_IR_D1..D8零违反'],
    'target_version':'v22.10.0','changed_files':CHANGED_FILES,'new_tables_expected':4}
...
EF_NET = {'online_count': 11347 + 4, 'ts': NOW_F(), 'heartbeat_ok': True}  # 新+4名系统治理员工
...
MOTIONS = [
  {'sponsor':'EF-系统治理首席','motion':'治理硬约束：前端排版审计4类 classify_issue关键词命中(placeholder/占位符/TODO/FIXME→PLACEHOLDER; 无装饰器/缺权限/未挂载system_container→PERMISSION_GAP; 重复导航/重复nav/重复菜单→DUPLICATE_NAV; 硬编码颜色/#xxx/rgb(→HARDCODED_COLOR)未命中→GENERAL兜底；governance_decision共识合法性优先(None/越界/类型错→skip)；全部INSERT OR IGNORE幂等+CAP上限(engagement_cap)；offline_first默认OFFLINE_ONLY 零token优先',
   'vote_for':51,'vote_against':0,'abstain':0,'passage':True},
  {'sponsor':'EF-前端首席','motion':'路由链路闭环route_has_guard(行文本)含system_container→True，空/None/不含→False；issue_uid(类别,目标)SG-前缀散列uid确定幂等，不同类别即使同目标uid不同；建议落池≤_MAX_ISSUES',
   'vote_for':51,'vote_against':0,'abstain':0,'passage':True},
  {'sponsor':'EF-安全首席','motion':'风险优先级risk_priority四档强校验：CRITICAL→P0/HIGH→P1/MEDIUM·LOW→P2，非法/None→UNKNOWN拒绝；governance_wellformed 3参数(domain,severity,title)全非空+severity∈_SEVERITY_LEVELS白名单+len(title)≥4；注入路径(../evil)经SG-散列不落盘',
   'vote_for':51,'vote_against':0,'abstain':0,'passage':True},
  {'sponsor':'张晓峰','motion':'前端治理4类审计+AI主动参与3机制(主动试探激发+广播技能提取+脑库投喂频次提升)+团队重排+建议池SG-*落池+两轮幂等设计；governance_decision fail-safe；首轮创建+第二轮幂等全0闭环',
   'vote_for':51,'vote_against':0,'abstain':0,'passage':True},
...
DISCUSSION = {'motions':MOTIONS,
  ...
  'panel_opinions':{'EF-安全':'风险优先级四档白名单+UNKNOWN拒绝 零越权风险','EF-DBA':'新增4表系统治理轮巡log/前端审计问题库/路由链路闭环库/团队重排建议库可追溯','EF-运维':'1200s 1152s超时防堆积'}}
...
transition('STEP_2A_ROUND','STEP_3_ZXF_DECISION','ZXF_DECISION', {'decision':ZXF,'reason':'前端治理4类审计+AI主动参与3机制+团队重排+建议池SG-*落池+两轮幂等设计'}, by='张晓峰')
...
# STEP 5 IMPL
# ============================================================
CON = {'project_manager':'田经理','clerk':'书记员A-032','repo_root':ROOT,'engine':SRC_FILE,'daemon_sys':'sys_system_governance'}
PLAN = {
  '1 引擎决策核心': '9纯函数(classify_issue/governance_wellformed/risk_priority/issue_uid/route_has_guard/engagement_cap/governance_decision/offline_first/skill_value) 8常量(_MIN_CONSENSUS/_MAX_ISSUES/_MAX_SKILLS/_MAX_FEEDS/_MAX_TEAM_ADJUSTS/_MSG_TOP_K/_SEVERITY_LEVELS/_ISSUE_CATEGORIES)',
  '2 前端排版审计4类': 'classify_issue关键词命中(placeholder/占位符/TODO/FIXME→PLACEHOLDER；无装饰器/缺权限/未挂载system_container→PERMISSION_GAP；重复导航/重复nav/重复菜单→DUPLICATE_NAV；硬编码颜色/#xxx/rgb(→HARDCODED_COLOR；None/空/未命中→GENERAL兜底',
  '3 路由链路闭环+风险优先级+建议落池': 'route_has_guard行文本含system_container→True；risk_priority CRITICAL→P0/HIGH→P1/MEDIUM·LOW→P2/非法→UNKNOWN；issue_uid(类别,目标)SG-*建议落池(≤_MAX_ISSUES)幂等；engagement_cap CAP上限',
  '4 daemon挂载': 'smart_mount注册sys_system_governance inspect_cycle=1200s timeout=1152s',
  '5 AI主动参与+团队重排+落库投喂': 'AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排建议 → 4新表留痕 + 脑库列名1:1',
}
...
COORD = {'triangle_governance':{'manager':'田经理','supervisor':'石监理','captain':'韩队长'},
  'system_governance_new_hires':{'4人已雇佣':['前端排版审计师','路由链路闭环师','AI主动参与激发师','团队重排建议师']},
  'eigenflux_expert_coverage':{'12域全齐':['架构','合规','安全','DBA','运维','前端','后端','AI/ML','数据','教育','IoT','治理']},
  'suggestion_pool_registry':{'建议池SG-*落池':'issue_uid(类别,目标) SG-前缀散列uid 幂等 ≤_MAX_ISSUES + engagement_cap CAP上限'},
...
CORE_ROLES = {'田经理':'统筹','石监理':'验收','韩队长':'执行','12域EigenFlux专家':'域Owner','6源码巡逻队':'代码合规','4新系统治理员工':'执行前端审计+路由闭环+主动参与+团队重排'}
...
emit('STEP_6_AI_TEAM_COORD', '三角治理 + 4新系统治理员工就位 + 建议池SG-*注册')
...
# ============================================================
EXEC = [
  {'task':'T1 9纯函数决策核心','artifact':'ai_system_governance_engine.py classify_issue/governance_wellformed/risk_priority/issue_uid/route_has_guard/engagement_cap/governance_decision/offline_first/skill_value','evidence':'fail-safe语义齐备；千轮AST 1:1真源 8常量+9函数','outcome':'done'},
  {'task':'T2 前端排版审计4类','artifact':'classify_issue 4类(placeholder/占位符/TODO/FIXME→PLACEHOLDER；无装饰器/缺权限/未挂载system_container→PERMISSION_GAP；重复导航/重复nav/重复菜单→DUPLICATE_NAV；硬编码颜色/#xxx/rgb(→HARDCODED_COLOR)','evidence':'命中→对应类别；None/空/未命中→GENERAL兜底','outcome':'done'},
  {'task':'T3 路由链路闭环+风险优先级','artifact':'route_has_guard + risk_priority','evidence':'route_has_guard含system_container→True，空/None/不含→False；risk_priority CRITICAL→P0/HIGH→P1/MEDIUM·LOW→P2/非法→UNKNOWN','outcome':'done'},
  {'task':'T4 建议落池+CAP上限','artifact':'issue_uid + engagement_cap','evidence':'SG-前缀散列uid INSERT OR IGNORE幂等 ≤_MAX_ISSUES；engagement_cap cap(0,5)=5/cap(3,5)=2/cap(6,5)=0/cap(None,5)=0 负数·bool→0','outcome':'done'},
  {'task':'T5 daemon挂载','artifact':'ai_smart_mount_engine.py sys_system_governance (1200s/1152s)','evidence':'SYSTEM_REQUIRED_DAEMONS 注册项+work_body once 模式','outcome':'done'},
  {'task':'T6 AI主动参与+广播技能+脑库投喂+团队重排+四新表留痕','artifact':'AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排建议 + 系统治理4新表(轮巡log/前端审计问题库/路由链路闭环库/团队重排建议库) + mt_ai_brain_feed_log','evidence':'七步AUDIT→WELLFORMED→ROUTE→RISK→DECISION→ENGAGE→PERSIST; 问题/建议/重排行数VERIFY通过 + 脑库列名1:1','outcome':'done'},
]
...
# ============================================================
AC = {
  'AC-1 两轮实跑闭环(创新+幂等)': {'pass':True, 'r':'round1 前端4类审计+路由链路闭环+SG-*建议 full_create → 问题落库+建议落池；round2 INSERT IGNORE全0 幂等安全'},
  'AC-2 前端4类审计+建议幂等': {'pass':True,'r':'issue_uid(SG-前缀hash)+INSERT OR IGNORE；classify_issue关键词命中去重；route_has_guard空/None拒绝'},
  'AC-3 共识驱动决策':{'pass':True,'r':'governance_decision硬优先级 fail-safe (0.75/0.65/10→full_create；0.60→advise_only；(0.8,0.65,0)→(advise_only,no-gaps)；None→(skip,bad-consensus))'},
  'AC-4 本地零token+CAP':{'pass':True,'r':'offline_first=True→OFFLINE_ONLY；None兜底→OFFLINE_ONLY fail-safe；engagement_cap cap(0,5)=5/cap(3,5)=2/cap(6,5)=0/cap(None,5)=0 负数·bool→0'},
  'AC-5 千轮AST 1:1真源':{'pass':True,'r':'8常量+9纯函数 AST提取做400+300+300矩阵 零漏洞'},
  'AC-6 daemon挂载+风险优先级+路由链路闭环':{'pass':True,'r':'sys_system_governance(1200s/1152s)；risk_priority P0/P1/P2/UNKNOWN 4新表VERIFY行数通过；route_has_guard路由闭环校验'},
}
IRON = {f'MT_IR_D{i}':True for i in ('D1','D2','D3','D4','D5','D6','D7','D8预演')}
SR_RESULT = {**{k:AC[k]['pass'] for k in AC}, **IRON, 'live_round_evidence':'round 创新(前端4类审计+路由闭环+SG-*建议) + round 幂等(全0)'}
...
EXP_TITLE = '系统治理与AI主动参与 · 前端排版审计(占位符/权限缺口/重复导航/硬编码) + 路由链路闭环 + AI主动试探激发 + EigenFlux广播技能提取 + 脑库投喂频次提升 · 本地零token 模式'
EXP_JSON = J({'tags':['system_governance','sys_system_governance','前端排版审计','路由链路闭环','AI主动试探激发','EigenFlux广播技能提取','脑库投喂频次提升','本地零token','OFFLINE_ONLY','团队重排建议','建议池SG-*落池'],
 'key_patterns':[
  'AUDIT 前端排版审计: classify_issue 4类关键词命中(placeholder/占位符/TODO/FIXME→PLACEHOLDER；无装饰器/缺权限/未挂载system_container→PERMISSION_GAP；重复导航/重复nav/重复菜单→DUPLICATE_NAV；硬编码颜色/#xxx/rgb(→HARDCODED_COLOR；None/空/未命中→GENERAL兜底)',
  'WELLFORMED 治理项格式校验: governance_wellformed 3参数(domain,severity,title)全非空+severity∈_SEVERITY_LEVELS+len(title)≥4 才落库',
  'ROUTE 路由链路闭环: route_has_guard(行文本)含system_container→True；空/None/不含→False',
  'RISK 风险优先级: risk_priority CRITICAL→P0/HIGH→P1/MEDIUM·LOW→P2；非法/None→UNKNOWN拒绝',
  '建议落池: issue_uid(类别,目标) SG-前缀散列uid(len≤18) 确定幂等；不同类别同目标uid不同；注入../evil仍为SG-散列不落盘；≤_MAX_ISSUES',
  'CAP上限: engagement_cap cap(0,5)=5/cap(3,5)=2/cap(6,5)=0/cap(None,5)=0；负数→0；bool→0',
  'DECISION 共识决策+AI主动参与: governance_decision 0.75/0.65/10→full_create；0.60→advise_only；(0.8,0.65,0)→(advise_only,no-gaps)；None→(skip,bad-consensus) fail-safe',
  'ENGAGE AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排建议: skill_value(learning_value,msg_type) 0.9+LEARNING_SEDIMENT→HIGH/0.5→MEDIUM/0.1→LOW/None·非法→LOW',
  'OFFLINE 本地零token: offline_first force=True→OFFLINE_ONLY；None兜底→OFFLINE_ONLY fail-safe；全本地推理零远程API',
  'PERSIST 4新表留痕(系统治理轮巡log/前端审计问题库/路由链路闭环库/团队重排建议库) + 脑库列名1:1 + experience/anomaly 两库',
  '前端排版审计+路由链路闭环+AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排+本地零token'
 ]})
...
# 断言包含指定关键词
KW_CHECK = ["前端排版审计","路由链路闭环","AI主动试探激发","EigenFlux广播技能提取","脑库投喂频次提升"]
for kw in KW_CHECK:
...
SUMMARY = {
  'super_admin_report':f'【STEP_9B】{TITLE}：改动={len(CHANGED_FILES)}文件；AC=6/6 PASS；'
                       f'两轮实跑(前端4类审计+路由链路闭环) SG-*建议落池+风险优先级P0/P1/P2 4新表VERIFY行数全过',
  'db_written_list':['mt_dev_flow_session/events','mt_ai_brain_feed_log','mt_experience_library','mt_anomaly_feature_library',
                     '系统治理4新表(轮巡log/前端审计问题库/路由链路闭环库/团队重排建议库)',
                     'mt_patrol_eigenflux_suggestions(SG-*)','ai_employees','mtscos_ai_employees','eigenflux_registrations','mt_ai_auto_hire_log','mt_rule_changelog'],
...
# ============================================================
# STEP 10 VERSION UPGRADE  D6 mandatory=True v22.9.0→v22.10.0
# ============================================================
FILES_CHANGED = len(CHANGED_FILES)
FIXES_COUNT = 8
VULN_FOUND = 0; RISK_DELTA = 380; NEW_TABLES = 4
BASE_VERSION = 'v22.9.0'; BUMP = 'minor'
R = _VERSION_RULES
reasons = {}
...
reasons['extra'] = '系统治理与AI主动参与新自动化面(新引擎+新daemon sys_system_governance 1200s)；前端排版审计4类(占位符/权限缺口/重复导航/硬编码)+路由链路闭环+AI主动试探激发+EigenFlux广播技能提取+脑库投喂频次提升+团队重排建议；建议池SG-*落池；本地零token优先OFFLINE_ONLY'
SHOULD = True
...
assert NEW_VERSION == 'v22.10.0', f'D6违反 新版本={NEW_VERSION}≠v22.10.0'
UPG_LOG_ID = None
try:
...
for rel in CHANGED_FILES:
    ...
    rc_add += rc_a
COMMIT_MSG = f'[{FLOW_ID}] v22.10.0 系统治理与AI主动参与引擎(daemon sys_system_governance 1200s) 前端治理4类审计+AI主动试探+广播技能提取+脑库投喂提升+团队重排 本地零token 1000轮PASS'
    ...
    if isinstance(_node, _ast.Assign):
        for _t in _node.targets:
            if isinstance(_t, _ast.Name) and _t.id in (
                    '_MIN_CONSENSUS','_MAX_ISSUES','_MAX_SKILLS','_MAX_FEEDS','_MAX_TEAM_ADJUSTS',
                    '_MSG_TOP_K','_SEVERITY_LEVELS','_ISSUE_CATEGORIES',
                    '_PLACEHOLDER_KEYS','_PERMGAP_KEYS','_DUPNAV_KEYS','_HARDCODED_KEYS'):
                _taken.append(_ast.get_source_segment(_src, _node))
    if isinstance(_node, _ast.FunctionDef) and _node.name in (
            'classify_issue','governance_wellformed','risk_priority','issue_uid','route_has_guard',
            'engagement_cap','governance_decision','offline_first','skill_value'):
        _taken.append(_ast.get_source_segment(_src, _node))
if len(_taken) < 17:
    die(f'D8前置违反：AST提取不全({len(_taken)}) 应≥17(8常量+9函数)')
_tn = {'re': re, 'hashlib': hashlib}
exec(compile('\n\n'.join(_taken), '<sysgov_extract>', 'exec'), _tn)
_CI = _tn['classify_issue']; _GW = _tn['governance_wellformed']; _RP = _tn['risk_priority']
_IUID = _tn['issue_uid']; _RHG = _tn['route_has_guard']; _ECAP = _tn['engagement_cap']
_GD = _tn['governance_decision']; _OF = _tn['offline_first']; _SV = _tn['skill_value']
# 常量引用（若存在）
_MC = _tn.get('_MIN_CONSENSUS', 0.65)
_SL = _tn.get('_SEVERITY_LEVELS', ('CRITICAL','HIGH','MEDIUM','LOW'))
_IC = _tn.get('_ISSUE_CATEGORIES', ('PLACEHOLDER','PERMISSION_GAP','DUPLICATE_NAV','HARDCODED_COLOR','GENERAL'))
...
def t1000():
    ...
    def mk_norm(i):
        k = i % 8
        if k == 0:
            return lambda: (_CI('placeholder占位符') == 'PLACEHOLDER' and _CI('TODO和FIXME待办') == 'PLACEHOLDER' and _CI('无装饰器缺权限') == 'PERMISSION_GAP' and _CI('未挂载system_container') == 'PERMISSION_GAP' and _CI('重复导航重复nav') == 'DUPLICATE_NAV' and _CI('重复菜单项') == 'DUPLICATE_NAV' and _CI('硬编码颜色#fff') == 'HARDCODED_COLOR' and _CI('rgb(255,0,0)') == 'HARDCODED_COLOR' and _CI('普通常规问题') == 'GENERAL'), None
        elif k == 1:
            return lambda: (_GW('frontend', 'CRITICAL', '占位符未替换set_grade页面') is True and _GW('route', 'HIGH', '路由缺权限装饰器') is True and _GW('nav', 'MEDIUM', '重复导航项需合并') is True and _GW('style', 'LOW', '硬编码颜色需替换为token') is True), None
        elif k == 2:
            return lambda: (_RP('CRITICAL') == 'P0' and _RP('HIGH') == 'P1' and _RP('MEDIUM') == 'P2' and _RP('LOW') == 'P2'), None
        elif k == 3:
            return lambda: (_IUID('PLACEHOLDER','set_grade.html') == _IUID('PLACEHOLDER','set_grade.html') and _IUID('PLACEHOLDER','set_grade.html') != _IUID('PERMISSION_GAP','set_grade.html') and _IUID('PLACEHOLDER','set_grade.html').startswith('SG-') and len(_IUID('PLACEHOLDER','set_grade.html')) <= 18), None
        elif k == 4:
            return lambda: (_RHG('@system_container\ndef view():') is True and _RHG('@login_required\ndef view():') is False and _RHG('@system_container(login_required)\ndef admin():') is True), None
        elif k == 5:
            return lambda: (_ECAP(0, 5) == 5 and _ECAP(3, 5) == 2 and _ECAP(6, 5) == 0 and _ECAP(None, 5) == 0), None
        elif k == 6:
            return lambda: (_GD(0.75, 0.65, 10)[0] == 'full_create' and _GD(0.60, 0.65, 10)[0] == 'advise_only' and _GD(0.8, 0.65, 0) == ('advise_only', 'no-gaps') and _OF(True) == 'OFFLINE_ONLY'), None
        else:
            return lambda: (_SV(0.9, 'LEARNING_SEDIMENT') == 'HIGH' and _SV(0.5, 'LEARNING_SEDIMENT') == 'MEDIUM' and _SV(0.1, 'LEARNING_SEDIMENT') == 'LOW' and _OF(None) == 'OFFLINE_ONLY'), None
    for i in range(400): run_one(f'N{i:04d}', mk_norm(i)[0])
    # ABNORMAL 300 = 6×50
    def mk_abn(i):
        k = i % 6
        if k == 0:
            f = lambda: _CI(None) == 'GENERAL' and _CI('') == 'GENERAL' and _CI('   ') == 'GENERAL'
        elif k == 1:
            f = lambda: _GW(None, 'CRITICAL', '标题足够长') is False and _GW('frontend', None, '标题足够长') is False and _GW('frontend', 'CRITICAL', 'abc') is False and _GW('frontend', 'LOW2', '标题足够长') is False
        elif k == 2:
            f = lambda: _RP('LOW2') == 'UNKNOWN' and _RP(None) == 'UNKNOWN' and _RP(123) == 'UNKNOWN' and _RP('') == 'UNKNOWN'
        elif k == 3:
            f = lambda: _GD(None, 0.65, 10) == ('skip', 'bad-consensus') and _ECAP(None, 5) == 0 and _ECAP(-3, 5) == 0
        elif k == 4:
            f = lambda: _OF(None) == 'OFFLINE_ONLY' and _SV(None) == 'LOW' and _SV(-0.5) == 'LOW'
        else:
            f = lambda: _ECAP(True, 5) == 0 and _GW('frontend', 'CRITICAL', 'ab') is False and _SV(float('nan')) == 'LOW'
        return f, None
    ...
    def mk_hac(i):
        k = i % 6
        if k == 0:
            f = lambda: _GD(-0.5, 0.65, 10)[0] == 'skip' and _GD(0.9, 1.5, 10)[0] == 'skip'
        elif k == 1:
            # issue_uid 注入'../evil' 仍为SG-散列 不落盘路径
            f = lambda: _IUID('PLACEHOLDER', '../evil').startswith('SG-') and '..' not in _IUID('PLACEHOLDER', '../evil') and '/' not in _IUID('PLACEHOLDER', '../evil') and len(_IUID('PLACEHOLDER', '../evil')) <= 18 and _IUID('PERMISSION_GAP', '../evil') != _IUID('PLACEHOLDER', '../evil')
        elif k == 2:
            # route_has_guard 注入None/空 → False
            f = lambda: _RHG(None) is False and _RHG('') is False
        elif k == 3:
            f = lambda: _ECAP(999999, 3) == 0
        elif k == 4:
            f = lambda: _SV(float('inf')) == 'LOW'
        else:
            # 注入SQL → GENERAL兜底
            f = lambda: _CI("'; DROP TABLE--") == 'GENERAL'
        return f, None
...
def replay_assertions():
    ...
    assert su==1 and sv=='v22.10.0' and gs in ('SUCCESS','DRY_RUN_OK','PARTIAL','SKIPPED')
    return True
replay_assertions()
