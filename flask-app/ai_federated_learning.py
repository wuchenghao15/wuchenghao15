...
class AIFederatedLearning:
    ...
    def register_client(self, client_id: str, client_name: str = '',
        ...
        """注册客户端"""
        client_id = client_id or f"FL-C-{(100000 + secrets.randbelow(int(999999) - int(100000) + 1))}"
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
    ...
    def create_task(self, task_name: str, model_name: str = 'default_model',
        ...
        task_id = f"FL-T-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
