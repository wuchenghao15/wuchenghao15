...
class AIHostManager:
    ...
    def stop(self):
        ...
        with self.lock:
            ...
            self.status = "stopped"
            logger.info("AI深度托管整合系统已停止")
    
    def set_cluster_manager(self, cluster_manager):
        """设置集群管理器"""
        with self.lock:
            self.cluster_manager = cluster_manager
            self.distributed_enabled = cluster_manager is not None
            logger.info(f"集群管理器已设置，分布式模式: {self.distributed_enabled}")
    ...
    def check_ai_instance_health(self, instance_id: str) -> Dict:
        ...
        with self.lock:
            ...
            logger.info(f"AI实例健康检查: {instance_id} - {result['health_status']}")
            return result
    
    def check_all_ai_instances_health(self):
        """检查所有AI实例的健康状态"""
        with self.lock:
            logger.info("执行所有AI实例健康检查...")
            results = {}
            for instance_id in self.ai_instances:
                results[instance_id] = self.check_ai_instance_health(instance_id)
    ...
    def distribute_load(self, ai_type: str) -> Optional[str]:
        ...
        with self.lock:
            ...
            available_instances = [
                ...
                if instance["ai_type"] == ai_type and 
    ...
    def release_load(self, instance_id: str):
        ...
        with self.lock:
            ...
            if instance:
                ...
                logger.info(f"负载释放: 实例 {instance_id} 负载降至 {instance['load']}")
    ...
    def handle_instance_failure(self, instance_id: str):
        ...
        with self.lock:
            ...
            instance = self.ai_instances[instance_id]
            logger.error(f"AI实例故障: {instance_id} - {instance['name']}")
    ...
    def auto_scale_instances(self, ai_type: str, min_instances: int = 1, max_instances: int = 10):
        ...
        with self.lock:
            ...
            type_instances = [
                instance for instance in self.ai_instances.values()
                if instance["ai_type"] == ai_type and instance["status"] in ["running", "created"]
            ]
            ...
            # 伸缩策略: 当平均负载超过70%且未达到最大实例数时，增加实例
            ...
            # 伸缩策略: 当平均负载低于30%且实例数超过最小值时，减少实例
            elif avg_load < 30.0 and current_count > min_instances:
                # 选择负载最低的实例关闭
                idle_instances = [
                ...
                if idle_instances:
                    ...
                    self.stop_ai_instance(instance_to_remove["instance_id"])
    ...
    def call_ai_service(self, service_id: str, method: str, **kwargs) -> Dict:
        """调用AI服务"""
        with self.lock:
            if service_id not in self.services:
                return {"success": False, "error": f"服务不存在: {service_id}"}
        ...
        with self.lock:
            ...
            if self.distributed_enabled and self.cluster_manager:
                ...
                status_info.update({
                    ...
                    "current_master": cluster_status.get("current_master"),
    ...
    def distribute_service(self, service_id: str, node_id: str) -> bool:
        """将服务分发到指定节点"""
        with self.lock:
            if not self.distributed_enabled or not self.cluster_manager:
    ...
    def get_service_location(self, service_id: str) -> Optional[str]:
        ...
        with self.lock:
            service = self.services.get(service_id)
            if service and service.get("distributed"):
    ...
    def _auto_schedule_ai_instances(self):
        ...
        for instance_id, instance in self.ai_instances.items():
            if instance["status"] == "running" and not instance.get("assigned_node"):
                target_node = healthy_nodes[node_index % len(healthy_nodes)]
...
def main():
    ...
    # 调用AI服务
    result = ai_host.call_ai_service("ai_brain", "search_knowledge", tags=["AI", "管理"])
    ...
    # 查看最终状态
    status = ai_host.get_system_status()
    print(f"最终系统状态: {status['system_status']}")
    print(f"运行服务数: {status['running_services']}")
