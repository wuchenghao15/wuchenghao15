...
class AIEmployeeIntegrator:
    ...
    def check_ai_ensemble(self):
        ...
        try:
            ...
            self.log(f"AI集统计信息: {json.dumps(ensemble_stats, ensure_ascii=False)}")
            
            # 获取项目功能和所需AI类型
            self.log(f"项目功能: {json.dumps(ai_ensemble.project_features, ensure_ascii=False)}")
            self.log(f"所需AI类型: {json.dumps(ai_ensemble.required_ai_types, ensure_ascii=False)}")
            ...
            self.log(f"AI集状态检查失败: {str(e)}", "ERROR")
            return False
    ...
    def check_ai_instance_manager(self):
        ...
        try:
            ...
            self.log(f"AI实例统计信息: {json.dumps(instance_stats, ensure_ascii=False)}")
    ...
    def integrate_ai_employees(self):
        """整合AI员工到系统"""
        self.log("="*80)
        self.log("开始整合AI员工到系统")
        self.log("="*80)
        
        # 步骤1: 检查数据库表结构
        if not self.check_database_tables():
            self.log("数据库表结构检查失败，整合过程终止", "ERROR")
            return False
        ...
        try:
            ...
            # 优化题库
            from app.ai.test_generator import ai_test_generator
...
def main():
    ...
    integrator.integrate_ai_employees()
