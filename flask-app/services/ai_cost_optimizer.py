...
class AICostOptimizer:
    ...
    def record_cost(self, cost_type: str, amount: float, model_id: str = None,
        ...
        """记录成本"""
        record_id = f"COST-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('''
    ...
    def set_budget(self, budget_name: str, amount: float,
        ...
        budget_id = f"BUDGET-{(100000 + secrets.randbelow(int(999999) - int(100000) + 1))}"
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        in_tokens = (100 + secrets.randbelow(int(2000) - int(100) + 1))
