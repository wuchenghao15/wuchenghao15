...
class ClusterManager:
    ...
    def _check_nodes_health(self):
        ...
        with self.lock:
            ...
            for node_id, node in self.nodes.items():
                # 检查心跳是否超时（3倍心跳间隔）
                if current_time - node.get("timestamp", 0) > self.heartbeat_interval * 3:
                    logger.warning(f"节点 {node_id} 心跳超时，标记为不健康")
    ...
    def _broadcast_heartbeat(self, heartbeat: Dict[str, Any]):
        """广播心跳信息到其他节点"""
        # 简化实现，实际应使用更可靠的通信机制（如TCP/UDP广播或消息队列）
        pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(10):
            ...
            logger.info(f"集群状态: {json.dumps(status, indent=2)}")
            time.sleep(1)
    finally:
        cm.shutdown()