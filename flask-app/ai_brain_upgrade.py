...
class AIBrainUpgradeSystem:
    ...
    def start(self):
        ...
        if not self.running:
            ...
            for task in self.upgrade_tasks:
                ...
                time.sleep(1)  # 错开启动时间，避免资源竞争
            
            logger.info(f"✅ 已启动 {len(self.threads)} 个升级任务线程")
    ...
    def _crawl_ai_techniques(self):
        ...
        for technique in ai_techniques:
            cursor.execute('''
                INSERT INTO ai_techniques (name, category, description, implementation, source, url)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                technique["name"],
                technique["category"],
    ...
    def _organize_brain_knowledge(self):
        ...
        for knowledge in brain_knowledge:
            cursor.execute('''
                INSERT INTO ai_brain_knowledge (title, category, content, source, url)
                VALUES (?, ?, ?, ?, ?)
            ''', (
                knowledge["title"],
                knowledge["category"],
    ...
    def _crawl_success_cases(self):
        ...
        success_cases = [
            {
                ...
                "implementation": "使用Python、Flask、TensorFlow等技术栈实现，包含意图识别、实体提取、对话管理等模块。",
                "source": "成功案例库",
                "url": "#"
            },
            {
                "title": "AI驱动的推荐系统",
                "description": "基于协同过滤和深度学习技术，实现了个性化推荐功能，提高了用户点击率和转化率。",
            ...
            },
            {
                ...
                "implementation": "使用Python、PyTorch、Django等技术栈实现，包含图像预处理、特征提取、模型推理等模块。",
                "source": "成功案例库",
                "url": "#"
            }
        ]
        ...
        for case in success_cases:
            cursor.execute('''
                INSERT INTO success_cases (title, description, implementation, source, url)
                VALUES (?, ?, ?, ?, ?)
            ''', (
                case["title"],
                case["description"],
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("🛑 接收到中断信号，停止AI脑库升级系统")
