...
class ShadowSystem:
    ...
    def stop(self):
        ...
        logger.info(f"停止影子系统: {self.system_id}")
    ...
    def inject_input(self, input_data, input_id=None):
        """注入输入到影子系统"""
        if not self.is_running or self.status != ShadowSystemStatus.RUNNING:
            return
    ...
    def _auto_scale(self):
        ...
        # 检查是否需要缩容
        elif queue_size < self.config["scaling_threshold_low"] and current_threads > self.min_threads:
            # 缩容
    ...
    def _process_input(self, input_id, input_data):
        ...
        if self.mode == ShadowMode.MONITOR:
            ...
            # 仅记录，不处理
            logger.debug(f"记录模式: 输入已记录: {input_id}")
    ...
    def _compare_results(self, input_id, input_data, main_result, shadow_result):
        ...
        comparison = {
            ...
            ) if main_result and shadow_result else 0
        }
        
        if not match:
            logger.warning(f"结果不匹配: {input_id}, 差异: {differences}")
        else:
    ...
    def generate_report(self):
        ...
        with self.lock:
            ...
            avg_processing_time = sum(r["processing_time"] for r in self.comparison_results) / total if total > 0 else 0
...
def test_shadow_system():
    ...
    for i, input_data in enumerate(test_inputs):
        ...
        print(f"  注入输入 {i+1}: {input_data['type']}")
    ...
    print(f"  不匹配次数: {report['statistics']['mismatches']}")
