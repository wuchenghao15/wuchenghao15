...
class JavaScriptReplacementAI:
    ...
    def get_js_function(self, function_name):
        """获取JavaScript替代函数"""
        if function_name in self.js_functions:
            return self.js_functions[function_name]()
        else:
            logger.warning(f"未知的JavaScript函数: {function_name}")
...
class CompleteStartupAI:
    ...
    def initialize_system_mapping_service(self):
        """初始化系统映射服务"""
        ...
        try:
            ...
            # 保存映射到文件
            system_mapping_service.save_mappings_to_file()
    ...
    def start_flask_app(self):
        ...
        def get_js_ai_code(function_name):
            """获取JS AI代码"""
        ...
        def get_system_version():
            """获取系统版本信息"""
            return json.dumps(self.version_monitor.get_version_info())
        
        # 注册已初始化组件路由
        @app.route('/api/system/components')
        def get_initialized_components():
            """获取已初始化组件"""
            return json.dumps(self.ai_initializer.get_initialized_components())
