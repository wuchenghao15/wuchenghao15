...
def create_system_rules_table():
    try:
        ...
        default_rules = [
            ...
            # ========== 版本号管理规则 (VERSION_ 前缀) ==========
            ('VERSION_FORMAT', '版本号格式', 'MAJOR.MINOR.PATCH', 'version', '版本号采用3级语义化版本格式(如15.2.0)', 1),
            ('VERSION_MAJOR_MAX', '主版本号最大值', '99', 'version', 'MAJOR版本号上限(1-99)', 1),
            ('VERSION_MINOR_MAX', '次版本号最大值', '99', 'version', 'MINOR版本号上限(0-99)', 1),
            ('VERSION_PATCH_MAX', '修订号最大值', '999', 'version', 'PATCH版本号上限(0-999)', 1),
            ('VERSION_MAJOR_INCREMENT_RULE', '主版本号递增规则', '不兼容的API变更或重大架构重构时递增', 'version', 'MAJOR递增条件：破坏性API变更或重大架构重构(预计每年0-1次)', 1),
            ('VERSION_MINOR_INCREMENT_RULE', '次版本号递增规则', '新增功能且向下兼容时递增', 'version', 'MINOR递增条件：新增功能且向下兼容(每月最多1次)', 1),
            ('VERSION_PATCH_INCREMENT_RULE', '修订号递增规则', 'Bug修复或小优化时递增', 'version', 'PATCH递增条件：仅Bug修复或小优化，不得包含新功能(可多次)', 1),
            ('VERSION_PATTERN_REGEX', '版本号正则表达式', r'^(\d{1,2})\.(\d{1,2})\.(\d{1,3})$', 'version', '版本号格式验证正则表达式', 1),
            ('VERSION_FILE_PATH', '版本文件路径', 'flask-app/VERSION', 'version', '存储当前版本号的文件路径', 1),
            ('VERSION_MANAGER_FILE', '版本管理器文件', 'flask-app/version_manager.py', 'version', '版本管理器模块文件路径', 1),
            ('VERSION_CHANGELOG_FILE', '变更日志文件', 'docs/CHANGELOG.md', 'version', '版本变更日志文件路径', 1),
            ('VERSION_CURRENT', '当前版本号', '15.2.0', 'version', '系统当前运行版本号', 1),
            ('VERSION_HISTORY_TABLE', '版本历史表', 'version_history', 'version', '存储版本历史记录的数据库表名', 1),
            ('VERSION_HISTORY_RETENTION', '版本历史保留数量', '0', 'version', '版本历史保留条数(0=全部保留)', 1),
            ('VERSION_AUTO_VALIDATE', '版本号自动验证', '1', 'version', '发布前自动验证版本号格式是否合法', 1),
            ('VERSION_GIT_TAG_ENABLED', 'Git标签启用', '1', 'version', '版本发布时是否自动创建Git标签', 1),
            ('VERSION_GIT_TAG_FORMAT', 'Git标签格式', 'v{version}', 'version', 'Git标签命名格式(如v15.2.0)', 1),
            ('VERSION_CHANGELOG_REQUIRED', '变更日志必需', '1', 'version', '版本发布时必须更新CHANGELOG.md', 1),
            ('VERSION_CHANGELOG_FORMAT', '变更日志格式', '## v{version} (YYYY-MM-DD)', 'version', '变更日志条目格式', 1),
            ('VERSION_SYNC_MANAGER', '版本管理器同步', '1', 'version', '版本号变更时自动同步version_manager.py中的CURRENT_VERSION', 1),
            ('VERSION_DATABASE_PERSIST', '版本号数据库持久化', '1', 'version', '版本号变更时自动写入数据库version_history表', 1),
            ('VERSION_BUILD_NUMBER_ENABLED', '构建号启用', '1', 'version', '是否启用构建号(日期+序号格式)', 1),
            ('VERSION_BUILD_NUMBER_FORMAT', '构建号格式', 'YYYYMMDDa', 'version', '构建号格式(如20260717a)', 1),
            ('VERSION_CODENAME_ENABLED', '版本代号启用', '1', 'version', '是否为每个版本分配英文代号', 1),
            ('VERSION_DOWNGRADE_PROHIBITED', '禁止版本降级', '1', 'version', '禁止将版本号回退到低于当前版本', 1),
            ('VERSION_SKIP_PROHIBITED', '禁止跳版本', '0', 'version', '禁止跳过版本号(如从15.0.0直接到15.2.0)，0=允许跳过', 1),
            ('VERSION_RELEASE_BRANCH_ENABLED', '发布分支启用', '0', 'version', '是否启用版本发布分支(如release/v15.2.0)', 1),
            ('VERSION_PRE_RELEASE_ENABLED', '预发布版本启用', '1', 'version', '是否支持预发布版本标识(alpha/beta/rc)', 1),
            ('VERSION_PRE_RELEASE_FORMAT', '预发布版本格式', '{version}-{stage}.{number}', 'version', '预发布版本格式(如15.2.0-beta.1)', 1),
            ('VERSION_NOTIFICATION_ENABLED', '版本更新通知启用', '1', 'version', '版本更新时是否发送通知', 1),
            ...
            ('SELF_LEARNING_ENABLED', 'AI自我学习启用', '1', 'maintenance', '是否启用AI自我学习引擎', 1),
            ('SELF_LEARNING_INTERVAL', 'AI自我学习间隔', '3600', 'maintenance', 'AI自我学习执行间隔(秒)', 1),
            ('SELF_LEARNING_NETWORK_ENABLED', '网络学习启用', '1', 'maintenance', '是否启用网络知识自动采集', 1),
            ('SELF_LEARNING_AWARENESS_ENABLED', '自我觉醒启用', '1', 'maintenance', '是否启用自我觉醒分析', 1),
            ('SELF_LEARNING_RULES_ENABLED', '学习规则启用', '1', 'maintenance', '是否启用学习规则自动生成', 1),
            ('SELF_LEARNING_POLICY_ENABLED', '学习政策启用', '1', 'maintenance', '是否启用学习政策执行', 1),
            ('SELF_LEARNING_BRAIN_FEEDING_ENABLED', '脑库投喂启用', '1', 'maintenance', '是否启用自我学习脑库投喂', 1),
            ('SELF_LEARNING_MIN_CONFIDENCE', '学习置信度阈值', '0.3', 'maintenance', '生成学习规则的最低置信度', 1),
            ('SELF_LEARNING_MAX_INSIGHTS', '最大洞察数', '50', 'maintenance', '每次自我觉醒最大发现洞察数', 1),
            ('SELF_LEARNING_MAX_RULES', '最大规则数', '20', 'maintenance', '每次生成的最大学习规则数', 1),
            ('SELF_LEARNING_RECORDS_RETENTION_DAYS', '学习记录保留天数', '90', 'maintenance', '学习记录在数据库中的保留天数', 1),
            ...
            ('GIT_SYNC_RETRY_DELAY', '同步重试延迟', '60', 'git', '同步重试间隔(秒)', 1),
