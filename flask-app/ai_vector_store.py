...
def text_to_word_vector(text: str, vocab: Optional[Dict[str, int]] = None,
    ...
    vec = [0.0] * dim
    for token in tokens:
        h = int(hashlib.sha256(token.encode('utf-8')).hexdigest(), 16) % dim
        vec[h] += 1.0
    # 归一化
    norm = math.sqrt(sum(v * v for v in vec))
    if norm > 0:
...
def text_to_hash_vector(text: str, dim: int = 256) -> List[float]:
    ...
    for ng in ngrams:
        h = int(hashlib.sha256(ng.encode('utf-8')).hexdigest(), 16) % dim
...
class AIVectorStore:
    ...
    def create_collection(self, name: str, dim: int = None, metric: str = None,
        ...
        collection_id = f"COL-{name}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
    ...
    def add(self, collection_id: str, vector: List[float], document: str = '',
        ...
        item_id = item_id or f"ITEM-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
