...
class ProjectMonitor:
    ...
    def stop_flask_app(self):
        ...
        if self.flask_process:
            print("\n" + "="*60)
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 正在停止Flask应用...")
            print("="*60 + "\n")
    ...
    def monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                print(f"  AI类型分布: {json.dumps(ai_status['instance_types'], ensure_ascii=False)}")
                print(f"  活跃AI实例列表: {json.dumps([ai['name'] for ai in ai_status['active_instances_list']], ensure_ascii=False)}")
                
                # 获取并打印功能状态
                print("\n2. 功能状态:")
                print("-" * 40)
                feature_status = self.get_feature_status()
                print(f"  项目功能: {json.dumps(feature_status['project_features'], ensure_ascii=False)}")
                print(f"  所需AI类型: {json.dumps(feature_status['required_ai_types'], ensure_ascii=False)}")
                print(f"  AI集统计: {json.dumps(feature_status['ensemble_stats'], ensure_ascii=False)}")
                ...
                print(f"  角色分布: {json.dumps(user_status['role_distribution'], ensure_ascii=False)}")
                print(f"  活跃用户: {json.dumps(user_status['active_users'], ensure_ascii=False)}")
                
                # 打印监控结束
                print("\n" + "="*60)
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 监控周期结束")
                print("="*60)
            except Exception as e:
