...
def register_all_blueprints(app):
    ...
    # 仙女座星系子系统 (v23.0.0 新增 — 抖音阶梯课程内容变现 + C1~C7 合规)
        ...
        try:
            ...
            try:
                ...
                if ('already been registered' in msg
                    or 'has already been registered' in msg
                    or 'overwriting an existing endpoint' in msg
                    or 'is not callable' in msg
                    or 'url_prefix' in msg):
                    # endpoint 覆盖 / 重复注册 / 非 callable → 跳过 (蓝图内部可能有重复, 但已注册的 route 仍然有效)
                    skipped += 1
                else:
