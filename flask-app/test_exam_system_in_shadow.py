...
class ShadowExamSystemTester:
    ...
    def create_shadow_exam_environment(self) -> Dict[str, Any]:
        """创建影子考试环境"""
        try:
            logger.info("开始创建影子考试环境")
            
            # 生成影子ID
            shadow_id = f"SHADOW_EXAM_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    def run_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.check_database():
                ...
                test_result['errors'].append('数据库检查失败')
            ...
            if report:
                ...
                test_result['errors'].append('测试报告生成失败')
                test_result['success'] = False
...
def main():
    ...
    if test_result.get('test_id'):
        logger.info(f"\n3. 测试报告")
        report_path = os.path.join(tester.exam_dir, f"{test_result['test_id']}_report.json")
