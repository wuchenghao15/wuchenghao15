...
def test_ai_request_classifier():
    ...
    try:
        from app.middlewares.ai_request_classifier import ai_request_classifier
        ...
        priority = ai_request_classifier._calculate_priority(test_request, category)
        
        print(f"✓ 分类结果: {category}")
        print(f"✓ 优先级: {priority}")
        print("✓ AI请求分类和优先级功能测试通过")
        return True
    except Exception as e:
...
def test_ai_smart_cache():
    ...
    try:
        from app.middlewares.ai_smart_cache import ai_smart_cache
...
def test_ai_middleware_learning():
    ...
    try:
        from app.middlewares.ai_middleware_learning import AIMiddlewareLearningSystem
        
        # 测试初始化
        ai_learning = AIMiddlewareLearningSystem()
        print("✓ AI中间件学习系统初始化成功")
...
def test_ai_middleware_optimizer():
    ...
    try:
        from app.middlewares.ai_middleware_optimizer import AIMiddlewareOptimizer
...
def test_middleware_registration():
    ...
    try:
        from app.middlewares import init_middlewares, middleware_manager
        
        # 初始化中间件
        init_middlewares()
        
        # 获取已注册的中间件
        registered_middlewares = middleware_manager.priority_list
...
def test_flask_integration():
    ...
    try:
        ...
        @app.route('/test')
        def test_route():
            return json.dumps({'message': 'Test successful'}), 200, {'Content-Type': 'application/json'}
