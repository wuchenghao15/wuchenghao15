...
class RealTimeServerMonitor:
            ...
            if hasattr(psutil, 'sensors_temperatures'):
                ...
                for name, entries in temps.items():
                    for entry in entries:
                        system_temperature[f"{name}_{entry.label or 'temp'}"] = entry.current
        except:
            pass
    ...
    def _detect_exceptions(self, stats):
        ...
        # 检查负载平均值
        if stats["load_average"][0] > self.thresholds["load_average"]:
            exceptions.append({
        ...
        if stats["connections"] > self.thresholds["connections"]:
            exceptions.append({
                ...
                "level": "warning",
                "description": f"网络连接数过多: {stats['connections']}",
        ...
        for service in critical_services:
            if service not in stats["running_services"]:
                exceptions.append({
                    "type": "service_down",
                    "level": "critical",
                    "description": f"关键服务 {service} 未运行",
    ...
    def _handle_exceptions(self, exceptions, server_stats):
        ...
        for exception in exceptions:
            try:
                ...
                exception["server_stats"] = server_stats
...
# 全局实时服务器监控实例
real_time_server_monitor = RealTimeServerMonitor()
