...
def save_config_and_verify():
    ...
    try:
        ...
        logger.info("\n验证服务器配置...")
        config = load_config()
        logger.info(f"当前服务器配置:\n  版本: {config.get('VERSION')}\n  主机: {config.get('SERVER_HOST')}\n  端口: {config.get('SERVER_PORT')}\n  协议: {config.get('PROTOCOL')}\n  环境: {config.get('ENV')}\n  DEBUG: {config.get('DEBUG')}")
...
def main():
    """主函数"""
    logger.info("=== MTSCOS AI Server 状态检查与配置保存 ===")
