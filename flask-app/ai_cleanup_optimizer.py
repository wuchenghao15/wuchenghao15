...
class AICleanupOptimizer:
    ...
    def __init__(self):
        ...
        self.cleanup_targets = {
            ...
            'temporary_directories': {
                'patterns': ['temp', 'tmp', 'cache', '__pycache__'],
                'directories': [self.project_root, self.data_dir],
                'max_age_days': 1
            }
        }
        ...
        self.redundant_configs = {
            'version_system_configs': [
                ...
                'version_general_retention_period'
            ],
            'documentation_system_configs': [
                'documentation_general_compression',
                'documentation_general_retention_period'
            ],
            'ai_system_configs': [
    ...
    def get_file_age_days(self, file_path: str) -> float:
        ...
        try:
            ...
            now = datetime.now()
            age_days = (now - file_mtime).total_seconds() / (24 * 3600)
            return age_days
    ...
    def cleanup_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cleanup_result['total_files_deleted'] = sum(cleanup_result['files_deleted_by_type'].values())
        ...
        try:
            ...
            for dir_path in necessary_dirs:
                try:
                    ...
                    optimize_result['directories_organized'] += 1
    ...
    def run_cleanup_and_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_result['steps'].append(f"清理文件完成，删除 {file_cleanup_result['total_files_deleted']} 个文件")
            ...
            overall_result['steps'].append(f"清理冗余配置完成，删除 {config_cleanup_result['total_configs_deleted']} 个配置")
