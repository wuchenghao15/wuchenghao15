...
def create_slave_server():
    ...
    slave_id = ai_server_cluster_manager.create_slave_server(name, ip, port, master_id)
    if not slave_id:
        return jsonify({'error': 'Invalid master ID'}), 400
    
    return jsonify({'slave_id': slave_id}), 201

@ai_management_api.route('/api/ai/servers/<node_id>', methods=['GET'])
def get_server(node_id):
    """获取服务器详情"""
...
def distribute_task():
    ...
    if node_id:
        return jsonify({'node_id': node_id, 'message': 'Task distributed'}), 200
    else:
        return jsonify({'error': 'Failed to distribute task'}), 500

# ------------------------------
# AI Collection Management Endpoints
# ------------------------------
...
def remove_employee_from_collection(collection_id, employee_id):
    ...
    return jsonify({'message': 'Employee removed from collection'}), 200
...
# ------------------------------
# AI Supervision and Upgrade Endpoints
# ------------------------------
...
def get_alerts():
    """获取所有活跃警报"""
    return jsonify(ai_supervision_manager.get_active_alerts())
...
def get_employee_collections(employee_id):
    """获取AI员工所属的所有集合"""
    collections = ai_collection_manager.get_employee_collections(employee_id)
...
def initialize_ai_management():
    ...
    # 创建默认主服务器
    master_id = ai_server_cluster_manager.create_master_server(
        name='Default Master Server',
        ip='127.0.0.1',
        port=5001
    )
