        ...
        for i, ln in enumerate(lines, 1):
            ...
            if b % 2 != 0:
                issues.append({
                    ...
                    'msg': '检测到未闭合的粗体标记 **，红色波浪线提示级别（建议补齐成对）',
        ...
        for i, ln in enumerate(lines, 1):
            if re.search(r"['\"]\s*\.\s*\$[a-zA-Z_]|\{[a-zA-Z_][\w]*\}\s*['\"]|\$_(GET|POST|REQUEST|SERVER)\[", ln):
                issues.append({
                    ...
                    'msg': '疑似SQL注入风险：检测到变量拼接（建议使用参数化查询/预处理语句）',
...
class AIInspectionLoopEngine:
    ...
    def _classify_console_error(cls, error_data: Dict[str, Any]) -> Tuple[str, str, str]:
        ...
        if not suggestion and ('Failed to load' in msg or 'LoadFailed' in msg or
            ...
            severity = 'high'
            suggestion = '【红色波浪线：资源加载失败】请检查静态资源路径/CDN/后端接口可达性，常见原因为：404、CORS、HTTPS混合内容'
    ...
    def report_console_error(self, error_data: Dict[str, Any]) -> str:
        ...
        error_id = f"CE-{datetime.now().strftime('%Y%m%d%H%M%S')}-{hashlib.sha256(json.dumps(error_data, sort_keys=True).encode()).hexdigest()[:8]}"
    ...
    def _scan_syntax_errors(self) -> Tuple[List[Dict], int]:
        ...
        skip_dirs = {'__pycache__', '.git', 'node_modules', 'venv', '.venv', 'flask-app-old', 'logs', 'data',
                     ...
                     '_archive', '_archive_old', 'recovery_snapshots', 'Manifests'}
        ...
        for m in re.finditer(r'style\s*=\s*(["\'])((?:\\.|(?!\1).)*)\1', content, re.DOTALL | re.IGNORECASE):
            ...
            if not prop_ok:
                ...
                all_issues.append({
                    ...
                    'msg': '内联 style 属性缺少「:」或「属性值为空」（可能触发 IDE CSS 红色波浪线：需要冒号/预期有标识符）',
            ...
            try:
                ...
                for line in lines:
                    for pattern, severity in error_patterns:
                        ...
                        if m:
                            msg = m.group(0)[:200]
                            msg_hash = hashlib.sha256(msg[:80].encode('utf-8')).hexdigest()
        ...
        try:
            with _get_conn() as conn:
                ...
                for row in rows:
                    ...
                    pattern_hash = hashlib.sha256(pattern_hash.encode('utf-8')).hexdigest()
