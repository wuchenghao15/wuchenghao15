...
def init_firewall_tables():
    ...
    try:
        with _conn() as conn:
            ...
            # 迁移：security_middleware 可能已创建了简化版 security_events_log，补齐缺失列
            existing_cols = {row[1] for row in conn.execute(f"PRAGMA table_info({EVENT_TABLE})").fetchall()}
            for col_def in [
                ('rule_code', 'TEXT'),
                ('user_agent', 'TEXT'),
                ('request_path', 'TEXT'),
                ...
                if col_def[0] not in existing_cols:
                    try:
                        conn.execute(f'ALTER TABLE {EVENT_TABLE} ADD COLUMN {col_def[0]} {col_def[1]}')
                    except Exception:
                        pass
            conn.commit()
        return True
    ...
    try:
        if RULE_CACHE['rules']:
            ...
            if global_rule and request_obj:
                # 每 64 条写一条（避免全局审计写爆）
                bk = f"GLOBAL_AUDIT_{datetime.now().minute // 2}_{hashlib.sha256((data['ip'] or data['ua']).encode()).hexdigest()[:4]}"
        ...
        with _conn() as conn:
            if rule_id:
                ...
                if not data.get('rule_code'):
                    data['rule_code'] = 'CUST_' + hashlib.sha256((data.get('name') or '') + now).hexdigest()[:10].upper()
                cols = ','.join(data.keys())
                phs = ','.join(['?'] * len(data))
