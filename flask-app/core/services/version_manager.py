...
def _resolve_current_version(_fallback='22.2.0'):
    """系统版本号单一权威源: 一律从 core.version_source(flask-app/VERSION) 派生, 禁止硬编码。"""
    try:
        from core.version_source import get_system_version
    ...
    try:
        ...
        for _ in range(7):
            if os.path.exists(os.path.join(_d, 'VERSION')) and os.path.isdir(os.path.join(_d, 'core')):
