...
class ExamGenerator:
    ...
    def _generate_spelling_distractor(self, correct_word, idx=0):
        ...
        if len(correct_word) <= 3:
            ...
            import random, secrets
            # 生成一个不同的字母
            new_char = chr((97 + secrets.randbelow(int(122) - int(97) + 1)))
    ...
    def _generate_synonym_distractor(self, correct_synonym):
        ...
        if correct_synonym.lower() in synonym_dict:
            # 从同义词列表中选择一个不同的词
            return secrets.choice([s for s in synonyms if s != correct_synonym.lower()])
    ...
    def update_listening_question_based_on_feedback(self, question_id, feedback):
        ...
        try:
            ...
            improvement_result = {'success': False, 'result': ''}

            if improvement_result['success']:
                improved_content = improvement_result['result']
                logger.info("成功生成改进后的听力题内容")
                # 根据反馈调整难度
    ...
    def _generate_audio_content(self, subject, difficulty):
        ...
        if subject == 'english':
            # 随机选择场景
            scenario = secrets.choice(scenarios[subject])
    ...
    def _generate_single_choice_comprehension_question(self, audio_content, subject, difficulty, index):
        ...
        if subject == 'english':
            questions = [
                "What did Speaker A offer to Speaker B?",
                "Where is the nearest subway station located?",
                "What did the customer order for dinner?",
                "Where was the new butterfly species discovered?",
                "What is cognitive dissonance?",
                "How long has the speaker been studying English?",
                "What is the main topic of the lecture?",
                "Who discovered the new species?",
                "What is the effect of the new policy?",
                "What does the speaker suggest doing?",
                "What time does the meeting start?",
                "Where will the event take place?",
                "Why did the speaker miss the deadline?",
                "What does the term 'AI' stand for?",
                "What is the speaker's opinion on climate change?", "How many people attended the conference?",
            ]
            options_list = [
    ...
    def _generate_fill_in_blank_comprehension_question(self, audio_content, subject, difficulty, index):
        ...
        if subject == 'english':
            questions = [
                "The subway station is on the ________ side of the street.",
                "The customer ordered ________ with rice.",
                "Sales increased by ________ percent compared to last quarter.",
                "The meeting will start at ________ AM."
            ]
            correct_answers = ["right", "salmon", "15", "10"]
        else:
            questions = [
                "地下鉄の駅は通りの________側にあります.",
                "売上高は先季度と比較して________%増加しました.",
                "会議は________時に始まります.",
                "顧客は________を注文しました."
            ]
    ...
    def _generate_short_answer_comprehension_question(self, audio_content, subject, difficulty, index):
        ...
        if subject == 'english':
            questions = [
                "What did Speaker A offer to Speaker B?",
                "What was the main reason for the sales increase?",
                "Where was the new butterfly species discovered?"
            ]
            correct_answers = [
    ...
    def generate_practice_exam(self, user_preferences):
        ...
        # 获取用户等级
        user_level = self.get_user_level(user_preferences['username'], user_preferences.get('subject', 'english'))
