...
class VPNServer:
    ...
    def __init__(self, host: str='127.0.0.1', port: int=51820):
        self.host = host
        self.port = port
        ...
        self.traffic_stats = {'total_bytes_sent': 0, 'total_bytes_received': 0, 'total_connections': 0, 'current_connections': 0, 'peak_connections': 0, 'start_time': datetime.now(), 'connections_history': [], 'traffic_history': []}
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if os.path.exists(config_path):
            with open(config_path, 'r', encoding='utf-8') as f:
                return json.load(f)
        return {'server': {'host': '127.0.0.1', 'port': 51820, 'max_clients': 10, 'connection_timeout': 300, 'max_login_attempts': 5, 'login_lockout_time': 60}, 'ssl': {'enabled': True, 'cert_file': 'vpn_server.crt', 'key_file': 'vpn_server.key'}, 'auth': {'enabled': True, 'users': {'mtscos_admin': 'MTSCOS_VPN_2026', 'admin': 'mtscos_vpn_2026'}}, 'firewall': {'enabled': True, 'whitelist': [], 'blacklist': [], 'allowed_ports': [80, 443, 22, 3389, 5432, 3306], 'blocked_ports': []}, 'logging': {'enabled': True, 'log_file': 'vpn_server.log', 'log_level': 'INFO'}, 'api': {'enabled': True, 'host': '127.0.0.1', 'port': 51821, 'api_key': 'MTSCOS_VPN_API_KEY_2026'}, 'traffic': {'enabled': True, 'limit_per_user': 1073741824}}
    ...
    def _generate_ssl_cert(self):
        ...
        try:
            ...
            subject = issuer = x509.Name([x509.NameAttribute(NameOID.COUNTRY_NAME, 'CN'), x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, 'Beijing'), x509.NameAttribute(NameOID.LOCALITY_NAME, 'Beijing'), x509.NameAttribute(NameOID.ORGANIZATION_NAME, 'MTSCOS AI'), x509.NameAttribute(NameOID.COMMON_NAME, 'mtscos-vpn')])
            cert = x509.CertificateBuilder().subject_name(subject).issuer_name(issuer).public_key(public_key).serial_number(x509.random_serial_number()).not_valid_before(datetime.datetime.utcnow()).not_valid_after(datetime.datetime.utcnow() + datetime.timedelta(days=365)).sign(private_key, hashes.SHA256(), default_backend())
            ...
            logger('[VPN] SSL证书生成成功')
            return True
        except ImportError:
            logger('[VPN] 未安装cryptography库，跳过SSL证书生成')
            return False
    ...
    def _check_traffic_limit(self, client_id: int) -> bool:
        ...
        with self.lock:
            if client_id in self.clients:
                total_traffic = self.clients[client_id].get('bytes_sent', 0) + self.clients[client_id].get('bytes_received', 0)
    ...
    def _update_stats(self, sent_bytes: int=0, received_bytes: int=0, connected: bool=False, disconnected: bool=False):
        ...
        with self.stats_lock:
            ...
            if sent_bytes > 0 or received_bytes > 0:
                traffic_record = {'time': datetime.now().isoformat(), 'bytes_sent': sent_bytes, 'bytes_received': received_bytes}
            ...
            if connected:
                ...
                record = {'time': datetime.now().isoformat(), 'event': 'connected'}
            ...
            if disconnected:
                self.traffic_stats['current_connections'] -= 1
                record = {'time': datetime.now().isoformat(), 'event': 'disconnected'}
                self.traffic_stats['connections_history'].append(record)
                if len(self.traffic_stats['connections_history']) > 100:
                    self.traffic_stats['connections_history'] = self.traffic_stats['connections_history'][-100:]
                        ...
                        if header_str.startswith('AUTH') and (not authenticated):
                            ...
                            if len(parts) >= 3:
                                ...
                                if self._authenticate(username, password):
                                    ...
                                    with self.lock:
                                        self.clients[client_id] = {'ip': client_ip, 'username': username, 'connected_at': datetime.now(), 'bytes_sent': 0, 'bytes_received': 0, 'last_activity': time.time()}
                            ...
                            if authenticated:
                                ...
                                self._log('[VPN] 未认证的CONNECT请求', 'WARNING')
                        buffer = rest
        ...
        def forward(source, destination, direction: str):
            while True:
                try:
                    ...
                    byte_count = len(data)
                    self._update_stats(sent_bytes=byte_count if direction == 'out' else 0, received_bytes=byte_count if direction == 'in' else 0)
                    with self.lock:
                        if client_id in self.clients:
                            if direction == 'out':
                ...
                try:
                    ...
                    with self.lock:
                        if len(self.clients) >= self.config['server']['max_clients']:
                            self._log('[VPN] 达到最大连接数限制', 'WARNING')
                            client_socket.sendall(b'HTTP/1.1 503 Service Unavailable\r\n\r\n')
                            client_socket.close()
                            continue
    ...
    def _start_api_server(self):
        ...
        class APIHandler(BaseHTTPRequestHandler):
            ...
            def _handle_status(self):
                status = {'status': 'running' if self.server_instance.is_running else 'stopped', 'port': self.server_instance.port, 'ssl_enabled': self.server_instance.config['ssl']['enabled'], 'auth_enabled': self.server_instance.config['auth']['enabled'], 'max_clients': self.server_instance.config['server']['max_clients'], 'current_clients': len(self.server_instance.clients)}
            ...
            def _handle_clients(self):
                with self.server_instance.lock:
                    ...
                    for (client_id, info) in self.server_instance.clients.items():
                        clients.append({'id': client_id, 'ip': info['ip'], 'username': info['username'], 'connected_at': info['connected_at'].isoformat(), 'bytes_sent': info['bytes_sent'], 'bytes_received': info['bytes_received']})
            ...
            def log_message(self, format, *args):
                return {'status': 'success', 'code': 0, 'message': 'auto_implemented_by_gap_engine', 'gap_flow_id': 'autogap_2c9ad859_20260827_002'}
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {'status': 'running' if self.is_running else 'stopped', 'port': self.port, 'ssl_enabled': self.config['ssl']['enabled'], 'auth_enabled': self.config['auth']['enabled'], 'max_clients': self.config['server']['max_clients'], 'current_clients': len(self.clients), 'connection_states': connection_states_copy, 'stats': self.traffic_stats}
