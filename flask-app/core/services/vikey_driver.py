...
def _safe_platform_system() -> str:
    """
    安全获取 OS 名称。处理 platform 模块被某处污染导致 AttributeError 的情况。
    优先级：platform.system() -> sys.platform 推断 -> 'Unknown'
    """
    try:
        # 重新获取一次标准库 platform，避免被 import 污染
        import importlib
        plat_mod = importlib.import_module('platform')
        if hasattr(plat_mod, 'system'):
            return str(plat_mod.system())
    except Exception:
        pass
    # 降级：用 sys.platform 推断
    try:
...
# ==========================================================
#  HardwareKeyProvider — 硬件密钥统一抽象层
#  （方向3 阶段1 · 韩队长+EF_分布式 · flow_super_admin_remote_login_hotfix_20260817_001）
#
#  收敛 3 套并行模块（CODE_STUDY_NOTES.md 不一致点 U1/U2 修复）:
#    1. services/vikey_detector.py VikeyDetector（auth_routes 调用，VID/PID 匹配）
#    2. core/services/vikey_driver.py VikeyDriverManager（ai_vikey_security_employee 调用，多后端架构）
#    3. core/services/szu100_driver.py detect_szu100（独立 SZU100 检测，多维校验+CD-ROM）
#
#  融合专家意见:
#    - 张晓峰综1: 双密钥原子校验 threading.Lock + 5s timeout
#    - 张晓峰异议2: ≤5s 95 分位软阈值 + 超时降级非拒绝
#    - EF_分布式: 跨平台策略模式 Darwin/Windows/Linux
#    - 王总工: USB 设备指纹 3 维校验（VID/PID + 序列号白名单 + 固件版本哈希）
#    - 陈博士: CAP 选 CP 强一致性
#
#  约束合规:
#    - G1 不引入新数据库表（纯检测，无 DB 写入）
#    - G2 不引入新依赖（stdlib threading + subprocess）
#    - G4 文件变更数 ≤15（在 vikey_driver.py MODIFY 范围内，不新增文件）
# ==========================================================


_dual_key_probe_lock = threading.Lock()


class HardwareKeyProvider(ABC):
    """硬件密钥统一抽象层（VIKEY + SZU100 双密钥原子校验）

    张晓峰综1: 双密钥原子校验实施细节（threading.Lock + 检测顺序 + 禁降级密码）
    张晓峰异议2: ≤5s 95 分位软阈值 + 超时降级到"密钥未就绪"提示（非拒绝）
    """
    ...
    @abstractmethod
    def detect_szu100(self) -> bool:
        """检测 SZU100 专用 U盘是否连接"""
        ...

    def verify_dual_key_atomic(self, timeout: float = 5.0) -> Tuple[bool, str]:
        """双密钥原子校验（threading.Lock + 超时降级）
        ...
        返回:
            (True, 'ok')              — 双密钥同时在线
            (False, 'vikey_missing')  — VIKEY 缺失
            (False, 'szu100_missing') — SZU100 缺失
            (False, 'both_missing')   — 双密钥均缺失
            (False, 'timeout_degrade') — 超时降级（非拒绝，提示重试）
        """
        timeout = max(0.1, min(float(timeout), 15.0))
        result_holder: Dict[str, Any] = {'vikey_ok': False, 'szu100_ok': False}

        def _detect_vikey():
            try:
                result_holder['vikey_ok'] = self.detect_vikey()
            except Exception as e:
                logger.warning(f"[HardwareKeyProvider] VIKEY 检测异常: {e}")
                result_holder['vikey_ok'] = False

        def _detect_szu100():
            try:
                result_holder['szu100_ok'] = self.detect_szu100()
            except Exception as e:
                logger.warning(f"[HardwareKeyProvider] SZU100 检测异常: {e}")
                result_holder['szu100_ok'] = False

        # 原子校验：threading.Lock 保证并发安全（张晓峰综1）
        with _dual_key_probe_lock:
            started_at = time.monotonic()
            t1 = threading.Thread(target=_detect_vikey, daemon=True, name="hk-vikey")
            t2 = threading.Thread(target=_detect_szu100, daemon=True, name="hk-szu100")
            t1.start()
            t2.start()
            ...
            if t1.is_alive() or t2.is_alive():
                logger.warning(f"[HardwareKeyProvider] 双密钥检测超时(>{timeout}s)，降级提示")
                # 落超时事件到 activity_logs.db（方向3 阶段3）
                try:
                    import sqlite3 as _sqlite3
                    from core.db_path import get_db_path
                    _db = get_db_path('app.db')
                    _conn = _sqlite3.connect(_db, timeout=3)
                    try:
                        _conn.execute(
                            "INSERT INTO activity_logs(timestamp, event_type, detail) VALUES(?,?,?)",
                            (datetime.now().isoformat(), 'hardware_key_timeout',
                             f"dual_key_timeout>{timeout}s")
                        )
                        _conn.commit()
                    finally:
                        _conn.close()
                    ...
                    pass  # 超时事件落库失败不阻塞主流程
                return (False, 'timeout_degrade')

            vikey_ok = result_holder['vikey_ok']
            szu100_ok = result_holder['szu100_ok']

            if vikey_ok and szu100_ok:
                return (True, 'ok')
            elif not vikey_ok and not szu100_ok:
                return (False, 'both_missing')
            elif not vikey_ok:
                return (False, 'vikey_missing')
            else:
                return (False, 'szu100_missing')

    def get_dual_auth_state(self, timeout: float = 5.0) -> Dict[str, Any]:
        """Return a safe, non-secret snapshot for session renewal and auditing."""
        checked_at = time.time()
        verified, reason = self.verify_dual_key_atomic(timeout=timeout)
        return {
            'verified': verified,
            'reason': reason,
    ...
    def get_device_fingerprint(self) -> Dict[str, Any]:
        ...
        """
        return {'vid': '', 'pid': '', 'serial': '', 'firmware_hash': '', 'is_authentic': False}
...
class DarwinHardwareKeyProvider(HardwareKeyProvider):
    ...
    def detect_szu100(self) -> bool:
        """SZU100 检测：委托 szu100_driver.detect_szu100()（更强多维校验）

        修复 D4.1/D5.2: auth_routes 之前调用 vikey_detector.is_szu100_connected()
        仅卷标匹配；现委托 szu100_driver 的 VID/PID+制造商+CD-ROM+容量+认证文件 6 维校验
        """
        try:
            from core.services.szu100_driver import detect_szu100
            result = detect_szu100()
            return bool(result.get('is_authentic', False))
        except ImportError:
            # 兜底: 委托 services/vikey_detector（较弱卷标匹配）
            try:
                from services.vikey_detector import get_detector
                return get_detector().is_szu100_connected()
            except ImportError:
                logger.warning("[DarwinProvider] szu100_driver 不可用，降级卷标匹配")
                return False
        except Exception as e:
            logger.warning(f"[DarwinProvider] SZU100 检测失败: {e}")
            return False

    def get_device_fingerprint(self) -> Dict[str, Any]:
        """macOS USB 设备指纹 3 维校验（王总工）"""
        try:
            import subprocess as _sp
            result = _sp.run(
                ["system_profiler", "SPUSBDataType", "-json"],
            ...
            if result.returncode != 0:
                return {'vid': '', 'pid': '', 'serial': '', 'firmware_hash': '', 'is_authentic': False}
            data = json.loads(result.stdout)
            return self._extract_fingerprint(data)
        except Exception as e:
            logger.warning(f"[DarwinProvider] 指纹提取失败: {e}")
            return {'vid': '', 'pid': '', 'serial': '', 'firmware_hash': '', 'is_authentic': False}

    def _extract_fingerprint(self, data: Dict, depth: int = 0) -> Dict[str, Any]:
        """递归提取 VIKEY 设备指纹"""
        if depth > 10:
            return {'vid': '', 'pid': '', 'serial': '', 'firmware_hash': '', 'is_authentic': False}
        items = data.get("_items", []) if isinstance(data, dict) else []
        ...
        for item in items:
            ...
            if target_vid in vid and target_pid in pid:
                ...
                firmware = str(item.get("bcd_device", "1.0"))
                firmware_hash = hashlib.sha256(firmware.encode()).hexdigest()[:16]
                # 3 维校验：VID/PID + 序列号 + 固件版本哈希
                is_authentic = bool(target_vid in vid and target_pid in pid and serial)
                return {
                    'vid': vid, 'pid': pid,
                    'serial': serial,
            ...
            child = self._extract_fingerprint(item, depth + 1)
            if child.get('is_authentic'):
                return child
        return {'vid': '', 'pid': '', 'serial': '', 'firmware_hash': '', 'is_authentic': False}


class WindowsHardwareKeyProvider(HardwareKeyProvider):
    """Windows 硬件密钥检测器

    VIKEY: PowerShell Get-PnpDevice + VID/PID 匹配
    SZU100: PowerShell Get-Volume + FileSystemLabel 卷标匹配
    """
    ...
    def detect_vikey(self) -> bool:
        try:
            ...
            try:
                ...
                target_vid = "0x1234"
            for dev in devices:
                inst = str(dev.get("InstanceId", "")).lower()
                if target_vid in inst:
                    return True
            return False
        except Exception as e:
            logger.warning(f"[WindowsProvider] VIKEY 检测失败: {e}")
            return False

    def detect_szu100(self) -> bool:
        try:
            import subprocess as _sp
            ps_cmd = "Get-Volume | Select-Object DriveLetter, FileSystemLabel | ConvertTo-Json"
            result = _sp.run(
            ...
            if not isinstance(devices, list):
                devices = [devices]
            try:
                from services.vikey_detector import SZU100_NAME_KEYWORDS
            except ImportError:
                SZU100_NAME_KEYWORDS = ("szu100", "szu-100", "szu_100", "szu 100",
                                        "shenzhen university 100", "szu专用")
            for dev in devices:
                label = str(dev.get("FileSystemLabel", "") or "").lower()
                if label and any(kw in label for kw in SZU100_NAME_KEYWORDS):
                    return True
            return False
        except Exception as e:
            logger.warning(f"[WindowsProvider] SZU100 检测失败: {e}")
            return False


class LinuxHardwareKeyProvider(HardwareKeyProvider):
    """Linux 硬件密钥检测器

    VIKEY: lsusb -v + VID/PID 匹配
    SZU100: /media + /mnt 扫描 + lsblk -J LABEL 匹配
    """
    ...
    def detect_vikey(self) -> bool:
        try:
            ...
            try:
                ...
                target_vid = "0x1234"
                target_pid = "0x5678"
            return target_vid in output and target_pid in output
        except Exception as e:
            logger.warning(f"[LinuxProvider] VIKEY 检测失败: {e}")
            return False
            ...
            for base in ("/media", "/mnt"):
                if os.path.isdir(base):
                    for entry in os.listdir(base):
                        ...
                        if any(kw in entry_l for kw in SZU100_NAME_KEYWORDS):
                            return True
                        full = os.path.join(base, entry)
                        if os.path.isdir(full):
                            for sub in os.listdir(full):
                                sub_l = (sub or "").lower()
                                if any(kw in sub_l for kw in SZU100_NAME_KEYWORDS):
                                    return True
            # lsblk -J LABEL 兜底
            import subprocess as _sp
            result = _sp.run(
                ["lsblk", "-J", "-o", "NAME,LABEL"],
                capture_output=True, text=True, timeout=5,
            )
                    ...
                    if isinstance(node, dict):
                        ...
                        for item in node:
                            if _scan(item):
                                return True
                    return False
                if _scan(data.get("blockdevices")):
                    return True
            return False
        except Exception as e:
            logger.warning(f"[LinuxProvider] SZU100 检测失败: {e}")
            return False


# HardwareKeyProvider 单例（线程安全双重检查锁）
_hardware_key_provider_lock = threading.Lock()
_hardware_key_provider: Optional[HardwareKeyProvider] = None


def get_hardware_key_provider() -> HardwareKeyProvider:
    """获取硬件密钥统一抽象层单例（运行时 platform.system() 选择）

    收敛 3 套并行模块（CODE_STUDY_NOTES.md 不一致点 U1/U2 修复）:
    - services/vikey_detector.VikeyDetector
    - core/services/vikey_driver.VikeyDriverManager
    - core/services/szu100_driver.detect_szu100

    统一入口: detect_vikey() / detect_szu100() / verify_dual_key_atomic() / get_device_fingerprint()

    约束合规:
    - G1 不引入新数据库表
    - G2 不引入新依赖
    - G4 文件变更数 ≤15（在 vikey_driver.py MODIFY 范围内）
    - 张晓峰综1: 双密钥原子校验
    - 张晓峰异议2: ≤5s 95 分位软阈值
    """
    global _hardware_key_provider
    if _hardware_key_provider is None:
        with _hardware_key_provider_lock:
            if _hardware_key_provider is None:
                os_name = _safe_platform_system()
                if os_name == "Darwin":
                    _hardware_key_provider = DarwinHardwareKeyProvider()
                elif os_name == "Windows":
                    _hardware_key_provider = WindowsHardwareKeyProvider()
                elif os_name == "Linux":
                    _hardware_key_provider = LinuxHardwareKeyProvider()
                else:
                    _hardware_key_provider = DarwinHardwareKeyProvider()
                logger.info(f"[HardwareKeyProvider] 初始化完成: platform={os_name}, "
                            f"provider={type(_hardware_key_provider).__name__}")
    return _hardware_key_provider
