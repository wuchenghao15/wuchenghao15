...
class GitSyncManager:
    ...
    def _run_command(self, cmd, capture_output=True):
        try:
            ...
            return {
                ...
                'stder': result.stderr.strip() if result.stderr else '',
                'returncode': result.returncode
            }
        except subprocess.TimeoutExpired:
            return {'success': False, 'stdout': '', 'stder': '命令执行超时', 'returncode': -1}
        except Exception as e:
            return {'success': False, 'stdout': '', 'stder': str(e), 'returncode': -2}

    def get_status(self):
        result = self._run_command([self.git_executable, 'status'])
        if not result['success']:
            return {'success': False, 'message': '获取状态失败', 'error': result['stder']}
    ...
    def get_log(self, limit=10):
        ...
        if not result['success']:
            return {'success': False, 'message': '获取日志失败', 'error': result['stder']}
    ...
    def pull(self, remote='origin', branch='main'):
        ...
        if not result['success'] and 'Already up to date' in result['stder']:
            return {'success': True, 'stdout': 'Already up to date', 'stder': '', 'returncode': 0}
        return result
    ...
    def sync(self, commit_message=None):
        ...
        if not add_result['success']:
            return {'success': False, 'message': '添加文件失败', 'error': add_result['stder']}

        commit_result = self.commit(commit_message)
        if not commit_result['success']:
            return {'success': False, 'message': '提交失败', 'error': commit_result['stder']}

        push_result = self.push()
        if not push_result['success']:
            return {'success': False, 'message': '推送失败', 'error': push_result['stder']}
    ...
    def auto_sync(self):
        try:
            ...
            add_result = self.add_all()
            if not add_result['success']:
                logger.error(f"Git添加文件失败: {add_result['stder']}")
                return add_result

            commit_result = self.commit(commit_message)
            if not commit_result['success']:
                logger.error(f"Git提交失败: {commit_result['stder']}")
                return commit_result

            push_result = self.push()
            if not push_result['success']:
                logger.error(f"Git推送失败: {push_result['stder']}")
                return push_result
