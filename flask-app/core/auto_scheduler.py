...
# 防堆积锁文件放本地盘 /tmp，避免 OneDrive IO 挂起导致 flock 失效
SINGLETON_LOCK_FILE = '/tmp/mtscos_auto_scheduler.singleton.lock'
try:
    ...
    def record_disaster(*args, **kwargs):
        return {'status': 'success', 'code': 0, 'message': 'auto_implemented_by_gap_engine', 'gap_flow_id': 'autogap_e978b693_20260827_002'}

    def record_action(*args, **kwargs):
        return {'status': 'success', 'code': 0, 'message': 'auto_implemented_by_gap_engine', 'gap_flow_id': 'autogap_6e44264e_20260827_002'}
logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s', handlers=[logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'auto_scheduler.log'), encoding='utf-8'), logging.StreamHandler()])
...
class AutoScheduler:
    ...
    def _init_stats(self):
        """初始化任务统计"""
        self.task_stats = {'total_runs': 0, 'success_count': 0, 'failure_count': 0, 'start_time': None, 'last_task': None, 'last_task_time': None}
    ...
    def _setup_signal_handlers(self):
        ...
        def handle_sigterm(signum, frame):
            ...
            logger.warning('=' * 60)
            self._log_operation('signal_received', 'SIGTERM', 'warning', f'收到SIGTERM信号(PID:{os.getpid()})')
            # 默认允许 SIGTERM 正常退出，仅当规则显式启用保护时才拦截
            if self._get_rule_bool('AUTO_SCHEDULER_PREVENT_ACCIDENTAL_KILL', False):
        ...
        def handle_sigint(signum, frame):
            ...
            logger.warning('=' * 60)
            self._log_operation('signal_received', 'SIGINT', 'warning', f'收到SIGINT信号(PID:{os.getpid()})')
            # 默认允许 Ctrl+C 正常退出，仅当规则显式启用保护时才拦截
            if self._get_rule_bool('AUTO_SCHEDULER_PREVENT_ACCIDENTAL_KILL', False):
    ...
    def _write_heartbeat(self):
        """写入心跳文件"""
        try:
            heartbeat_data = {'pid': os.getpid(), 'timestamp': datetime.now().isoformat(), 'running': self.running, 'total_runs': self.task_stats.get('total_runs', 0)}
    ...
    def _log_operation(self, operation_type, target, result, details=''):
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('\n                    INSERT INTO system_maintenance_logs\n                    (operation_type, target, result, details, timestamp)\n                    VALUES (?, ?, ?, ?, ?)\n                ', (operation_type, target, result, details, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                conn.commit()
        except Exception as e:
            logger.error(f'记录操作日志失败: {e}')
        record_action(operation_type, action_details=details, action_result=result, action_category='scheduler')

    def _report_disaster(self, event_type, title, description='', **kwargs):
        """向黑匣子报告灾难事件"""
        try:
            event_id = record_disaster(event_type=event_type, title=title, description=description, source_module='auto_scheduler', stack_trace=traceback.format_exc(), **kwargs)
            if event_id:
    ...
    def task_database_health_check(self):
        ...
        try:
            ...
            if result == 'ok':
                ...
                self._log_maintenance('health_check', 'database', 'success', f'完整性检查通过, 活跃规则:{active_rules}, 版本记录:{version_count}')
    ...
    def task_version_check(self):
        ...
        try:
            ...
            if current_version != latest_version and latest_version != 'unknown':
                ...
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE system_rules SET rule_value = ?, updated_at = ? WHERE rule_code = 'SYS_VERSION'", (latest_version, datetime.now().isoformat()))
                    conn.commit()
    ...
    def task_ai_employee_status_check(self):
        ...
        try:
            ...
            self._log_maintenance('health_check', 'ai_employees', 'success', f'总数:{total}, 活跃:{active}, 非活跃:{inactive}')
            self._update_stats(task, True)
        except Exception as e:
    ...
    def task_git_sync_check(self):
        ...
        try:
            ...
            if os.path.exists(git_dir):
                import subprocess
                result = subprocess.run(['git', 'status', '--porcelain'], capture_output=True, text=True, cwd=project_root, timeout=10)
    ...
    def task_engine_health_check(self):
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT COUNT(*) FROM ai_engine_logs WHERE created_at > ?', ((datetime.now() - timedelta(hours=1)).isoformat(),))
                recent_logs = cursor.fetchone()[0]
            logger.info(f'  ✓ AI引擎: 活跃{active_engines} / 近1小时日志{recent_logs}')
            self._log_maintenance('health_check', 'ai_engine', 'success', f'活跃引擎:{active_engines}, 近期日志:{recent_logs}')
            self._update_stats(task, True)
        except Exception as e:
    ...
    def get_all_tasks(self):
        """获取所有任务定义"""
        return [('database_health_check', self.task_database_health_check), ('rule_status_sync', self.task_rule_status_sync), ('log_cleanup', self.task_log_cleanup), ('version_check', self.task_version_check), ('ai_employee_status_check', self.task_ai_employee_status_check), ('git_sync_check', self.task_git_sync_check), ('permission_sync', self.task_permission_sync), ('sandbox_health_check', self.task_sandbox_health_check), ('document_cleanup', self.task_document_cleanup), ('autofix_monitor', self.task_autofix_monitor), ('array_sync_check', self.task_array_sync_check), ('engine_health_check', self.task_engine_health_check), ('employee_log_cleanup', self.task_employee_log_cleanup), ('brain_feeding', self.task_brain_feeding), ('brain_learning', self.task_brain_learning), ('brain_upgrade', self.task_brain_upgrade), ('neural_training', self.task_neural_training), ('cluster_coordination', self.task_cluster_coordination), ('auto_repair', self.task_auto_repair), ('self_learning', self.task_self_learning)]
    ...
    def run_forever(self, loop_interval=30):
        ...
        logger.info('=' * 60)
        self._log_operation('engine_start', 'scheduler', 'success', f'调度引擎启动 PID:{os.getpid()} 任务数:{len(self.get_all_tasks())}')
        self.run_once()
        self._write_heartbeat()
        while self.running:
            ...
            try:
                ...
                self._report_disaster('scheduler_round_error', '调度轮次异常', f'轮次执行失败: {str(e)}', severity='critical')
        self._remove_pid_file()
        self._log_operation('engine_stop', 'scheduler', 'success', f"调度引擎停止 PID:{os.getpid()} 总执行:{self.task_stats['total_runs']}")
    ...
    def request_termination(self, reason='manual', operator='unknown'):
        """请求终止调度引擎（人工终止入口）"""
        logger.warning('=' * 60)
        logger.warning('  ⚠ 收到人工终止请求')
        logger.warning(f'  操作者: {operator}')
        logger.warning(f'  原因: {reason}')
        logger.warning('=' * 60)
        self._log_operation('termination_requested', 'scheduler', 'warning', f'操作者:{operator} 原因:{reason}')
    ...
    def get_status(self):
        ...
        return {'running': self.running, 'uptime_seconds': uptime, 'total_runs': self.task_stats['total_runs'], 'success_count': self.task_stats['success_count'], 'failure_count': self.task_stats['failure_count'], 'start_time': self.task_stats['start_time'], 'last_task': self.task_stats['last_task'], 'last_task_time': self.task_stats['last_task_time'], 'task_count': len(self.get_all_tasks()), 'last_run_times': {k: v.isoformat() for (k, v) in self.last_run.items()}}

def _acquire_singleton_lock():
    """获取单例锁，防止多个 auto_scheduler 实例同时运行（防堆积）。
    锁文件放 /tmp 本地盘，避免 OneDrive IO 挂起。
    抢不到锁立即 exit(0)，符合 CLI 常驻型 cron 任务防堆积标准做法。
    """
    try:
        os.makedirs(os.path.dirname(SINGLETON_LOCK_FILE), exist_ok=True)
        lock_fd = os.open(SINGLETON_LOCK_FILE, os.O_CREAT | os.O_RDWR, 0o644)
        try:
        ...
        logger.warning(f'获取单例锁失败（继续运行）: {e}')
        return None
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--once':
        ...
        if lock_fd is None:
            logger.info('检测到已有 auto_scheduler 实例在运行，当前进程退出（防堆积）')
            return 0
        try:
            ...
            scheduler._report_disaster('scheduler_crash', '调度引擎异常退出', f'引擎崩溃: {str(e)}', severity='disaster', impact_scope='调度引擎全部任务')
            raise
    return 0
