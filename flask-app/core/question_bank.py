...
class QuestionBankExpander:
    ...
    def expand_from_online(self, subject: str, grade_level: str,
        ...
        try:
            ...
            if os.path.exists(db_path):
                ...
                sql = "SELECT * FROM questions WHERE subject=? ORDER BY RANDOM() LIMIT ?"
                rows = conn.execute(sql, (subject, count)).fetchall()
                for r in rows:
                    d = dict(r)
                    d['source'] = '本地题库'
                    questions.append(d)
    ...
    def _generate_mock_question(self, subject: str, grade_level: str, index: int) -> Dict[str, Any]:
        ...
        if question_type == "all":
            q_type = secrets.choice(self.question_types)
        else:
        ...
        content = secrets.choice(type_templates)
