...
class AIRuleEnhancer:
    ...
    def _define_required_rules(self):
        ...
        return {
            ...
            'ai_brain_auto_update': {
                ...
                'type': 'json',
                'description': 'AI脑库自动更新配置，控制题库自动扩充频率和数量',
                'is_active': 1
            },
            # 考试生成规则
            ...
            'user_management_rules': {
                ...
                'type': 'json',
                'description': '用户管理规则，包括默认角色、密码策略和登录限制',
                'is_active': 1
            },
            # 数据清理规则
            'data_cleanup_rules': {
                'value': json.dumps({
                    ...
                    'remove_inactive_users_days': 180,
                    'remove_old_test_results_days': 365,
                    'remove_temp_files_hours': 24
                }),
                'type': 'json',
            ...
            'ai_exam_scoring': {
                'value': json.dumps({
                    'enabled': True,
                    'scoring_accuracy': 'high',
                    'max_response_time_seconds': 30,
                    'enable_feedback': True
                }),
                'type': 'json',
                'description': 'AI考试评分规则，控制评分准确性和反馈生成',
                'is_active': 1
            },
            # 系统日志规则
    ...
    def enhance_system_rules(self):
        ...
        logger.info(f"AI规则完善完成：创建了 {created_count} 条规则，更新了 {updated_count} 条规则")
