...
def system_status():
    return jsonify({'status': 'running', 'version': "8.7.0", 'timestamp': datetime.now().isoformat()})
...
try:
    from app.api.super_admin_api import super_admin_api
...
# ==================== 统一版本管理API ====================
try:
    from app.api.version_unified_api import version_unified_api
    app.register_blueprint(version_unified_api)
    logger.info("✓ 注册蓝图: version_unified_api")
except Exception as e:
    logger.error(f"✗ 注册蓝图 version_unified_api 失败: {e}")

# ==================== AI赋能API ====================
try:
    from app.api.ai_empowerment_api import ai_empowerment_api
    app.register_blueprint(ai_empowerment_api)
    logger.info("✓ 注册蓝图: ai_empowerment_api")
except Exception as e:
    logger.error(f"✗ 注册蓝图 ai_empowerment_api 失败: {e}")

# ==================== 系统扩展API ====================
try:
    from app.api.system_extension_api import system_extension_api
    app.register_blueprint(system_extension_api)
    logger.info("✓ 注册蓝图: system_extension_api")
except Exception as e:
