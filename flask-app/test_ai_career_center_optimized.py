...
def test_skill_search():
    ...
    try:
        ...
        # 按关键词搜索
        logger.info("\n2.1 按关键词搜索 'Git'")
        git_skills = ai_career_center_optimized.search_skills(keyword='Git')
...
def test_career_path_search():
    """测试职业路径搜索"""
    try:
        logger.info("\n" + "=" * 60)
        logger.info("测试3: 职业路径搜索功能")
        logger.info("=" * 60)
...
def test_learning_path_tracking():
    ...
    try:
        logger.info("\n" + "=" * 60)
        logger.info("测试5: 学习路径跟踪")
        logger.info("=" * 60)
        
        from app.ai.ai_career_center_optimized import ai_career_center_optimized
        
        # 先注册一个AI
        ai_career_center_optimized.register_ai(
            'test_ai_002',
            'git_specialist',
            'Git学习AI',
            ['git_basic']
        ...
        # 跟踪学习进度
        logger.info("\n5.1 初始学习进度")
        progress1 = ai_career_center_optimized.track_learning_progress('test_ai_002', 'learn_git_expert')
...
def test_performance_recording():
    ...
    try:
        ...
        performances = [
            ...
            ('code_analysis', 0.92, 'code_review'),
        ]
        ...
        for skill_id, score, task_type in performances:
            ...
            logger.info(f"记录 {skill_id} 性能: {score} -> 成功: {success}")
        ...
        assert summary['overall_average'] > 0, "平均分应该大于0"
...
def test_statistics():
    ...
    try:
        ...
        stats = ai_career_center_optimized.get_statistics()
        ...
        logger.info(f"总性能记录: {stats['total_performance_records']}")
        ...
        logger.info("\nAI按类型分布:")
        for ai_type, count in stats['ais_by_type'].items():
            logger.info(f"  {ai_type}: {count}")
        
        assert 'total_skills' in stats, "统计数据不完整"
...
def main():
    ...
    results.append(("学习路径跟踪", test_learning_path_tracking()))
