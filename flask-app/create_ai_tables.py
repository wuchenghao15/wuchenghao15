    ...
    def monitor_database_performance(self):
        ...
        return {
            ...
            'performance_metrics': {
                ...
                'table_count': len(self.get_database_schema())
            }
        }
                ''',
                'status': 'active',
                'description': 'SQLite数据库AI适配器，提供数据库架构获取、查询性能分析、优化查询生成和数据库优化功能'
            })
            logger.info("默认AI数据库适配器插入成功")
        
        logger.info("所有AI相关表创建完成")
        return True
