...
class AILearningAgent:
    ...
    def learn_from_model(self, source_model_name, target_model_name):
        """从一个模型学习到另一个模型"""
        try:
...
class AILearningSystem:
    ...
    def __init__(self, ai_service_manager=None, knowledge_base=None):
        # ai_service_manager 可选: 缺失时从 ai_engines.ai_service 导入单例, 再缺失做空兜底
        if ai_service_manager is None:
            try:
                from ai_engines.ai_service import ai_service_manager as _mgr
                ai_service_manager = _mgr
            except Exception:
                try:
                    from ai_service import ai_service_manager as _mgr
                    ai_service_manager = _mgr
                except Exception:
                    class _EmptyMgr:
    ...
    def run_learning_cycle(self):
        """AI学习周期 — server_real_db.py 调用入口
        委托 learning_agent.self_upgrade_cycle, 返回结构化结果
        """
        try:
            logger.info("[AI-LEARNING] run_learning_cycle 开始")
            result = self.learning_agent.self_upgrade_cycle()
            self.last_learning_time = time.time()
            logger.info(f"[AI-LEARNING] run_learning_cycle 完成: {result}")
            return result
        except Exception as e:
            logger.error(f"[AI-LEARNING] run_learning_cycle 失败: {e}")
            return False
