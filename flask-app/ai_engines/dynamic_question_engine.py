...
QUESTION_TEMPLATES = {
    'single_choice': {
        ...
        'chinese': [
            lambda kp, diff: "下列词语中，书写正确的是：",
            lambda kp, diff: f"成语'{kp[0]}'的含义是：",
            lambda kp, diff: f"下列句子中，使用{kp[0]}修辞手法的是：",
            lambda kp, diff: f"古诗词'{kp[0]}'的作者是："
        ],
        'english': [
            lambda kp, diff: f"The word '{secrets.choice(['beautiful', 'important', 'difficult', 'necessary'])}' means:",
            lambda kp, diff: "Choose the correct form of the verb:",
            lambda kp, diff: "Which sentence is grammatically correct?",
            lambda kp, diff: "The main idea of the passage is about:"
        ],
        'physics': [
        ...
        'japanese': [
            lambda kp, diff: f"「{secrets.choice(['日本', '学校', '友達', '食事'])}」の読み方は：",
            lambda kp, diff: f"「{secrets.choice(['日本', '学校', '友達', '食事'])}」の読み方は：",
            lambda kp, diff: f"「{random.choice(['は', 'が', 'を', 'に'])}」の使い方は：",
            lambda kp, diff: "次の単語の意味はどれですか：",
            lambda kp, diff: "正しい文はどれですか："
        ]
    },
    'fill_blank': {
        'math': [
            lambda kp, diff: f"{kp[0]}的计算公式是______。",
            lambda kp, diff: f"若{kp[0]} = {random.randint(1, 10)}，则{kp[1]} = ______。",
        ...
        'english': [
            lambda kp, diff: "She ______ (go) to school every day.",
            lambda kp, diff: "The ______ of the story is very interesting.",
            lambda kp, diff: "He is good ______ math.",
            lambda kp, diff: "If I ______ (be) you, I would study harder."
        ],
        'physics': [
            lambda kp, diff: f"{kp[0]}的单位是______。",
            lambda kp, diff: "光在真空中的传播速度是______ m/s。",
            lambda kp, diff: f"{kp[0]}的定律表达式是______。",
            lambda kp, diff: "力的三要素是大小、方向和______。"
        ],
        'chemistry': [
            lambda kp, diff: "水的化学式是______。",
            lambda kp, diff: f"{kp[0]}的原子序数是______。",
            lambda kp, diff: "元素周期表中，第一周期有______种元素。",
            lambda kp, diff: f"{kp[0]}的常见化合价是______。"
        ],
        'biology': [
            lambda kp, diff: f"{kp[0]}的主要功能是______。",
            lambda kp, diff: "光合作用的场所是______。",
            lambda kp, diff: "DNA的全称是______。",
        ...
        'history': [
            ...
            lambda kp, diff: "中国近代史的开端是______。"
        ],
        'geography': [
            lambda kp, diff: f"{kp[0]}的特点是______。",
            lambda kp, diff: "地球自转的方向是______。",
            lambda kp, diff: "世界上最大的洋是______。",
            lambda kp, diff: f"{kp[0]}形成的原因是______。"
        ],
        'politics': [
            lambda kp, diff: f"{kp[0]}的核心内容是______。",
            lambda kp, diff: "社会主义核心价值观在国家层面是______。",
            lambda kp, diff: "我国的根本政治制度是______。",
            lambda kp, diff: f"{kp[0]}的基本原则是______。"
        ],
        'japanese': [
            lambda kp, diff: f"「{secrets.choice(['こんにちは', 'ありがとう', 'すみません'])}」は______の意味です。",
            lambda kp, diff: f"「{random.choice(['食べる', '行く', 'する'])}」のます形は______です。",
            lambda kp, diff: "日本の首都は______です。",
    ...
    'short_answer': {
        ...
        'english': [
            ...
            lambda kp, diff: "What is the main idea of the passage?",
            lambda kp, diff: "Translate the following sentence into English."
        ],
        'physics': [
        ...
        'japanese': [
            ...
            lambda kp, diff: "次の文章を日本語に訳してください。"
        ]
    },
    'calculation': {
        'math': [
            lambda kp, diff: f"计算：{(10 + secrets.randbelow(int(100) - int(10) + 1))} + {(10 + secrets.randbelow(int(100) - int(10) + 1))} = ?",
            lambda kp, diff: f"解方程：{(1 + secrets.randbelow(int(5) - int(1) + 1))}x + {(1 + secrets.randbelow(int(20) - int(1) + 1))} = {(20 + secrets.randbelow(int(100) - int(20) + 1))}",
            lambda kp, diff: f"求函数 f(x) = {(1 + secrets.randbelow(int(5) - int(1) + 1))}x² + {(-10 + secrets.randbelow(int(10) - int(-10) + 1))}x + {(1 + secrets.randbelow(int(20) - int(1) + 1))} 的极值。",
            lambda kp, diff: f"已知三角形三边长分别为{random.randint(3, 10)}、{random.randint(4, 12)}、{random.randint(5, 13)}，求面积。"
        ],
        'physics': [
            lambda kp, diff: f"一辆汽车以{(20 + secrets.randbelow(int(100) - int(20) + 1))}km/h的速度行驶，{(1 + secrets.randbelow(int(5) - int(1) + 1))}小时行驶多少公里？",
            lambda kp, diff: f"一个质量为{(1 + secrets.randbelow(int(10) - int(1) + 1))}kg的物体受到{(10 + secrets.randbelow(int(100) - int(10) + 1))}N的力，求加速度。",
            lambda kp, diff: f"电阻为{(10 + secrets.randbelow(int(100) - int(10) + 1))}Ω的导体，通过{(1 + secrets.randbelow(int(10) - int(1) + 1))}A的电流，求电压。",
            lambda kp, diff: f"物体从{random.randint(10, 100)}m高处自由落下，求落地时的速度（g=9.8m/s²）。"
        ],
        'chemistry': [
            lambda kp, diff: f"计算{(1 + secrets.randbelow(int(10) - int(1) + 1))}mol {secrets.choice(['H₂O', 'CO₂', 'NaCl'])}的质量。",
            lambda kp, diff: f"将{(10 + secrets.randbelow(int(100) - int(10) + 1))}g {secrets.choice(['NaCl', 'KNO₃'])}溶于{(100 + secrets.randbelow(int(500) - int(100) + 1))}g水中，求溶质质量分数。",
            lambda kp, diff: f"计算{secrets.choice(['H₂', 'O₂', 'N₂'])}在标准状况下{(1 + secrets.randbelow(int(22.4) - int(1) + 1))}L的物质的量。",
            lambda kp, diff: f"已知{random.choice(['HCl', 'NaOH'])}溶液浓度为{random.randint(0.1, 2)}mol/L，求{random.randint(10, 100)}mL溶液中溶质的物质的量。"
        ]
    },
    'judge': {
        'math': [
            lambda kp, diff: f"{kp[0]}的计算公式是{secrets.choice(['正确', '错误'])}的。",
        ...
        'chinese': [
            ...
            lambda kp, diff: f"{secrets.choice(['李白', '杜甫'])}是{kp[0]}的作者。",
            lambda kp, diff: f"{kp[0]}是{random.choice(['唐诗', '宋词'])}的代表作品。"
        ],
        'english': [
            lambda kp, diff: f"The word '{secrets.choice(['affect', 'effect'])}' is a {secrets.choice(['verb', 'noun'])}.",
            lambda kp, diff: f"{kp[0]} is used to express {secrets.choice(['past', 'future'])} tense.",
            lambda kp, diff: f"All verbs in English have {secrets.choice(['past tense', 'present tense'])}.",
            lambda kp, diff: f"{kp[0]} is a type of {random.choice(['clause', 'phrase'])}."
        ],
        'physics': [
            lambda kp, diff: f"{kp[0]}的单位是{secrets.choice(['牛顿', '焦耳'])}。",
            lambda kp, diff: f"{kp[0]}和{secrets.choice(['质量', '重量'])}是同一个概念。",
            lambda kp, diff: f"{kp[0]}的方向总是{secrets.choice(['竖直向下', '水平'])}的。",
            lambda kp, diff: f"{kp[0]}是{random.choice(['矢量', '标量'])}。"
        ],
        'chemistry': [
            lambda kp, diff: f"{kp[0]}是{secrets.choice(['电解质', '非电解质'])}。",
            lambda kp, diff: f"{kp[0]}的化合价是{secrets.choice(['+1', '-1'])}。",
            lambda kp, diff: f"{kp[0]}和{secrets.choice(['酸', '碱'])}能发生中和反应。",
            lambda kp, diff: f"{kp[0]}是{random.choice(['有机物', '无机物'])}。"
        ],
        'biology': [
            lambda kp, diff: f"{kp[0]}是{secrets.choice(['植物', '动物'])}细胞特有的结构。",
            lambda kp, diff: f"{kp[0]}的主要功能是{secrets.choice(['光合作用', '呼吸作用'])}。",
            lambda kp, diff: f"{kp[0]}是{secrets.choice(['DNA', 'RNA'])}的组成部分。",
            lambda kp, diff: f"{kp[0]}能进行{random.choice(['有丝分裂', '减数分裂'])}。"
        ],
        'history': [
            lambda kp, diff: f"{kp[0]}发生在{secrets.choice(['19世纪', '20世纪'])}。",
            lambda kp, diff: f"{kp[0]}是{secrets.choice(['资产阶级革命', '无产阶级革命'])}。",
            lambda kp, diff: f"{kp[0]}的领导人是{secrets.choice(['孙中山', '毛泽东'])}。",
            lambda kp, diff: f"{kp[0]}标志着{random.choice(['中国近代史', '中国现代史'])}的开端。"
        ],
        'geography': [
            lambda kp, diff: f"{kp[0]}的形成与{secrets.choice(['板块运动', '气候'])}有关。",
            lambda kp, diff: f"{kp[0]}主要分布在{secrets.choice(['热带', '温带'])}地区。",
            lambda kp, diff: f"{kp[0]}是{secrets.choice(['可再生', '不可再生'])}资源。",
            lambda kp, diff: f"{kp[0]}的特点是{random.choice(['高温多雨', '寒冷干燥'])}。"
        ],
        'politics': [
            lambda kp, diff: f"{kp[0]}是社会主义核心价值观的内容。",
            lambda kp, diff: f"{kp[0]}是我国的{secrets.choice(['根本政治制度', '基本政治制度'])}。",
            lambda kp, diff: f"{kp[0]}是{secrets.choice(['依法治国', '以德治国'])}的体现。",
            lambda kp, diff: f"{kp[0]}是{random.choice(['中国共产党', '人民代表大会'])}的宗旨。"
        ],
        'japanese': [
            lambda kp, diff: f"「{secrets.choice(['は', 'が'])}」は主題を示す助詞です。",
            lambda kp, diff: f"{kp[0]}は{secrets.choice(['名詞', '動詞'])}です。",
            lambda kp, diff: f"日本語には{secrets.choice(['敬語', '自謙語'])}があります。",
            lambda kp, diff: f"「{random.choice(['た形', 'て形'])}」は{random.choice(['過去', '現在'])}を表します。"
        ]
    },
    'translation': {
        'english': [
            lambda kp, diff: f"Translate: {secrets.choice(['科技改变生活', '学习使人进步', '时间就是金钱'])}",
            lambda kp, diff: f"Translate: {secrets.choice(['我喜欢阅读', '他正在学习英语', '我们应该保护环境'])}",
            lambda kp, diff: f"Translate: {secrets.choice(['知识就是力量', '实践出真知', '团结就是力量'])}",
            lambda kp, diff: f"Translate: {random.choice(['今天天气很好', '我想去旅行', '这本书很有趣'])}"
        ],
        'japanese': [
            lambda kp, diff: f"Translate: {secrets.choice(['我喜欢学习日语', '今天天气很好', '谢谢'])}",
            lambda kp, diff: f"Translate: {secrets.choice(['早上好', '再见', '请多关照'])}",
            lambda kp, diff: f"Translate: {secrets.choice(['学校在哪里', '我想吃饭', '你好'])}",
...
ANSWER_GENERATORS = {
    'single_choice': {
        'math': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'chinese': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'english': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'physics': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'chemistry': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'biology': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'history': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'geography': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'politics': lambda: str(secrets.choice(['A', 'B', 'C', 'D'])),
        'japanese': lambda: str(random.choice(['A', 'B', 'C', 'D']))
    },
    'fill_blank': {
        'math': lambda: str((1 + secrets.randbelow(int(100) - int(1) + 1))),
        'chinese': lambda: secrets.choice(['答案', '正确答案', '标准答案']),
        'english': lambda: secrets.choice(['is', 'are', 'was', 'were']),
        'physics': lambda: str((1 + secrets.randbelow(int(1000) - int(1) + 1))),
        'chemistry': lambda: secrets.choice(['H₂O', 'CO₂', 'NaCl']),
        'biology': lambda: secrets.choice(['细胞', 'DNA', '基因']),
        'history': lambda: str((1800 + secrets.randbelow(int(2020) - int(1800) + 1))),
        'geography': lambda: secrets.choice(['太平洋', '亚洲', '赤道']),
        'politics': lambda: secrets.choice(['人民', '社会主义', '民主']),
        'japanese': lambda: random.choice(['は', 'が', 'を'])
    },
    'short_answer': {
        'math': lambda: f"{secrets.choice(['根据定义', '通过计算', '利用公式'])}可得答案。",
        'chinese': lambda: f"{secrets.choice(['根据文章内容', '结合语境', '分析修辞手法'])}可知。",
        'english': lambda: f"{secrets.choice(['According to the passage', 'Based on context', 'Analyzing the grammar'])} we can conclude.",
        'physics': lambda: f"{secrets.choice(['根据牛顿定律', '利用能量守恒', '通过实验验证'])}可得。",
        'chemistry': lambda: f"{secrets.choice(['根据化学反应', '利用元素守恒', '分析物质结构'])}可得。",
        'biology': lambda: f"{secrets.choice(['根据细胞结构', '利用遗传规律', '分析生态系统'])}可得。",
        'history': lambda: f"{secrets.choice(['根据历史背景', '分析事件影响', '评价历史意义'])}可得。",
        'geography': lambda: f"{secrets.choice(['根据地理特征', '分析气候影响', '结合区域特点'])}可得。",
        'politics': lambda: f"{secrets.choice(['根据理论分析', '结合实践经验', '分析政策意义'])}可得。",
        'japanese': lambda: f"{random.choice(['文脈によると', '文法的には', '意味を考えると'])}分かります。"
    },
    'calculation': {
        'math': lambda: str((1 + secrets.randbelow(int(1000) - int(1) + 1))),
        'physics': lambda: f"{(1 + secrets.randbelow(int(100) - int(1) + 1))} {secrets.choice(['m/s', 'N', 'J'])}",
        'chemistry': lambda: f"{random.randint(1, 100)} {random.choice(['g', 'mol', '%'])}"
    },
    'judge': {
        'math': lambda: secrets.choice(['正确', '错误']),
        'chinese': lambda: secrets.choice(['正确', '错误']),
        'english': lambda: secrets.choice(['True', 'False']),
        'physics': lambda: secrets.choice(['正确', '错误']),
        'chemistry': lambda: secrets.choice(['正确', '错误']),
        'biology': lambda: secrets.choice(['正确', '错误']),
        'history': lambda: secrets.choice(['正确', '错误']),
        'geography': lambda: secrets.choice(['正确', '错误']),
        'politics': lambda: secrets.choice(['正确', '错误']),
        'japanese': lambda: random.choice(['正しい', '誤り'])
    },
    'translation': {
        'english': lambda: secrets.choice(['Technology changes life.', 'Learning makes progress.', 'Time is money.']),
...
class DynamicQuestionEngine:
    ...
    def generate_question(self, subject: str, question_type: str, difficulty: str, grade: str = '初中') -> Optional[Dict]:
        ...
        try:
            ...
            knowledge_group = secrets.choice(knowledge_groups)
    ...
    def batch_generate(self, subject: str = None, count: int = 10, difficulty: str = None, question_type: str = None, grade: str = '初中') -> Dict:
        ...
        for _ in range(count):
            s = secrets.choice(subjects)
            qt = secrets.choice([qt for qt in question_types if qt in QUESTION_TEMPLATES])
    ...
    def _crawl_single_subject(self, subject: str) -> Optional[Dict]:
        ...
        try:
            ...
            question_type = secrets.choice(['single_choice', 'fill_blank', 'short_answer'])
...
if __name__ == '__main__':
    ...
    print("=== 测试动态题目生成 ===")
    for i in range(5):
        subject = secrets.choice(list(SUBJECTS.keys()))
        question_type = secrets.choice(list(QUESTION_TYPES.keys()))
