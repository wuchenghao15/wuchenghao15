...
# 开发活动关键词清单（精确匹配 + 模糊正则双重保障）
# §14 v1.3.0 §3.4 定义 — v23.9.1 升级: 加模糊正则 + 近义词 + 拆分模式
...
# 模糊正则（v23.9.1 新增 — 解决精确匹配太死板的问题）
# 模式: "动词.*名词" 中间允许插任意字
_DEV_ACTIVITY_FUZZY_PATTERNS = [
    # 创建类动词 + 任意 + 名词
    r"创建.{0,15}(功能|模块|引擎|服务|接口|表|项目|页面|路由|文件|脚本|配置)",
    r"新建.{0,15}(功能|模块|引擎|服务|接口|表|项目|页面|路由|文件|脚本|配置|API)",
    r"新增.{0,15}(功能|模块|引擎|服务|接口|表|项目|页面|路由|文件|脚本|配置)",
    # 开发/实现类
    r"开发.{0,15}(功能|模块|引擎|服务|接口|表|项目|页面|路由)",
    r"实现.{0,15}(功能|模块|引擎|服务|接口|表|项目|页面|路由)",
    r"编写.{0,15}(代码|脚本|函数|类|模块|引擎)",
    r"写.{0,15}(代码|脚本|函数|类|模块|引擎|配置)",
    # 修改/修复类
    r"修改.{0,15}(代码|文件|配置|脚本|函数|类|路由)",
    r"修复.{0,15}(bug|Bug|BUG|问题|错误|异常|缺陷|bug)",
    r"解决.{0,15}(bug|Bug|BUG|问题|错误|异常|缺陷)",
    r"调整.{0,15}(配置|参数|代码|设置|规则)",
    r"优化.{0,15}(代码|性能|配置|逻辑|算法|查询)",
    r"重构.{0,15}(代码|模块|架构|项目)",
    # 安装/部署/升级类
    r"安装.{0,15}(包|依赖|库|引擎|插件|工具)",
    r"部署.{0,15}(服务|应用|项目|系统|容器)",
    r"升级.{0,15}(版本|引擎|框架|库|系统|依赖)",
    r"迁移.{0,15}(数据|服务|项目|数据库|系统)",
    # 数据变更类
    r"(INSERT|UPDATE|DELETE|ALTER|CREATE|DROP)\s+(INTO|TABLE|FROM)\b",
    r"数据库.{0,15}(变更|操作|查询|写入|更新|删除)",
    r"(加|增|删|改).{0,15}(字段|列|索引|约束|表)",
    # Git/包管理类
    r"git\s+(commit|push|pull|add|merge|rebase|checkout)",
    r"(pip|npm|yarn|conda)\s+install",
    r"git\s+log",
    # 系统/服务类
    r"(启动|运行|执行).{0,15}(docker|ollama|python|进程|服务|脚本|命令)",
    r"(停|关|重).{0,15}(服务|进程|docker|服务|daemon)",
    # AI/模型类
    r"(训练|微调|fine-tune).{0,15}(模型|LLM|神经网络|AI)",
    r"(推理|inference).{0,15}(模型|AI|神经网络)",
]
