...
class AIPerformanceOptimizer:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, 'latency', ?, 'ms', ?)
                ''', (
                    f"PM-{(100000 + secrets.randbelow(int(999999) - int(100000) + 1))}",
    ...
    def run_benchmark(self, model_id: str, predict_fn: Callable,
        ...
        n = len(sorted_lats)
        benchmark_id = f"BENCH-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
