...
class AILearningSystem:
    ...
    def _update_feature_vectors(self):
        ...
        # 3. 从本地AI引擎支持的功能中收集功能描述
        local_ai_features = [
            "text-generation", "chatbot", "question-answering",
            "translation", "summarization", "code-generation",
            "creative-writing", "code-review", "bug-analysis",
            "deep-thinking", "feature-association", "knowledge-enhancement"
        ]
        for feature in local_ai_features:
            if feature not in self._feature_descriptions:
                self._feature_descriptions[feature] = f"本地AI支持的{feature}功能"
    ...
    def enhance_with_ai(self, task_type: str, content: str, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 调用本地AI引擎 (零token优先)
            result = local_ai_chat(prompt, flow_id=f"ai_learning_{task_type}")

            if result and result.get("success"):
                return {
                    "success": True,
                    "enhanced_content": result["response"],
                    "engine_used": result.get("route", "local_ollama"),
                    "task_type": task_type,
                    "tokens_saved": result.get("tokens_saved", 0),
                    "timestamp": time.time()
                }
            else:
                logger.error(f"AI增强失败: {result.get('error') if result else '未知错误'}")
                return None
