...
class DispatchAIEmployee:
    ...
    def execute_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        dispatch_id = f"disp_{datetime.now().strftime('%Y%m%d%H%M%S')}_{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
    ...
    def _execute_on_target(self, target: Dict, task_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # 回退：返回未找到可执行员工的错误，不返回模拟结果
            return {
                'success': False,
                'error': f"员工 {target.get('name', '未知')} 不具备执行任务的能力",
