...
class QuestionGenerator:
    ...
    def generate_japanese_question(self, difficulty: str) -> dict:
        ...
        item = secrets.choice(vocab_list)
    ...
    def generate_math_question(self, difficulty: str) -> dict:
        """生成数学题目"""
        if difficulty == 'high':
            a = (10 + secrets.randbelow(int(50) - int(10) + 1))
            b = (10 + secrets.randbelow(int(50) - int(10) + 1))
            c = random.randint(10, 50)
            answer = a + b + c
            content = f"计算: {a} + {b} + {c} = ?"
        elif difficulty == 'medium':
            a = (10 + secrets.randbelow(int(30) - int(10) + 1))
            b = random.randint(1, 10)
            answer = a * b
            content = f"计算: {a} × {b} = ?"
        else:
            a = (1 + secrets.randbelow(int(20) - int(1) + 1))
            b = random.randint(1, 20)
            answer = a + b
            content = f"计算: {a} + {b} = ?"

        options = [answer, answer + (1 + secrets.randbelow(int(10) - int(1) + 1)),
                   answer - (1 + secrets.randbelow(int(10) - int(1) + 1)),
