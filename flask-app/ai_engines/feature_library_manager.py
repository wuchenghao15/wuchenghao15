...
class FeatureLibraryManager:
    ...
    def get_feature(self, feature_id):
        ...
        try:
            ...
            if result:
                feature = dict(result)
                if feature.get("tags"):
                    # 安全修复：使用json.loads替代eval
                    import json as _json
                    import ast as _ast
                    try:
                        feature["tags"] = _json.loads(feature["tags"])
                    except (ValueError, TypeError):
                        try:
                            feature["tags"] = _ast.literal_eval(feature["tags"])
                        except (ValueError, SyntaxError):
                            pass
                if feature.get("resolution"):
                    try:
                        feature["resolution"] = _json.loads(feature["resolution"])
                    except (ValueError, TypeError):
                        try:
                            feature["resolution"] = _ast.literal_eval(feature["resolution"])
                        except (ValueError, SyntaxError):
                            pass
                return feature
            return None
    ...
    def analyze_feature(self, feature_id):
        ...
        try:
            ...
            response = local_ai_chat(prompt, flow_id="feature_library_analyze")
            if response and response.get("success"):
                ...
                logger.info(f"特征分析完成: {feature_id} (route={response.get('route')}, saved={response.get('tokens_saved')} tokens)")
                return analysis
            return None
        except Exception as e:
