...
from m_flow.knowledge.graph_ops.exceptions import ConceptNotFoundError, ConceptAlreadyExistsError
...
@pytest.fixture
def setup_graph():
    """Fixture to initialize a MemoryGraph instance."""
    return MemoryGraph()
...
async def test_project_graph_from_db_nodeset_subgraph(setup_graph, mock_adapter):
    ...
    mock_adapter.extract_typed_subgraph = AsyncMock(return_value=(nodes_data, edges_data))
    ...
    assert graph.get_node("1") is not None
    assert len(graph.edges) == 1
    mock_adapter.extract_typed_subgraph.assert_called_once()


@pytest.mark.asyncio
async def test_project_graph_from_db_empty_graph(setup_graph, mock_adapter):
    """Test projecting empty graph raises ConceptNotFoundError."""
    graph = setup_graph
