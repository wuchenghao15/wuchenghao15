"""Unified Triplet Search Test — m_flow.tests.unit.modules.retrieval.unified_triplet_search_test"""
...
class TestUnifiedTripletSearch:
    @pytest.mark.asyncio
    async def test_graph_completion_context_simple(self):
        system_directory_path = os.path.join(
            pathlib.Path(__file__).parent, ".mflow/system/test_graph_completion_context_simple"
        )
        m_flow.config.system_root_directory(system_directory_path)
        data_directory_path = os.path.join(
            pathlib.Path(__file__).parent, ".data_storage/test_graph_completion_context_simple"
        )
        m_flow.config.data_root_directory(data_directory_path)
        ...
        await persist_memory_nodes(entities)
    ...
    async def test_graph_completion_context_complex(self):
        system_directory_path = os.path.join(
            pathlib.Path(__file__).parent, ".mflow/system/test_graph_completion_context_complex"
        )
        m_flow.config.system_root_directory(system_directory_path)
        data_directory_path = os.path.join(
        ...
        class Car(MemoryNode):
            brand: str
            model: str
            year: int

        class Location(MemoryNode):
            country: str
            city: str

        class Home(MemoryNode):
            location: Location
            rooms: int
            sqm: int
        ...
        retriever = UnifiedTripletSearch(top_k=20)
    ...
    async def test_get_graph_completion_context_on_empty_graph(self):
        system_directory_path = os.path.join(
            ...
            ".mflow/system/test_get_graph_completion_context_on_empty_graph",
        )
        m_flow.config.system_root_directory(system_directory_path)
        data_directory_path = os.path.join(
            pathlib.Path(__file__).parent,
