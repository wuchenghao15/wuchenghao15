        ...
        try:
            with get_db_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    employee.last_heartbeat = time.time()
...
# 🆕 2026-09-17: 延迟单例初始化, 避免 Flask import 时启动后台线程抢大库锁
# 之前模块级 ai_cluster_manager = AIClusterManager() 导致每次 import 就启动 monitoring thread
# 不停写 DB → Flask worker 刚 listen 就被锁堵 → HTTP timeout
# 改成 lazy: 第一次访问 _get_manager() 才创建, 并且延迟 30s 启动 monitoring
_ai_cluster_manager_instance = None
_ai_cluster_manager_lock = threading.Lock() if 'threading' in dir() else __import__('threading').Lock()

def _get_manager():
    global _ai_cluster_manager_instance
    if _ai_cluster_manager_instance is None:
        with _ai_cluster_manager_lock:
            if _ai_cluster_manager_instance is None:
                _ai_cluster_manager_instance = AIClusterManager()
                # 🆕 延迟 30s 启动 monitoring thread, 给 Flask 足够时间稳定
                def _delayed_start():
                    import time as _t
                    _t.sleep(30)
                    try:
                        _ai_cluster_manager_instance._start_monitoring_thread()
                        logger.info('[AI_Cluster] monitoring thread started (延迟 30s)')
                    except Exception as e:
                        logger.warning(f'[AI_Cluster] monitoring start fail: {e}')
                import threading as _thr
                _thr.Thread(target=_delayed_start, daemon=True, name='ai-cluster-delay').start()
    return _ai_cluster_manager_instance

# 保持向后兼容: ai_cluster_manager 作为 lazy property
class _LazyProxy:
    def __getattr__(self, name):
        return getattr(_get_manager(), name)
    def __repr__(self):
        return repr(_get_manager())
ai_cluster_manager = _LazyProxy()
