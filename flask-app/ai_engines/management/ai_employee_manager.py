...
from ai_engines.management.ai_employee_system import ValidationAIEmployee, RoutingAIEmployee, TestSystemAIEmployee, AIEmployee
...
class AIEmployeeManager:
    ...
    def _load_employees_from_database(self):
        ...
        try:
            ...
            # 列兼容性补齐：ai_employees 表可能由其他模块以不同 schema 创建，
            # CREATE TABLE IF NOT EXISTS 不会为已存在表补列，这里对加载所需列做 ALTER（已存在则忽略）
            for _col, _decl in [
                ('learning_rate', 'REAL DEFAULT 0.05'),
                ('knowledge_base_size', 'INTEGER DEFAULT 0'),
                ('last_training', 'TEXT'),
                ...
                try:
                    cursor.execute(f'ALTER TABLE ai_employees ADD COLUMN {_col} {_decl}')
                except Exception:
                    pass  # 列已存在
            conn.commit()
