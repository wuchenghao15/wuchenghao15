...
class TextGenerationModel(AIModel):
    ...
    def infer(self, input_data, **kwargs):
        ...
        try:
            ...
            # 尝试调用真实AI引擎
            generated_text = self._call_real_ai_engine(input_data, **kwargs)
            ...
            return {
                'success': False,
                'error': 'no_real_ai_engine_available',
                'inference_time': inference_time
            }
        except Exception as e:
    ...
    def _call_real_ai_engine(self, input_data, **kwargs):
        """尝试调用本地推理引擎 → 失败则fallback到外部API"""
        # 优先: 本地推理引擎 (零token消耗)
        try:
            import sys, os
            engine_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'engines')
            if engine_dir not in sys.path:
                sys.path.insert(0, engine_dir)
            from ai_local_inference_engine import route_to_local
            result = route_to_local("chat_completion", input_data)
            if result and result.get("output"):
                logger.info(f"[LOCAL-AI] text generated locally, tokens_saved={result.get('tokens_saved', 0)}")
                return result["output"]
        except Exception as e:
            logger.warning(f"Local inference fallback failed: {e}")

        # Fallback: 外部API (消耗token)
        try:
            import os
            api_key = os.environ.get('AI_API_KEY') or os.environ.get('OPENAI_API_KEY')
            api_url = os.environ.get('AI_API_URL') or os.environ.get('OPENAI_API_URL')
            if not api_key or not api_url:
                return None
            import requests
            headers = {'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'}
            payload = {'prompt': input_data, 'max_tokens': kwargs.get('max_tokens', 500)}
            resp = requests.post(api_url, json=payload, headers=headers, timeout=30)
            if resp.status_code == 200:
                data = resp.json()
                return data.get('text') or data.get('result') or data.get('choices', [{}])[0].get('text', '')
        except Exception as e:
            logger.error(f"Real AI engine call failed: {e}")
        return None
...
class ClassificationModel(AIModel):
    ...
    def infer(self, input_data, **kwargs):
        ...
        try:
            ...
            # 尝试调用真实分类引擎
            result = self._call_real_classifier(input_data, **kwargs)
            inference_time = time.time() - start_time
            ...
            return {
                'success': False,
                'error': 'no_real_classifier_available',
                'inference_time': inference_time
            }
        except Exception as e:
            return {
    ...
    def _call_real_classifier(self, input_data, **kwargs):
        """尝试调用本地分类引擎 → 失败则fallback到外部API"""
        # 优先: 本地推理引擎 (零token消耗)
        try:
            import sys, os
            engine_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'engines')
            if engine_dir not in sys.path:
                sys.path.insert(0, engine_dir)
            from ai_local_inference_engine import route_to_local
            result = route_to_local("code_classify", input_data)
            if result and result.get("output"):
                logger.info(f"[LOCAL-AI] classified locally, tokens_saved={result.get('tokens_saved', 0)}")
                import json
                return json.loads(result["output"])
        except Exception as e:
            logger.warning(f"Local classifier fallback failed: {e}")

        # Fallback: 外部API (消耗token)
        try:
            import os
            api_key = os.environ.get('AI_API_KEY') or os.environ.get('OPENAI_API_KEY')
            api_url = os.environ.get('AI_CLASSIFY_URL') or os.environ.get('OPENAI_API_URL')
            if not api_key or not api_url:
                return None
            import requests
            headers = {'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'}
            categories = kwargs.get('categories', [])
            payload = {'text': input_data, 'categories': categories}
            resp = requests.post(api_url, json=payload, headers=headers, timeout=30)
            if resp.status_code == 200:
                return resp.json()
        except Exception as e:
            logger.error(f"Real classifier call failed: {e}")
        return None
...
class TranslationModel(AIModel):
    ...
    def infer(self, input_data, **kwargs):
        ...
        try:
            ...
            # 尝试调用真实翻译引擎
            translated_text = self._call_real_translator(input_data, src_lang, tgt_lang)
            inference_time = time.time() - start_time
            ...
            return {
                'success': False,
                'error': 'no_real_translator_available',
                'inference_time': inference_time
            }
        except Exception as e:
    ...
    def _call_real_translator(self, text, src_lang, tgt_lang):
        """尝试调用真实翻译引擎，不可用时返回None"""
        try:
            import os
            api_key = os.environ.get('AI_API_KEY') or os.environ.get('TRANSLATE_API_KEY')
            api_url = os.environ.get('AI_TRANSLATE_URL') or os.environ.get('TRANSLATE_API_URL')
            if not api_key or not api_url:
                return None
            import requests
            headers = {'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'}
            payload = {'text': text, 'source': src_lang, 'target': tgt_lang}
            resp = requests.post(api_url, json=payload, headers=headers, timeout=30)
            if resp.status_code == 200:
                data = resp.json()
                return data.get('translated_text') or data.get('result') or data.get('text', '')
        except Exception as e:
            logger.error(f"Real translator call failed: {e}")
        return None
...
class AIServiceManager:
    ...
    def __init__(self):
        ...
        self.auto_upgrade_enabled = True       # L498 线程循环访问 — 修复 AttributeError
        self.last_upgrade_time = time.time()   # L500 间隔判断依赖 — 修复 NameError
