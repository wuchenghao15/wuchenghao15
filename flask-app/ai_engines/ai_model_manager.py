...
class ModelGroup:
    ...
    def select_model(self, models: Dict[str, AIModel]) -> Optional[str]:
        ...
        if self.strategy == 'weighted':
            ...
            import random
            return secrets.choice(active).model_id
...
class AIModelManager:
    ...
    def __init__(self, employee_id: str = "model_mgr_001", name: str = "AI模型管理专家"):
        # 兼容 AIEmployeeManager.create_initial_employees 中的统一调用约定：
        # AIModelManager("model_mgr_001", "模型管理AI") —— 与其他 7 个员工类
        # (AIAutoRepairAgent / AIDataAnalyzer / AIOpsAgent 等) 签名一致。
        self.employee_id = employee_id
        self.name = name
        self.employee_type = "model_manager"
        self.level = 8
        self.status = "active"
        self.last_active = None  # 由 _safe_start_employee 调用时填充
    ...
    # 与其他员工类一致的运行时接口 —— 供 AIEmployeeManager._safe_start_employee 调用
    def start(self):
        """启动模型管理服务"""
    ...
    def _register_default_models(self):
        """注册默认模型 — 本地推理引擎优先(零token消耗)"""
        defaults = [
            # 本地推理引擎: 最高优先级, 零token消耗
            AIModel('model_local_inference', 'Local Inference Engine', 'local', 'llm',
                   'local://ai_local_inference_engine', '', 4096, 0.8, True, 1),
            # 外部API: fallback, 消耗token
            AIModel('model_gpt4', 'GPT-4', 'openai', 'llm',
                   'https://api.openai.com/v1', '', 8192, 0.7, False, 3),
            AIModel('model_gpt35', 'GPT-3.5 Turbo', 'openai', 'llm',
                   'https://api.openai.com/v1', '', 4096, 0.7, False, 2),
            AIModel('model_claude', 'Claude 3', 'anthropic', 'llm',
                   'https://api.anthropic.com', '', 8192, 0.7, False, 3),
        ...
        # 默认使用本地推理引擎 (零token消耗)
        self.default_model_id = 'model_local_inference'

        # 创建默认模型组 — 本地推理引擎排第一
        llm_group = ModelGroup('group_llm', 'LLM模型组', 'weighted')
        llm_group.model_ids = ['model_local_inference', 'model_local', 'model_gpt35', 'model_gpt4', 'model_claude']
