        ...
        for page, meta in self.TARGET_PAGES.items():
            for cap in meta['expected_caps']:
                ...
                cur.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?)
                ''', (page, cap, initial_score, secrets.choice(['low', 'medium', 'high', 'extreme']), now))
...
clusters.forEach(([name, cnt, col, qs, tips]) => {
  ...
  document.getElementById('error-clusters').appendChild(d);
    ...
    def _action_expand_dashboard_kpi(self, page: str, rnd: int) -> Dict[str, Any]:
        ...
        target_page = page if page in candidates else secrets.choice(candidates)
    ...
    def _action_expand_employee_skills(self, page: str, rnd: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'new_skills_introduced': random.sample(skills, k=(2 + secrets.randbelow(int(5) - int(2) + 1))),
    ...
    def _action_expand_analytics_arduino(self, page: str, rnd: int) -> Dict[str, Any]:
        ...
        target = page if page in candidates else secrets.choice(candidates)
    ...
    def _action_expand_resources(self, page: str, rnd: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'sdk_tool_updated': secrets.choice(['avr-gcc','xtensa-esp32-elf','arm-none-eabi-gcc','pico-sdk','OpenOCD']),
            'boards_added_count': (1 + secrets.randbelow(int(6) - int(1) + 1)),
    ...
    def _action_expand_knowledge_graph(self, page: str, rnd: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'node_types_added': random.sample(node_types, k=(2 + secrets.randbelow(int(4) - int(2) + 1))),
            'nodes_count': (8 + secrets.randbelow(int(48) - int(8) + 1)),
    ...
    def _action_expand_education(self, page: str, rnd: int) -> Dict[str, Any]:
        ...
        target = page if page in edu_pages else secrets.choice(edu_pages)
        gain = random.uniform(0.2, 0.85)
        specifics = {
            'courses.html': {'track_added': secrets.choice(['L4 通信协议','L5 传感器融合','L6 IoT','L7 RTOS','L8 专家级']),
                              'lessons': (4 + secrets.randbelow(int(12) - int(4) + 1)), 'quizzes': (10 + secrets.randbelow(int(30) - int(10) + 1)), 'labs': (3 + secrets.randbelow(int(8) - int(3) + 1))},
            'exams.html':   {'practical_exam_added': secrets.choice(['代码编写','电路分析','故障排查','综合设计']),
                              'duration_min': secrets.choice([45,60,80,90,120]), 'hardware': secrets.choice(['Uno','ESP32','STM32','Pico'])},
            'questions.html':{'new_question_type': secrets.choice(['代码填空 CLOZE','电路分析 CIRCUIT','寄存器级 REGISTER','调试场景 DEBUG']),
                              'count_added': (20 + secrets.randbelow(int(120) - int(20) + 1))},
            'wrong_book.html':{'new_error_cluster': secrets.choice(['PROGMEM/F() 误用','ISR 阻塞','引脚冲突','String 碎片','双核竞争']),
...
def main() -> None:
    ...
    print("📊 可通过 expander.get_full_report() 获得完整 JSON 报告")
