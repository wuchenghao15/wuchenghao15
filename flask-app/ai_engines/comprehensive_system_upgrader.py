...
class ComprehensiveSystemUpgrader:
    ...
    def _init_database(self):
        ...
        for col_def in [
            ...
            if col_def[0] not in columns:
                cursor.execute(f'ALTER TABLE system_version_history ADD COLUMN {col_def[0]} {col_def[1]}')
    ...
    def start_comprehensive_upgrade(self, phases: List[str] = None) -> UpgradeReport:
        ...
        with self._lock:
            ...
            upgrade_id = f"UPG-{int(time.time())}-{hashlib.sha256(str(random.random()).encode()).hexdigest()[:8]}"
            ...
            try:
                ...
                # ═══════════════════════════════════════════════════════════════
                # 仙女座-阿尔法 AI pre-flight hook
                # 让仙女座自动分析系统状态 → 推荐执行哪些 phase
                # ═══════════════════════════════════════════════════════════════
                try:
                    ...
                    try:
                        ...
                        status_snapshot['neuralhub_routes'] = conn.execute(
                            "SELECT COUNT(*) FROM mt_ai_neural_routes WHERE enabled=1").fetchone()[0]
                        status_snapshot['evolution_logs'] = conn.execute(
                            "SELECT COUNT(*) FROM mt_ai_self_evolution_log").fetchone()[0]
                        status_snapshot['employees'] = conn.execute(
                            "SELECT COUNT(*) FROM mt_andromeda_employee_registry").fetchone()[0]
                        status_snapshot['recent_errors'] = conn.execute(
                            "SELECT COUNT(*) FROM mt_ai_neural_calls WHERE success=0 AND datetime(triggered_at) > datetime('now','-1 hour')"
                        ).fetchone()[0]
                        status_snapshot['calls_last_hour'] = conn.execute(
                            "SELECT COUNT(*) FROM mt_ai_neural_calls WHERE datetime(triggered_at) > datetime('now','-1 hour')"
                        ).fetchone()[0]
                        conn.close()
                    except Exception as _e:
                        status_snapshot['db_error'] = str(_e)[:100]

                    # 让仙女座 AI 推荐计划
                    plan_payload = _json.dumps({
                        "current_phases_requested": target_phases,
                        "available_phases": list(self.upgrade_modules.keys()),
                        "system_status": status_snapshot,
                        "timestamp": _dt.now().strftime("%Y-%m-%d %H:%M:%S"),
                    }, ensure_ascii=False)

                    result = hub.call('system_plan', plan_payload,
                                     flow_id=f'upgrader-{upgrade_id}-preflight')

                    if result.get('success') and result.get('response'):
                        logger.info(f"[仙女座-AI] pre-flight plan: {result['response'][:200]}")
                        report.summary = f"AI recommended: {result['response'][:300]}"
                    sys.path = _prev_path  # 恢复 sys.path
                except Exception as _e:
                    logger.info(f"[仙女座-AI] pre-flight skipped (non-fatal): {_e}")
                # ═══════════════════════════════════════════════════════════════
    ...
    def _record_version_history(self):
        ...
        try:
            ...
            cursor.execute('''INSERT INTO system_version_history
                (version, version_number, previous_version, version_type, changes, ai_suggestions,
                 release_notes, is_active, created_at, is_current)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)''',
                (version, version, self._get_current_version(), 'upgrade',
    ...
    def _upgrade_brain_structure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for category, title, content in knowledge_categories:
                kid = f"KNOW-{int(time.time())}-{hashlib.sha256(title.encode()).hexdigest()[:8]}"
    ...
    def _build_error_classification(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for error_type, desc in error_types:
                sid = f"SOL-{int(time.time())}-{hashlib.sha256(error_type.encode()).hexdigest()[:8]}"
    ...
    def _build_port_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for name, port, protocol in ports:
                pid = f"PORT-{port}-{hashlib.sha256(name.encode()).hexdigest()[:6]}"
                    ...
                    for q_type, templates in question_templates.items():
                        for template in templates[:2]:
                            try:
                                ...
                                qid = f"ADULT-{hashlib.sha256(f'{exam_type}{subject}{q_type}{total_added}'.encode()).hexdigest()[:12]}"
                                ...
                                cursor.execute('''INSERT OR IGNORE INTO adult_education_questions
                                    ...
                                    (qid, exam_type, subject, q_type,
                                     secrets.choice(difficulty_levels),
                                     content, options, 'A',
                ...
                for subject in subjects[:2]:
                    for q_type in question_types[:2]:
                        try:
                            ...
                            qid = f"PROF-{hashlib.sha256(f'{exam_name}{subject}{q_type}{total_added}'.encode()).hexdigest()[:12]}"
    ...
    def _build_knowledge_relations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for source, target, rel_type, weight, desc in relations:
                try:
                    rid = f"REL-{hashlib.sha256(f'{source}_{target}_{rel_type}'.encode()).hexdigest()[:12]}"
                    source_id = f"NODE-{hashlib.sha256(source.encode()).hexdigest()[:12]}"
