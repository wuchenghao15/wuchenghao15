...
class AIQuestionBankService:
    ...
    def _generate_single_choice(self, kp: List[str], difficulty: str, category: str) -> Dict:
        ...
        topic = secrets.choice(list(topics.keys()))
        sub_topic = secrets.choice(topics[topic].get(level, topics[topic]['基础']))
    ...
    def ai_generate_batch(self,
        ...
        for _ in range(count):
            q_type = secrets.choice(types)
            difficulty = secrets.choice(difficulties)
...
question_bank_service = ai_question_bank_service
