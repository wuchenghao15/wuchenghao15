...
class SolutionFinder:
    ...
    def _search_web(self, category: str, issue: Dict) -> Optional[Dict]:
        """从数据库查询已存储的解决方案，不生成模拟数据"""
        logger.info(f"[SolutionFinder] 查询已存储解决方案: {category}")

        search_query = f"{category} security vulnerability fix python flask"

        try:
            result = self._search_solutions_in_db(category)
            if result:
                self._log_search(search_query, len(result['solutions']), True)
                return result

            self._log_search(search_query, 0, False)
        except Exception as e:
            logger.error(f"[SolutionFinder] 查询解决方案失败: {e}")
            self._log_search(search_query, 0, False)

        return None

    def _search_solutions_in_db(self, category: str) -> Optional[Dict]:
        """从数据库查询真实已存储的解决方案"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                row = conn.execute(
                    "SELECT title, solutions, ref_links, source FROM issue_solutions "
                    "WHERE category = ? ORDER BY searched_at DESC LIMIT 1",
                    (category,)
                ).fetchone()
                if row:
                    solutions_list = []
                    try:
                        solutions_list = json.loads(row['solutions']) if row['solutions'] else []
                    except (json.JSONDecodeError, TypeError):
                        solutions_list = [row['solutions']] if row['solutions'] else []
                    ref_links = []
                    try:
                        ref_links = json.loads(row['ref_links']) if row['ref_links'] else []
                    except (json.JSONDecodeError, TypeError):
                        ref_links = [row['ref_links']] if row['ref_links'] else []
                    return {
                        'title': row['title'] or f"{category}解决方案",
                        'solutions': solutions_list,
                        'references': ref_links,
                        'source': row['source'] or 'database'
                    }
        except Exception as e:
            logger.error(f"[SolutionFinder] 数据库查询失败: {e}")
        return None
