...
class Graph(EmbeddingMixin, TraversalMixin):
    ...
    def v(self, vertex=None, func=None, track_paths=True):
        """
        Start a new traversal from a vertex, a list of vertices, or all vertices.

        :param vertex: A vertex ID, a list of vertex IDs, or None (all vertices).
        :param func: Deprecated — use filter() instead.
        :param track_paths: When True (default), hops build per-path Vertex
            objects so that tag(), back(), graph(), and view() work. When False,
            hops use deduplicated ID sets for significantly faster multi-hop
            traversals; tag()/back()/graph()/view() will raise an error.
        :return: self for method chaining.
        """
        if self._cloud:
            self._cloud_reset_chain()
    ...
    def back(self, tag):
        ...
        if not self._track_paths:
            raise RuntimeError(
                "back() requires path tracking. Use v(..., track_paths=True) "
                "(the default) to enable tag/back support."
            )
        vertices = []
        for v in self.last_visited_vertices:
            if tag in v.tags:
    ...
    def tag(self, tag_names):
        ...
        if not self._track_paths:
            raise RuntimeError(
                "tag() requires path tracking. Use v(..., track_paths=True) "
                "(the default) to enable tag/back support."
            )
    ...
    def graph(self):
        ...
        Requires path tracking (the default). If you started the query with
        ``v(..., track_paths=False)``, this method will raise an error.
        ...
        if not self._track_paths:
            raise RuntimeError(
                "graph() requires path tracking. Use v(..., track_paths=True) "
                "(the default) to enable graph/view support."
            )
        nodes = {}
        links = {}
