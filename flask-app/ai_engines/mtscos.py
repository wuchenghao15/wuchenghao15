...
# ---- 服务管理/基础设施模块（已重构到 core.services.*，兼容 fallback）----
# 规则§重构说明: 早期架构用裸顶层 service_manager/backup_manager/api_gateway 等，
# 现统一归置到 core.services.*；mtscos.py 入口做双路径兼容 + 空兜底
def _try_import(name, alt=None):
    """先尝试旧路径(兼容), 再尝试 core.services.*(新架构), 都失败返回 None"""
    for p in ([name] if not alt else [name, alt]):
        try:
            _m = __import__(p, fromlist=[name.split('.')[-1]])
            return getattr(_m, name.split('.')[-1]) if '.' not in p else _m
        except Exception:
            continue
    return None

service_manager = _try_import('core.services.service_manager', 'services.service_manager')
if service_manager is None:
    logger("[入口] service_manager 模块未找到 (历史遗留顶层模块已重构到 core.services，已做空兜底)")

# 以下 5 个同理：历史遗留裸模块，全部空兜底，不阻断主流程
for _dep_name, _alt in [
    ('backup_manager', None), ('api_gateway', None),
    ('notification_center', None), ('data_validator', None), ('file_manager', None),
]:
    _dep = _try_import(_dep_name, _alt)
    if _dep is None:
        logger(f"[入口] {_dep_name} 模块未找到 (历史遗留裸模块, 空兜底不阻断)")
