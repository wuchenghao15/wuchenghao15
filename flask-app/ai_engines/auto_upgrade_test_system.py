    ...
    def check_for_upgrades(self) -> List[Dict]:
        ...
        try:
            ...
            if os.path.exists(db_path):
                ...
                rows = conn.execute(
                    "SELECT version, changelog, requires_restart, download_url, critical, compatibility "
                    "FROM system_upgrades WHERE status='pending' ORDER BY created_at DESC"
                ).fetchall()
                for r in rows:
                    upgrades.append(dict(r))
                conn.close()
        except Exception as e:
            logger.error(f"查询升级记录失败: {e}")
        self.pending_upgrades = upgrades
        return upgrades
