...
Tools:
    ...
    - export_jsonld   : export a KA to JSON-LD (same as `he export jsonld`)
...
def export_jsonld(ka_path: str, output: str, overwrite: bool = False) -> str:
    """Export a knowledge abstract to JSON-LD.

    Args:
        ka_path: Path to the knowledge abstract directory.
        output: Destination ``.jsonld`` file.
        overwrite: Overwrite an existing, non-empty JSON-LD file.

    Uses the same ``export_ka_jsonld`` adapter as ``he export jsonld``.
    Does not create, mutate, or delete the KA.
    """
