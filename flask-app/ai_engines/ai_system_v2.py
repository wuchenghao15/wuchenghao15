...
class AISystem:
    ...
    def _execute_task(self, task: AITask, engine_config: AIEngineConfig,
        ...
        """执行AI任务 - 使用真实AI引擎处理，不返回模拟数据"""
        try:
            from ai_engines.ai_service import ai_service
            result = ai_service.process(task.prompt, engine_type=engine_config.engine_type.value)
            if result and isinstance(result, dict):
                result.setdefault('confidence', engine_state.accuracy)
                return result
            elif result and isinstance(result, str):
                return {'result': result, 'confidence': engine_state.accuracy}
        except Exception as e:
            pass
        return {'error': 'no_ai_engine_available', 'confidence': 0}
