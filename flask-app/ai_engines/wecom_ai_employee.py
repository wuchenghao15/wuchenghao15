...
def _get_db_path_safe() -> Optional[str]:
    """安全获取数据库路径"""
    try:
...
class WeComContactManager(AIEmployee):
    ...
    def _search_contact(self, task_data: Dict) -> Dict[str, Any]:
        ...
        # 缓存没有结果时，从数据库查询真实用户数据
        if not results:
            results = self._search_users_in_db(query)
    ...
    def _search_users_in_db(self, query: str) -> List[Dict]:
        """从数据库查询真实用户数据，不生成模拟数据"""
        results = []
        ...
        try:
            ...
            rows = conn.execute(
                ...
                "WHERE username LIKE ? OR email LIKE ? OR role LIKE ? OR phone LIKE ? "
    ...
    def _get_department(self, task_data: Dict) -> Dict[str, Any]:
        ...
        if db_path and os.path.exists(db_path):
            try:
                ...
                conn.row_factory = sqlite3.Row
                rows = conn.execute(
                    "SELECT role AS name, COUNT(*) AS member_count FROM users "
                    "WHERE role IS NOT NULL AND role != '' GROUP BY role ORDER BY member_count DESC"
                ).fetchall()
                conn.close()
                ...
                logger.error(f"查询部门信息失败: {e}")

        if not departments:
            return {"success": False, "message": "未查询到部门数据"}
    ...
    def _build_org_chart(self, task_data: Dict) -> Dict[str, Any]:
        ...
        if db_path and os.path.exists(db_path):
            try:
                ...
                conn.row_factory = sqlite3.Row
                rows = conn.execute(
                    "SELECT role AS name, COUNT(*) AS cnt FROM users "
                    "WHERE role IS NOT NULL AND role != '' GROUP BY role ORDER BY cnt DESC"
                ).fetchall()
                conn.close()
                for r in rows:
                    total_employees += r['cnt']
                    departments.append({"name": r['name'], "count": r['cnt']})
            except Exception as e:
                logger.error(f"构建组织架构图失败: {e}")

        if not departments:
            return {"success": False, "message": "未查询到组织架构数据"}

        children = []
        nodes = [{"id": "1", "label": "组织总览", "level": 0}]
        edges = []
        for idx, dept in enumerate(departments, 2):
            ...
            nodes.append({"id": str(idx), "label": dept['name'], "level": 1})
            edges.append({"from": "1", "to": str(idx)})

        org_chart = {
            "root": {"id": "1", "name": "组织总览", "type": "company"},
            "children": children,
