...
def detect_model_stealing(query_patterns: List[Dict]) -> Dict:
    ...
    if total_queries > 100:
        ...
        for q in query_patterns:
            input_hash = hashlib.sha256(json.dumps(q.get('input', ''), sort_keys=True).encode()).hexdigest()
...
class AISecurityGuard:
    ...
    def scan_input(self, text: str, client_id: str = None,
                  ...
                  baseline_stats: Dict = None) -> Dict:
        """综合输入安全扫描"""
        scan_id = f"SEC-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
        findings = []
        max_score = 0
    ...
    def scan_output(self, text: str, client_id: str = None) -> Dict:
        scan_id = f"SEC-OUT-{datetime.now().strftime('%Y%m%d%H%M%S')}-{(1000 + secrets.randbelow(int(9999) - int(1000) + 1))}"
