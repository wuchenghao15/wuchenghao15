...
from ai_engines.local_ai_unified_gateway import chat as local_ai_chat
...
class AIBrainEnhancer:
    ...
    def _integrate_external_ai_engines(self):
        """整合本地AI引擎 (零token优先)"""
        logger.info("开始整合本地AI引擎")

        test_prompt = "请简要解释AI脑库的概念和作用"
        test_result = local_ai_chat(test_prompt, flow_id="brain_enhancer_integrate")

        if test_result and test_result.get("success"):
            ai_brain_service.add_knowledge(
                title="AI脑库概念解释",
                content=test_result.get("response", ""),
                knowledge_type="concept",
                source="local_ai",
                source_id=test_result.get("model", "qwen2.5:7b"),
                tags=["ai_brain", "concept", "local_ai"]
            )

        self.enhancement_history.append({
            "type": "integrate_local_ai_engines",
            "timestamp": datetime.now().isoformat(),
            "details": "整合了本地AI引擎, 优先使用本地Ollama(qwen2.5:7b)零token推理, 火山引擎兜底",
            "route": test_result.get("route", "none") if test_result else "none",
            "tokens_saved": test_result.get("tokens_saved", 0) if test_result else 0
        })

        logger.info("完成整合本地AI引擎")
