...
def _get_settings_db_path() -> Optional[str]:
    """获取设置数据库路径"""
    try:
...
class SyncManager:
    ...
    def _sync_setting(self, setting: Setting):
        ...
        try:
            db_path = _get_settings_db_path()
            if not db_path or not os.path.exists(db_path):
                setting.sync_status = SettingSyncStatus.CONFLICT
                logger.error(f"数据库不可用，同步失败: {setting.key}")
                return
            ...
            cursor.execute('''
                ...
                (key, name, type, scope, value, default_value, description, options,
                 ai_recommended, ai_score, ai_reason, created_at, updated_at,
            ...
            ''', (
                ...
                setting.type.value,
                setting.scope.value,
                json.dumps(setting.value, ensure_ascii=False) if setting.value is not None else None,
                json.dumps(setting.default_value, ensure_ascii=False) if setting.default_value is not None else None,
                setting.description,
                json.dumps(setting.options, ensure_ascii=False) if setting.options else None,
                1 if setting.ai_recommended else 0,
                setting.ai_score,
                setting.ai_reason,
                setting.created_at,
                setting.updated_at,
                datetime.now().isoformat(),
                SettingSyncStatus.SYNCED.value,
                json.dumps(setting.metadata, ensure_ascii=False) if setting.metadata else None
            ))
            conn.commit()
            conn.close()
