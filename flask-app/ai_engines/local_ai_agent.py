...
# 数据库与运行目录锚定到本文件所在目录, 避免相对路径随CWD漂移
_ENGINE_DIR = os.path.dirname(os.path.abspath(__file__))
_DB_PATH = os.path.join(_ENGINE_DIR, 'data', 'ai_agent.db')
...
class LocalAIAgentEngine:
    ...
    def _gateway_chat(self, prompt: str, flow_id: str) -> Optional[Dict]:
        """统一走本地AI网关 (Ollama零token优先 → 火山兜底), 失败返回None"""
        try:
            from local_ai_unified_gateway import chat as gateway_chat
            result = gateway_chat(prompt, "你是MTSCOS系统的本地AI员工, 输出简洁准确的中文结果。", flow_id)
    ...
    def _auto_fix_problems(self, problems):
        ...
        for problem in problems:
            ...
            if risk in ['high', 'medium']:
                # 本地AI网关诊断 (Ollama零token优先), 拿不到推理结果则如实标记待人工
                diag = self._gateway_chat(
                    f"分析以下系统问题并给出修复建议(不超过5行):\n{title}\n详情: {json.dumps(problem, ensure_ascii=False)}",
                    "local_ai_agent_auto_fix"
                )
                if diag:
                    fixed.append({
                        'problem': title,
                        'status': 'diagnosed',
                        'method': f"local_ai_gateway({diag.get('route', '')})",
                        'diagnosis': diag.get('response', '')[:500]
                    })
                else:
                    fixed.append({
                        'problem': title,
    ...
    def _expand_knowledge(self, topics):
        expanded = []
        for topic in topics:
            # 本地AI网关生成真实知识内容 (Ollama零token优先), 失败降级模板
            gen = self._gateway_chat(
                f"针对主题「{topic}」写一条不超过200字的知识条目, 包含定义与要点。",
                "local_ai_agent_expand_knowledge"
            )
            if gen:
                entry = {
                    'id': str(uuid.uuid4()),
                    'key': topic,
                    'value': gen.get('response', '')[:1000],
                    'category': 'expanded',
                    'source': f"local_ai_gateway:{gen.get('route', '')}",
    ...
    def _health_check(self):
        ...
        try:
            ...
            conn.close()
            checks.append({'component': 'database', 'status': 'healthy', 'path': _DB_PATH})
        except Exception as e:
            checks.append({'component': 'database', 'status': 'unhealthy', 'error': str(e)})
        # 本地AI网关真实健康检查
        try:
            from local_ai_unified_gateway import health as gateway_health
            gh = gateway_health()
            ollama_ok = bool(gh.get('local_ollama', {}).get('available'))
            checks.append({
                'component': 'local_ai_gateway',
                'status': 'healthy' if ollama_ok else 'degraded',
                'route': 'local_ollama' if ollama_ok else 'fallback',
                'detail': gh.get('local_ollama', {})
            })
        except Exception as e:
            checks.append({'component': 'local_ai_gateway', 'status': 'unhealthy', 'error': str(e)})
