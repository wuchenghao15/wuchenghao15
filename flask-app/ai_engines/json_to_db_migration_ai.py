...
class JsonToDbMigrationAI:
    ...
    def migrate_services_config(self):
        ...
        try:
            ...
            # 插入默认配置
            default_services = [
                ('web_server', str({"host": "0.0.0.0", "port": 5000, "debug": False}), 'running'),
                ('ai_engine', str({"enabled": True, "timeout": 30}), 'running'),
                ('cache', str({"enabled": True, "size": 1024}), 'running')
            ]
    ...
    def report_to_database(self):
        ...
        try:
            ...
            cursor.execute("INSERT INTO json_to_db_migration (migration_id, json_files_count, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)", (
