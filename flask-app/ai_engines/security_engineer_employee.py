                    ...
                    try:
                        with urllib.request.urlopen(req, timeout=5) as response:
                            ...
                            if status_code >= 200 and status_code < 300:
                                results.append({
                                    'test_id': f"api_{hashlib.sha256(test['endpoint'].encode()).hexdigest()[:8]}",
                                    'test_name': test['name'],
                                    'test_type': 'functional',
            ...
            for test in db_tests:
                ...
                try:
                    if os.path.exists(db_full_path):
                        ...
                        results.append({
                            'test_id': f"db_{hashlib.sha256(test['db_path'].encode()).hexdigest()[:8]}",
                            'test_name': test['name'],
                            'test_type': 'functional',
                            'priority': test['priority'],
                            'status': 'passed',
        ...
        try:
            ...
            for test in ai_tests:
                ...
                try:
                    ...
                    results.append({
                        'test_id': f"ai_{hashlib.sha256(test['module'].encode()).hexdigest()[:8]}",
                        'test_name': test['name'],
                        'test_type': 'functional',
                        'priority': test['priority'],
                        'status': 'passed',
