...
class AIMonitor:
    ...
    def __init__(self):
        ...
        # ====== VIKEY 告警防刷屏优化（F3 专项） ======
        # 探测最小间隔：调试环境下不插狗是常态，不需要每3秒打一次驱动
        self._vikey_min_probe_interval = 60.0  # 秒，至少60秒才探测一次硬件
        # 告警抑制：连续异常已触发CRIT后，至少间隔 N*probe_interval 才再打一条告警
        self._vikey_alert_suppress_factor = 5  # 5*60=5分钟才提醒一次
        self._vikey_last_alert_time = 0.0     # 上次发 CRIT 告警的绝对时间戳
        self._vikey_alert_counter = 0         # 已抑制的告警数（打日志时一次性说明）
    ...
    def _check_vikey_health(self):
        """VIKEY硬件密钥健康检测（MTS架构v2.0新增）——F3优化版
        优化点：
        1) 探测间隔拉长到 _vikey_min_probe_interval（默认60秒），避免每3秒打一次驱动
        2) 达到连续异常>=3后，告警抑制（默认5倍间隔=5分钟一条），用DEBUG级日志替代刷屏WARNING
        3) 调试环境未插狗（present=False且有间隔保护）直接降级到 DEBUG，不打 WARNING
        """
        now = time.time()
        # ===== F3优化1：最小探测间隔节流 =====
        last_probe = self.vikey_status.get('last_probe_time') or 0
        if (now - last_probe) < self._vikey_min_probe_interval:
            # 间隔内直接复用上次状态，不打驱动
            return
        ...
        try:
            ...
            if abnormal:
                ...
                if self.vikey_status['consecutive_anomalies'] >= 3:
                    # ===== F3优化2：告警抑制（默认5倍间隔=5分钟一条WARNING）=====
                    suppress_window = self._vikey_min_probe_interval * self._vikey_alert_suppress_factor
                    if (now - self._vikey_last_alert_time) >= suppress_window:
                        suppressed = self._vikey_alert_counter
                        self._vikey_alert_counter = 0
                        self._vikey_last_alert_time = now
                        msg = (f"VIKEY硬件密钥异常: present={present}, bound={bound}, "
                               f"serial={serial}, 连续异常={self.vikey_status['consecutive_anomalies']}次"
                               + (f" (期间已抑制{suppressed}条刷屏告警)" if suppressed > 0 else ""))
                        self._create_alert('vikey', 'CRITICAL', msg)
                        with self.error_lock:
                            self.error_count['vikey'] += 1
                            self.metrics['total_errors'] += 1
                    else:
                        # 在抑制窗口内：仅记录内部计数，降级到 DEBUG，不刷屏 WARNING
                        self._vikey_alert_counter += 1
                        logger.debug(
                            f"[vikey-suppressed #{self._vikey_alert_counter}] "
                            f"present={present} bound={bound} 连续异常={self.vikey_status['consecutive_anomalies']}"
                        )
            else:
