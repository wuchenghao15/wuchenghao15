...
class EnhancedAIEmployee(BaseModel):
            ...
            if key in ['capabilities', 'config'] and value:
                if isinstance(value, str):
                    ...
                    try:
                        ...
                        try:
                            result[key] = _ast.literal_eval(value)
                        except (ValueError, SyntaxError):
                            result[key] = value
                else:
                    result[key] = value
    ...
    def upgrade_system_version(self):
        ...
        service = get_system_version_service()
        handler = getattr(service, 'upgrade_system_version', None)
        if not callable(handler):
            # 正规版本服务(VersionService)无自动升级入口；版本号 bump 属高风险操作，
            # 须走 §14 十二步骤与正规版本管理器，禁止 AI 员工擅自升级，安全拦截避免 AttributeError
            logger.warning(
                f"AI员工 {self.name} 请求版本升级，但正规版本服务无自动升级入口，已安全拦截"
                f"(版本升级须走§14十二步骤流程)"
            )
            return {
                'success': False,
                'code': 'VERSION_UPGRADE_NOT_AUTOMATED',
                'message': '系统版本升级不支持AI员工自动执行，须走§14十二步骤与正规版本管理器'
            }
        result = handler()
