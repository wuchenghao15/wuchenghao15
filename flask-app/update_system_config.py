...
def update_system_config():
    ...
    configs_to_update = [
        ...
        # 题目生成配置参数
        ("supported_languages", json.dumps(["japanese", "english", "chinese"]), "array", "支持的语言列表"),
        ("supported_categories", json.dumps(["词汇", "语法", "阅读", "听力", "写作", "口语", "翻译"]), "array", "支持的题目类别列表"),
        ("supported_question_types", json.dumps(["single", "multiple", "fill", "short_answer", "essay", "speaking", "translation"]), "array", "支持的题目类型列表"),
...
def check_updated_configs():
    ...
    # 查询所有配置
    cursor.execute("SELECT config_key, config_value, config_type, description FROM system_config WHERE is_active = 1")
