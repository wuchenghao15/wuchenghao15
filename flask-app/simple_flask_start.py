...
def custom_json_response(data, status_code=200, pretty=True, include_context=False):
    ...
    try:
        ...
        error_response = make_response(
            json.dumps(error_data, ensure_ascii=False, indent=2, sort_keys=True)
        )
...
def list_ips():
    """获取IP列表"""
    ip_type = request.args.get('type', 'whitelist')
    ips = ip_manager.get_list(ip_type)
