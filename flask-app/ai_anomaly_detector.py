...
class EnhancedAIAnomalyDetector:
    ...
    def _init_db(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_timestamp ON anomaly_logs(timestamp)')
    ...
    def _log_anomaly(self, client_ip, action, is_anomalous, anomaly_score, anomaly_details, 
        ...
        try:
            cursor.execute('''
                ...
                (client_ip, action, is_anomalous, anomaly_score, anomaly_details, 
                user_agent, path, metadata)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                client_ip, action, 1 if is_anomalous else 0, anomaly_score, 
                anomaly_details, user_agent, path, json.dumps(metadata)
            ))
            conn.commit()
        except Exception as e:
    ...
    def _calculate_anomaly_score(self, client_ip, attempt_record):
        ...
        # 1. 快速连续尝试因子
        recent_attempts_count = len(client_history['login_attempts'])
        ...
        self._log_anomaly(
            ...
            metadata={
                'client_history': {
                    ...
                    'recent_attempts': len(client_history['login_attempts'])
                },
