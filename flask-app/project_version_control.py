...
class ProjectVersionControl:
    ...
    def rollback_to_version(self, version_index):
        ...
        if version_index < 0 or version_index >= len(history['versions']):
    ...
    def _analyze_change_trend(self, versions, file_changes):
        ...
        # 计算平均变更数
        avg_changes = sum(vc['changes_count'] for vc in version_changes) / len(version_changes)
        ...
        return {
            ...
            'least_changes_version': least_changes,
    ...
    def export_version_summary(self, summary=None, start_version=None, end_version=None):
        ...
        logger.info(f"导出版本总结报告成功: {summary_path}")
        return summary_path
...
| 最长版本间隔 (小时) | {summary['version_statistics']['longest_version_interval_hours']} |

## 变更趋势分析

- **总体趋势**: {summary['change_trend']['trend']}
- **平均每个版本变更文件数**: {summary['change_trend']['average_changes_per_version']}

### 变更最多的版本
- **版本名称**: {summary['change_trend']['most_changes_version']['version_name']}
- **变更文件数**: {summary['change_trend']['most_changes_version']['changes_count']}
...
"""
        ...
        for version in summary['versions_summary']:
    ...
    def save_version_summary_markdown(self, start_version=None, end_version=None):
        ...
        logger.info(f"保存Markdown版本总结报告成功: {md_path}")
        return md_path
    ...
    def get_version_difference(self, version1_index, version2_index):
        ...
        # 比较核心文件
        version1_files = set(version1['core_files'].keys())
        ...
        for file in common_files:
            if version1['core_files'][file] != version2['core_files'][file]:
        ...
        difference = {
            ...
            'total_differences': len(file_differences['added']) + len(file_differences['modified']) + len(file_differences['removed']),
            'difference_summary': {
...
def main():
    ...
    while True:
        ...
        if choice == '0':
            ...
            operation_type = input("请输入操作类型: ").strip()
            ...
            for file, info in state['core_files'].items():
                ...
                print(f"  {file}: {status}, 大小: {info['size']} 字节")
