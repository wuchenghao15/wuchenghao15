...
class AIEmployeeManager:
    ...
    def get_employees_by_type(self, employee_type: str) -> list:
        ...
        return [self.employees[emp_id] for emp_id in employee_ids]
    ...
    def auto_assign_task(self, task_data: dict, required_level: int = 1) -> dict:
        ...
        if task_type in ["login", "register", "request"]:
            ...
            required_employee_type = "routing"
        elif task_type in ["manage_parameters", "upload_data", "analyze_performance", "self_upgrade", 
                          "generate_test_content", "create_test_page_config", "optimize_test_page", 
                          "analyze_test_results", "get_test_page_config", "maintain_question_bank", 
                          ...
                          "generate_topic_explanation", "analyze_user_weaknesses", "get_recommended_topics", 
                          "analyze_student_preferences", "optimize_learning_path", "personalize_recommendations"]:
    ...
    def optimize_performance(self) -> dict:
        ...
        # 1. 清理不活跃的AI员工
        inactive_employees = [emp for emp in self.employees.values() if emp.status == "inactive"]
        if inactive_employees:
            for emp in inactive_employees:
                self.remove_employee_from_organizations(emp.employee_id)
                emp.stop()
                del self.employees[emp.employee_id]
        ...
        for employee_id, employee in self.employees.items():
            ...
            if employee.performance_score >= 80 and employee.level < 10:
                ...
                optimization_results["optimizations"].append(f"AI员工 {employee_id} 因高性能自动升级到级别 {employee.level}")
...
if __name__ == "__main__":
    ...
    result = manager.auto_assign_task(test_task_data, required_level=3)
