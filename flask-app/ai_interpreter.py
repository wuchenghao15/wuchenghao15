...
class Parser:
    ...
    def eat(self, token_type: str) -> None:
        ...
        if self.current_token.token_type == token_type:
            ...
            raise SyntaxError(f"语法错误: 期望 {token_type}，但得到 {self.current_token.token_type}")
        ...
        while self.current_token.token_type != 'EOF':
            if self.current_token.value.lower() in flag_options:
                ...
                if self.current_token.token_type in ['STRING', 'NUMBER', 'IDENTIFIER']:
                    ...
                    raise SyntaxError(f"语法错误: 期望选项值，但得到 {self.current_token.token_type}")
            else:
                logger.error(f"语法错误: 期望选项，但得到 {self.current_token.value}")
                raise SyntaxError(f"语法错误: 期望选项，但得到 {self.current_token.value}")
...
class CommandHandler:
    ...
    def handle_update(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            from ai_brain_updater import AIBrainUpdater
    ...
    def handle_query(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if keyword:
                ...
                # 如果没有关键字，获取热门知识
                knowledge_items = AIBrainSearchEnhancer.get_trending_knowledge(limit=limit)
                result = {
                    'type': 'trending',
                    'limit': limit,
                    'count': len(knowledge_items),
